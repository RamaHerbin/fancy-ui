/**
 * The action every "reveal on scroll" / "notice when visible" component in
 * this family uses instead of hand-rolling its own IntersectionObserver.
 * Four components elsewhere in this repo (blur-reveal, box-reveal,
...
export const inView: Action<Element, InViewOptions> = (node, opts) => {
	if (typeof window === "undefined" || typeof IntersectionObserver === "undefined") {
		...
		// page. There is nothing to tear down, so no update()/destroy() is
		// returned at all.
		opts.onChange(true);
		return {};
	}
