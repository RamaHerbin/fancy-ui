...
- **Exactly two children** sit directly under the root: an items-column wrapper and the panel. In the wide layout, this is a CSS Grid: one `grid-template-columns: repeat(auto-fit, minmax(min(100%, 20rem), 1fr))` track list does double duty as both the two-column layout AND (down to a point) its own single-column stacking fallback, with no explicit `grid-column` placement to keep in sync — an explicit column assignment on either item would make CSS Grid manufacture a real extra column instead of stacking when the container is narrow.
- **`panelSide` is placed via CSS `order`**, not DOM reordering — the panel is always rendered after the items in source order, and `order` alone decides whether it displays first or second. Combined with the grid's native RTL-aware inline flow, this makes `panelSide="start"`/`"end"` genuinely logical without a separate `:dir(rtl)` override.
- **The stacked-layout breakpoint is a `@container` query** (`container-type: inline-size` on the root), not a viewport media query, so it reacts to the _container's_ width rather than the viewport's. The threshold (`42rem`) is chosen to match the grid's own `auto-fit` collapse point for the **default** gap — the arithmetic is `2 × (20rem + gap) − gap`, which is `42rem` at `gap: 2rem`, not the gap-blind `40rem` "two 20rem tracks" reading might suggest. A `--ft-stickyscroll-gap` override moves the grid's real collapse point but not this query — a container query condition can't read a custom property — so a large enough override can put the two out of sync by that same amount.
- **The stack is a real layout switch, not just a size change.** Below the threshold, `.ft-stickyscroll` itself switches from `display: grid` to `display: flex; flex-direction: column`. This is what gives the panel actual sticky travel once stacked: a grid item's containing block is its own grid area — in a collapsed single-column grid that area is auto-sized to the panel's own content, so `position: sticky; top: 0` would compile but have zero room to travel in. A flex item's containing block is the whole flex container's content box instead, which does give it somewhere to stick. The panel is placed first visually via `order: -1` (DOM order is untouched, so the screen-reader/tab sequence stays narrative-then-summary), and each item gets `scroll-margin-block-start` matching the stacked panel's height so that focusing or anchoring into a row doesn't park it underneath the now-overlaying panel. On a browser without container query support, the panel simply keeps its two-column sizing and position even once the grid has stacked — a cosmetic difference, not a broken layout.
