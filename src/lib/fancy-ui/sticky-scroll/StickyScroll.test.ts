...
import StickyScrollHarness from "./StickyScrollHarness.test.svelte";
 ...
 * constructed instance (one per `<section>`, since `use:inView` is applied
...
describe("StickyScroll", () => {
	...
	it("renders one section per item, and the panel with the active item's content", () => {
		const { container } = render(StickyScrollHarness, {
			props: { items: items(3) },
		});
	...
	it("passes (item, index, active) to the item snippet — active true only for the current row", () => {
		const { container } = render(StickyScrollHarness, {
			props: { items: items(3), activeIndex: 1 },
		});
	...
	it("panelSide defaults to end and reflects into data-panel-side", () => {
		...
		const { container: start } = render(StickyScrollHarness, {
			props: { items: items(2), panelSide: "start" },
		});
	...
	it("panelHidden defaults to true (aria-hidden) and can be turned off", () => {
		...
		const { container: shown } = render(StickyScrollHarness, {
			props: { items: items(2), panelHidden: false },
		});
	...
	it("reduced motion: the panel swap is synchronous — Svelte's own duration:0 fast path, no Element.prototype.animate call", async () => {
		...
		const { container, rerender } = render(StickyScrollHarness, {
			props: { items: items(3), activeIndex: 0 },
		});
		await tick();
	...
	it("with default props (motion on), the panel swap actually crossfades: Element.prototype.animate runs and both frames briefly coexist", async () => {
		...
		const rerendered = rerender({ items: items(3), activeIndex: 1 });
		...
		// This has to be observed before awaiting the rerender/tick settles,
		// since the WAAPI stub in test-setup.ts finishes on a microtask that
		// a later `await` would otherwise let run first.
	...
	it("crossfade={false} also takes the synchronous path, independent of reduced motion", async () => {
		const animateSpy = vi.spyOn(Element.prototype, "animate");
		const { container, rerender } = render(StickyScrollHarness, {
			props: { items: items(2), activeIndex: 0, crossfade: false },
		});
		await tick();
	...
	it("activeIndex clamps for panel rendering if it is out of range for a shrunk items list", async () => {
		const { container, rerender } = render(StickyScrollHarness, {
			props: { items: items(3), activeIndex: 2 },
		});
	...
	it("two instances on the same page don't cross-talk: triggering one's IO mock leaves the other's activeIndex untouched", async () => {
		const { container: first } = render(StickyScrollHarness, { props: { items: items(2) } });
		const { container: second } = render(StickyScrollHarness, { props: { items: items(2) } });
	...
	it("cleanup: unmount disconnects every section's observer", () => {
		const { unmount } = render(StickyScrollHarness, {
			props: { items: items(4) },
		});
