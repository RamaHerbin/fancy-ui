...
- The timestamp renders through the shared `formatRelativeTime` helper and sits in a `<time datetime>` element whose `title` is the ISO string, so the exact moment is one hover away.
