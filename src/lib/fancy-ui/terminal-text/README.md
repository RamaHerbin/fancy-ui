...
- Streaming and glitching are two independent `$effect`s. The streaming effect depends on `lines`, `speed`, and `delay` only (`void`-read to declare the dependency without using the value), so toggling `glitch` mid-stream does not restart the text. The glitch effect depends only on `glitch` and starts/stops its own self-rescheduling loop (`scheduleGlitch`, random 2–4s interval). That loop owns its timer handles separately from the streaming pool (`stopGlitchLoop`, called from the glitch effect's teardown), so restarting the stream never kills a pending reschedule and toggling `glitch` off and back on never stacks a second loop — exactly one chain is alive at a time, and none survives unmount.
