...
describe("ScrollProgress", () => {
	...
	afterEach(() => {
		...
		// calls too (the same lesson transitions.test.ts documents for
		// Element.prototype.animate).
		vi.restoreAllMocks();
	});

	it("defaults to JS mode — jsdom has no CSS.supports, so the @supports gate can never pass", async () => {
		mockWindowScroll(0);
	...
	it("CSS mode: no target, no label, and a stubbed CSS.supports — no scroll/resize listeners attach", async () => {
	...
	it("a `target` forces JS mode even when CSS.supports would otherwise allow the zero-JS path", async () => {
		vi.stubGlobal("CSS", { supports: () => true });
		const target = document.createElement("div");
		document.body.appendChild(target);
		mockScrollable(target, { scrollTop: 0, scrollHeight: 100, clientHeight: 100 });

		const { container } = render(ScrollProgress, { props: { target } });
		vi.advanceTimersToNextFrame();
	...
	it("mode is derived, not latched: a `target` handed to the component AFTER mount flips it from css to js and attaches a scroll listener", async () => {
		vi.stubGlobal("CSS", { supports: () => true });
		const target = document.createElement("div");
		document.body.appendChild(target);
		mockScrollable(target, { scrollTop: 0, scrollHeight: 100, clientHeight: 100 });
		...
		// A target arrives afterward (e.g. a `bind:this` element resolved on a
		// later tick, or an `{#if}`-gated scroller). If `mode` were latched at
		// `onMount` this would never be reflected.
	...
	it("a `label` forces JS mode and renders progressbar ARIA + an initial aria-valuenow", async () => {
		mockWindowScroll(0);
		Object.defineProperty(document.documentElement, "scrollHeight", {
			value: 1000,
			configurable: true,
		...
		const { container } = render(ScrollProgress, { props: { label: "Reading progress" } });
		vi.advanceTimersToNextFrame();
	...
	it("position sets data-position", () => {
		...
		const { container: bottom } = render(ScrollProgress, { props: { position: "bottom" } });
		expect(root(bottom).dataset.position).toBe("bottom");
		cleanup();

		const { container: inline } = render(ScrollProgress, { props: { position: "inline" } });
		expect(root(inline).dataset.position).toBe("inline");
	});

	it("JS mode: a scroll event, throttled to a frame, writes --ft-scrollprogress-value and rounds aria-valuenow", async () => {
		mockWindowScroll(0);
		Object.defineProperty(document.documentElement, "scrollHeight", {
			value: 2000,
			configurable: true,
		...
		const { container } = render(ScrollProgress, { props: { label: "Progress" } });
		...
		vi.advanceTimersToNextFrame();
		// The CSS var write bypasses reactivity (direct `style.setProperty`,
		// visible immediately); `aria-valuenow` goes through `$state` →
		// template, which patches the DOM on Svelte's own microtask flush —
		// a real `tick()` is required to observe it, not just the rAF advance.
		await tick();
	...
	it("a burst of scroll events before the frame runs still only paints once, with the latest value", async () => {
		mockWindowScroll(0);
		Object.defineProperty(document.documentElement, "scrollHeight", {
			value: 2000,
			configurable: true,
	...
	it("clamps a scroll position beyond the scrollable range to [0, 1]", async () => {
		const target = document.createElement("div");
		document.body.appendChild(target);
		mockScrollable(target, { scrollTop: 99999, scrollHeight: 1000, clientHeight: 500 });

		const { container } = render(ScrollProgress, { props: { target, label: "P" } });
		vi.advanceTimersToNextFrame();
	...
	it("content with nothing to scroll reads as 0, never NaN", async () => {
		mockWindowScroll(0);
		Object.defineProperty(document.documentElement, "scrollHeight", {
			value: 500,
			configurable: true,
		...
		const { container } = render(ScrollProgress, { props: { label: "P" } });
		vi.advanceTimersToNextFrame();
	...
	it("`target` mode attaches listeners on the target, not window", async () => {
		const target = document.createElement("div");
		document.body.appendChild(target);
		mockScrollable(target, { scrollTop: 0, scrollHeight: 100, clientHeight: 100 });
		const targetAdd = vi.spyOn(target, "addEventListener");
		...
		render(ScrollProgress, { props: { target } });
		vi.advanceTimersToNextFrame();
	...
	it("registers scroll/resize as passive listeners", async () => {
		const addSpy = vi.spyOn(window, "addEventListener");

		render(ScrollProgress, { props: { label: "P" } });
		vi.advanceTimersToNextFrame();
	...
	it("reduced motion does not disable JS-mode tracking — ScrollProgress is not gated by that preference", async () => {
	...
	it("cleanup: unmount removes the scroll/resize listeners and cancels the pending frame", async () => {
		mockWindowScroll(0);
		Object.defineProperty(document.documentElement, "scrollHeight", {
			value: 2000,
			configurable: true,
	...
	it("merges a custom class alongside ft-scrollprogress", () => {
		const { container } = render(ScrollProgress, { props: { class: "my-bar" } });
