 ...
 * contract: lazy hydration, persistence, preference mutation, subscribers
 * and cross-tab sync.
 */
...
describe("sound controller", () => {
	...
	// "First access from inside a $derived must not throw" is proven by
	// SoundToggle.test.ts ("renders against the real controller…"): runes are
	// not available in a .test.ts module, and SoundToggle's `$derived(sound.enabled)`
	// is exactly that first access.
...
describe("sound controller — lazy hydration at the module boundary", () => {
	...
	it("does not read localStorage on import; the first property access reads it exactly once", async () => {
		...
		vi.resetModules();
		const mod = await import("./sound.svelte.js");
