...
- The stopwatch is built with `createElapsed()` and starts at `0`, never seeded from `since`: reading the wall clock at construction happens once on the server and again at hydration, and the two can land on different seconds, which would mismatch the markup being hydrated. The ticking effect corrects it to the real elapsed time immediately after mounting — client-only, since effects never run during SSR.
