 ...
 * archetype. Magnetic constructs its observer by calling the `inView` action
 * as a plain function (not via a template `use:` directive, see the
...
const CHILD = () => snippet('<button type="button">Hover me</button>');
...
/**
 * `magnetState`'s `data-state` binding is a Svelte-templated reactive
 * attribute, updated through Svelte's own effect scheduler — unlike the
 * `--ft-magnetic-x/y` vars (written by a direct, synchronous
 * `style.setProperty` call), a `$state` write made from a raw
 * `window`/`document` listener (attached outside any Svelte-compiled
 * `on:`/`onclick=` handler) does not flush the DOM until the next
 * microtask. Every dispatch that can flip `data-state` runs the frame queue
 * and then awaits `tick()` before assertions read the DOM.
 */
...
describe("Magnetic", () => {
	...
	it("toggling disabled on mid-interaction tears the field down and releases", async () => {
		...
		const removeSpy = vi.spyOn(window, "removeEventListener");
		await rerender({ children: CHILD(), disabled: true });
		...
		await rerender({ children: CHILD(), disabled: false });
		MockIntersectionObserver.instances.at(-1)!.trigger(true);
		await moveAndFlush(130, 125);
		expect(outer.dataset.state).toBe("active");
	});
	...
	it("a radius prop change rebuilds the field with the new value", async () => {
		...
		await rerender({ children: CHILD(), radius: 80 });
		MockIntersectionObserver.instances.at(-1)!.trigger(true);
		await moveAndFlush(151, 120);
