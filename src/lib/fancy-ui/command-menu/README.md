...
- `role="dialog"` with `aria-modal="true"` and `aria-label={label}` (default `"Command menu"`). Focus moves into the search field on open and returns to whatever had focus before it opened at the instant you dismiss — not when the closing animation finishes, which would leave a keyboard user on `<body>` for the length of it. Tab is trapped inside the panel while it is open, and the closing panel is `inert`, so it is never in the tab order on its way out.
...
The panel and its backdrop arrive together over 300 ms on the shared arrival curve (`DURATIONS.base` and `JS_EASINGS.out` from the motion foundation) and leave together over 200 ms on the departure curve (`DURATIONS.exit`, `JS_EASINGS.in`). That is the **modal** rung — the one dialogs are on, not the 150 ms rung the dropdown and context menus use: a centred, backdrop-covered, focus-trapping, scroll-locking surface belongs with dialogs rather than with a menu hanging off a button. The panel scales as well as fades, growing from a `0.92` floor on the way in and collapsing only to `0.96` on the way out, because leaving is a smaller gesture than arriving. The backdrop fades on opacity alone: a full-viewport fixed element has no business acquiring a compositing layer for a transform it never uses.

Both are Svelte transitions, not CSS animations, so there is no `--ft-*` variable to override here; the timing comes from the shared token ladder and moves with it.

The close is where the work is. `open` still flips the instant you dismiss — nothing a caller can observe waits for the fade, `onOpenChange` included — but the panel stays mounted while it plays, and four things happen at the dismiss instant rather than at the end of it:

- **Focus comes back immediately.** Whatever had focus before the menu opened is refocused as the exit starts, not when it finishes. Waiting would leave a keyboard user on `<body>` for the whole 200 ms, because the closing panel is made `inert` the moment the exit begins.
...
- **The page stays locked until the backdrop is gone.** The scroll lock releases at unmount, not at dismiss, so the page behind can never be scrolled while a backdrop is still on screen. This replaced an effect that released the instant `open` flipped, roughly 200 ms early.
- **Reopening mid-exit reverses rather than stacking.** One bidirectional transition per surface, so a menu reopened while it is fading continues from wherever it is instead of snapping to invisible first. Focus follows it back: a reversed exit is not a fresh mount, so the focus trap is re-armed as the entrance restarts.

The panel reports `data-state="open"` while it is open and `data-state="closing"` for the length of the exit, should you want to key your own styling off either.

- **Reduced motion** — both transitions collapse to a duration of zero, which makes the framework skip the animation entirely. The menu appears and disappears instantly, the close is fully synchronous again, and the scroll lock is released in the same tick — exactly the behaviour this component had before it animated at all. Neither surface has a hidden resting state, so nothing is ever left invisible waiting for an animation that will not run.
- **Touch and coarse pointers** — unchanged; neither the entrance nor the exit is pointer-gated.

One older bug went with the old keyframes, and it is worth naming because the fix looks like a pure motion change: the entrance restated the panel's centring `translateX(-50%)` inside a `transform` keyframe, on an element whose centring actually comes from Tailwind v4's separate `translate` property. The two composed, so the panel drifted in from half its own width to the left. A `transform` that is scale and nothing else composes after `translate` and scales the panel about its own centre without disturbing the centring.
