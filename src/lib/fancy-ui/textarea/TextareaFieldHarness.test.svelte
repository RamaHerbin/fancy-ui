<!--
  Test-only rig proving Textarea consumes the shared field context rather
...
<script lang="ts">
	...
	// instant — see svelte.dev/e/state_referenced_locally. Tests here only
	// ever pass one fixed context per render, but the indirection is what
...
<Textarea id="own-id" invalid={false} required={false} disabled={false} />
