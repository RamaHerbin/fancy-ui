...
describe('SparklesText', () => {
	...
	it('renders sparkle SVG elements', () => {
		const { container } = render(SparklesText, { props: { text: 'Test', sparklesCount: 5 } });
	...
	it('sparkle SVGs have animation styles', () => {
		const { container } = render(SparklesText, { props: { text: 'X', sparklesCount: 1 } });
	...
	it('uses custom colors on sparkle paths', () => {
		const { container } = render(SparklesText, {
			props: { text: 'X', sparklesCount: 20, colors: { first: '#FF0000', second: '#00FF00' } }
		});
