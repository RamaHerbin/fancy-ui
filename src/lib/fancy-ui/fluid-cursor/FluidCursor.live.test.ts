 ...
 *
 * FluidCursor never declared an `$effect`, but in runes mode a destructured
 * prop is a getter: the closures that outlived mount re-read the current value
 * on every call. `generateColor()` (every frame, every splat) read `fluidColor`
 * and `fluidColors`; the rect cache, the pointer mapping, the splat-radius
 * correction and the teardown read `contained`; the teardown read
 * `interactive`; the singleton registration read `allowMultiple`. Moving the
 * engine into a core turned those into snapshots unless the wrapper keeps
...
const FluidCursor = (await import("./FluidCursor.svelte")).default;
...
describe("FluidCursor live props", () => {
	...
	it("pushes nothing on mount: onMount owns the initial values", () => {
	...
	it("never recreates the engine, and never pushes a stale live value", async () => {
		// A mount-only prop change re-runs the effect (it reads `$$props`), but
		// the push it makes carries the unchanged live set, so it is a no-op —
		// and the simulation is never torn down and rebuilt.
