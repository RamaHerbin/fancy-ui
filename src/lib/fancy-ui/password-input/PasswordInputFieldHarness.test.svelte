<!--
  Test-only rig proving PasswordInput consumes the shared field context rather
  ...
  FieldContext surface, not against each other, so a fake provider here is the
  one way to test the consumer side in isolation. Not exported from index.ts,
...
<script lang="ts">
	...
	interface Props {
		...
		/**
		 * PasswordInput's own props, deliberately configurable rather than
...
<PasswordInput {id} {invalid} {required} {disabled} />
