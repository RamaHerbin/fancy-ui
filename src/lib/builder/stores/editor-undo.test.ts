...
describe("EditorState undo/redo integration", () => {
	...
	describe("prop updates", () => {
		it("edit prop, edit different prop, undo twice — both revert separately", () => {
			const editor = new EditorState(makePage({ body: [makeBlock("b1"), makeBlock("b2")] }));
	...
	describe("add block", () => {
		it("add block then undo removes it", () => {
			const editor = new EditorState(makePage({ body: [makeBlock("existing")] }));
	...
	describe("selection restoration", () => {
		it("restores selectedBlockId on undo", () => {
			const editor = new EditorState(makePage({ body: [makeBlock("a"), makeBlock("b")] }));
	...
	describe("arrow navigation", () => {
		...
		it("handles nested children in tree order", () => {
			...
			const editor = new EditorState(makePage({ body: [parentBlock, makeBlock("sibling")] }));
