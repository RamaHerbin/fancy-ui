...
<script lang="ts">
	...
	let badgeText = $derived(editor.dragSource?.type === "palette" ? "Add" : "Move");
</script>

{#if editor.isDragging}
	<div
		bind:this={overlayEl}
		class="bg-card/90 pointer-events-none fixed top-0 left-0 z-[9999] flex items-center gap-2 rounded-md border px-3 py-1.5 text-sm shadow-lg backdrop-blur-sm will-change-transform"
	>
		{#if dragIcon}
			{@const Icon = dragIcon}
		...
		<span class="bg-primary/10 text-primary rounded px-1.5 py-0.5 text-xs">{badgeText}</span>
	</div>
{/if}
