        ...
        children: [
          {
            id: "text-1",
            type: "_text",
            props: { content: "Hello World", tag: "p" },
          },
          {
            id: "text-2",
            type: "_text",
            props: { content: "Second paragraph", tag: "p" },
          },
        ],
      },
    ],
...
describe("block selection with nested blocks", () => {
  it("renders nested BlockWrappers with pointer-events-auto override", () => {
    const { container } = render(BlockSelectionTestHarness, {
      props: { page: makePage() },
    });
    ...
    expect(
      sectionPointerNone!.classList.contains(
        "[&_[data-drop-id]]:pointer-events-auto",
      ),
    ).toBe(true);
  });
