...
export class GitHubStorage implements PageStorage {
  ...
  constructor(config: GitHubStorageConfig) {
    ...
    this.contentPath = (config.contentPath ?? "content/pages").replace(
      /^\/+|\/+$/g,
      "",
    );
  }
    ...
    if (!res.ok) {
      ...
      if (res.status === 409) {
        throw new StorageError(
          "CONFLICT",
          "SHA conflict — file was modified concurrently",
        );
      }
    ...
    await Promise.all(
      fileEntries.map(async (entry) => {
        try {
          ...
          const doc: PageDocument = JSON.parse(
            this.decodeContent(file.content),
          );
          pages.push({
            slug: doc.meta.slug,
