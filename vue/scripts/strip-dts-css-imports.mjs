#!/usr/bin/env node
/**
 * Removes the side-effect CSS imports vue-tsc copies into the emitted declarations.
 *
 ...
 * `vue-tsc -p tsconfig.build.json` has no such step: it copies the import
 * verbatim into `Button.d.ts`, which then names a file dist does not contain.
 * A consumer whose tsconfig leaves `skipLibCheck` at the compiler default
 * (false) gets one TS2307 per component the moment they import anything from
 * the package, and cannot fix it from their side. Templates that set
 * `skipLibCheck: true` — Vite and Nuxt both do — never see it, which is why
 * the build has been shipping it.
 *
 ...
 * the same nothing.
 *
 * Runs between `vue-tsc` and the checks in the `build` script;
