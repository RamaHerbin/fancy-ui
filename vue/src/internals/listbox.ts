...
export function createListbox(options: ListboxOptions): ListboxState {
	...
	function move(delta: number): void {
		...
		// itself.
		let from = activeIndex.value === -1 ? (direction === 1 ? -1 : count) : activeIndex.value;
	...
	function typeahead(char: string, labelAt: (index: number) => string): void {
		...
		if (isRepeatChar) {
			...
			for (let i = 1; i <= count; i++) {
				const candidate = (activeIndex.value + i + count) % count;
...
/**
 * One listbox store per component, torn down with the scope that created it.
 *
 * The state IS the handle: its identity never changes, `activeIndex` is a live
 * reactive read a template can bind straight to, and `destroy()` — which only
 * clears the typeahead timer — runs on scope dispose rather than being left to
 * the caller to remember.
 */
export function useListbox(options: ListboxOptions): ListboxState {
	const listbox = createListbox(options);
	onScopeDispose(() => listbox.destroy());
	return listbox;
}
