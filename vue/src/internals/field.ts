...
/** Context key `FormField` publishes under. Consumers read it only via `useField()`. */
 ...
 *
 * The explicit `undefined` default is what makes a control mounted outside a
 * `FormField` degrade quietly: without it, a missing provider is reported as a
 * development warning instead of simply reading back `undefined`.
 */
export function useField(): FieldContext | undefined {
	return inject<FieldContext | undefined>(FIELD_KEY, undefined);
}
...
/**
 * Builds the object `FormField` publishes via `provide(FIELD_KEY, ...)`.
 *
 ...
 * registration written into reactive state would add a step between "the
 * paragraph is in the DOM" and "the id is in the list" — and that step runs
 * inside a mount effect, which never fires during SSR, so the very first paint
