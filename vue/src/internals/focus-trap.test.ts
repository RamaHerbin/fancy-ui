...
describe("focusTrap — re-arming after a reopen mid-exit", () => {
	...
	it("hands over a re-arm function alongside the return handle, at the same moment", () => {
		...
		// `!` only: this package compiles under `noUncheckedIndexedAccess`,
		// where `args[0]` is `unknown[] | undefined`. The preceding assertion
		// is the guard; everything else in this file is the Svelte bytes.
