// Parser half of the markdown security merge gate: adversarial parse cost,
// `sanitizeHref` bypass attempts and recursion bounds, all framework-free.
// The rendered-DOM half — the proof that the token tree is turned into
// elements and never into an HTML string — needs the components mounted and
// lives in `Markdown.security.test.ts`. The two halves gate together; neither
...
describe("DoS: adversarial parse cost stays bounded", () => {
	...
	it(
		"streaming re-parse of a bracket run (what a chat surface actually does)",
		STREAM_MEASURE_TIMEOUT,
		() => {
