...
import { defineComponent, effectScope, h, nextTick, useTemplateRef } from "vue";
import { createMenuFocus, useMenuFocus, useMenuItem } from "./menu.js";
...
describe("useMenuFocus", () => {
	...
	it("drives a menu exactly like the factory it wraps", () => {
		const { items } = mountItems(["Rename", "Duplicate"]);
		const scope = effectScope();
		const menu = scope.run(() => useMenuFocus())!;
		registerAll(menu, items);
	...
	it("destroys the store when its scope disposes, clearing a pending buffer", () => {
		// The Svelte consumer has to call `destroy()` itself and the dropdown
		// surface never does; here the scope owns it, so a buffer left open by
		// the last keystroke cannot outlive the menu.
		const { items } = mountItems(["Save", "Hide"]);
		const scope = effectScope();
		const menu = scope.run(() => useMenuFocus())!;
		registerAll(menu, items);
...
describe("useMenuItem", () => {
	function itemComponent(menu: ReturnType<typeof createMenuFocus>) {
		return defineComponent({
			props: { label: { type: String, required: true } },
			setup(props) {
				const el = useTemplateRef<HTMLElement>("item");
				useMenuItem(menu, el);
				return () => h("div", { ref: "item", role: "menuitem", tabindex: -1 }, props.label);
			},
		});
	}

	it("registers the item element once it exists in the DOM", async () => {
		const menu = createMenuFocus();
		const Item = itemComponent(menu);
		const Menu = defineComponent({
			setup() {
				return () =>
					h("div", { role: "menu" }, [
						h(Item, { label: "Rename" }),
						h(Item, { label: "Duplicate" }),
					]);
			},
		});

		const { container } = render(Menu);
		// A string template ref is published in a post-render job, so the
		// registering watcher runs on the tick after the mount flush — before
		// any keystroke can reach the surface, and the reason the dropdown's
		// open handshake keeps its `nextTick()`.
		await nextTick();
		const rendered = Array.from(container.querySelectorAll<HTMLElement>('[role="menuitem"]'));
	...
	it("unregisters when the item's scope disposes", async () => {
		const menu = createMenuFocus();
		const Item = itemComponent(menu);
		const Menu = defineComponent({
			setup() {
				return () => h("div", { role: "menu" }, [h(Item, { label: "Rename" })]);
			},
		});
		...
		unmount();
		expect(menu.focusedIndex).toBe(-1);
		// And the store is empty rather than holding a detached element.
		menu.move(1);
		expect(menu.focusedIndex).toBe(-1);
	});
});
