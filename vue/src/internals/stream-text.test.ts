...
import { createTextStream, type StreamSegment } from "./stream-text.js";
...
describe("createTextStream", () => {
	...
	// Not on the Svelte factory's surface — `internals-api.md` §12 requires
	// `flush()` of `useTextStream`, so it lives here with `list` and the timers.
	describe("flush", () => {
		it("settles every fresh segment at once, cancelling their timers", () => {
			vi.useFakeTimers();
			const stream = createTextStream("Hello", { settleMs: 100 });
		...
		it("publishes fresh objects so a keyed list sees the change", () => {
			vi.useFakeTimers();
			const stream = createTextStream("Hello", { settleMs: 100 });
			...
			expect(stream.segments).not.toBe(before);
		});
		...
		it("is a no-op with nothing fresh and nothing pending", () => {
			vi.useFakeTimers();
		...
		it("leaves the stream usable: a later push still animates", async () => {
			vi.useFakeTimers();
			const stream = createTextStream("Hello", { settleMs: 100 });
