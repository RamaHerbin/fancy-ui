...
describe("Markdown", () => {
	...
	/*
	 * A blockquote renders this component inside itself. The Svelte source proves
	 * the recursive instance keeps the parent's style scope, because its rules are
	 * compiler-scoped. Here the block styles ship in a plain stylesheet anchored on
	 * `.ft-md`, so the equivalent guarantee is that the nested instance carries the
	 * same anchor class and its blocks carry the same hooks — if that stops
	 * holding, quoted content silently loses every default.
	 */
	it("gives a nested blockquote the same style anchor as the root", () => {
		...
		expect(nested.querySelector(".ft-md-p")).toBeInTheDocument();
	});
});
...
describe("Markdown trailingCursor", () => {
	...
	const cursor = { trailingCursor: '<span class="test-cursor"></span>' };

	// `render` hands back an `Element`, not an `HTMLElement`; `querySelector` is
	// all this needs, so the parameter is typed to what the caller actually has.
	const cursorParent = (container: Element) =>
