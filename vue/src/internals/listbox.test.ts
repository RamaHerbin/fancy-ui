...
import { createListbox, useListbox } from "./listbox.js";
...
describe("useListbox", () => {
	...
	it("returns the store itself, activeIndex included", () => {
		const items: Item[] = [{ label: "A" }, { label: "B" }];
		const scope = effectScope();
		const lb = scope.run(() => useListbox({ count: () => items.length }))!;
	...
	it("destroys the store when its scope disposes, clearing a pending buffer", () => {
		const items: Item[] = [{ label: "Save" }, { label: "Hide" }];
		const scope = effectScope();
		const lb = scope.run(() => useListbox({ count: () => items.length }))!;

		lb.typeahead("s", labelAt(items));
		scope.stop();
		lb.typeahead("h", labelAt(items));
		expect(lb.activeIndex).toBe(1);
	});
});
