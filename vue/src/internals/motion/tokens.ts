 ...
 * still has to type the identical literal into its own colocated `.css` as
 * that CSS custom property's fallback (this package ships no shared runtime
 * stylesheet for motion — see `tailwind.css`, which carries only the semantic
 * colour vocabulary). Nothing enforces that a component's CSS fallback
 * matches the value here; this file is the source of truth a component author
 * copies FROM, not a value CSS reads at runtime. Keep the two in sync by hand.
 *
 ...
 * have exact JS equivalents in `./easing.js` (`expoOut`/`expoIn`, see
 * `JS_EASINGS` below) so a component that needs the SAME curve as both a CSS
 * string (`transition-timing-function`) and a JS easing function (a
 * transition spec's `easing` field) never has two numbers to keep in sync.
 *
...
import { expoIn, expoOut } from "./easing.js";
...
/** CSS `cubic-bezier()` strings — for a component's own `.css` file,
 * never injected by this file. */
export const EASINGS = {
	out: "cubic-bezier(0.16, 1, 0.3, 1)", // arrivals   ↔ expoOut
 ...
 * `transitions.ts`'s `preset()` — a `TransitionSpec.easing` field wants a
 * `(t: number) => number`, not a CSS string. Only `out`/`in` exist here:
 * `inout` never drives a JS-timed transition (it is CSS-only, for
