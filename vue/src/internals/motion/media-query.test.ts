...
import { createMediaQuery, createReducedMotion, REDUCED_MOTION_QUERY } from "./media-query.js";
 ...
 * like the Svelte suite's override pattern. */
...
describe("createMediaQuery", () => {
	...
	it("start() returns its own stop function, usable directly as cleanup", () => {
		const stub = stubMatchMedia(false);
		const mq = createMediaQuery("(min-width: 1px)");
		const stop = mq.start();
		stop();
