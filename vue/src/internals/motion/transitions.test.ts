...
// below runs without a component framework. The two harness-driven suites the
// Svelte file also carries — mounting a transition through WAAPI, and the
// reduced-motion `duration: 0` fast path — belong to the units that own
// `runTransition` (`animate.ts`) and `usePresence` (`presence.ts`), together
// with the `Element.prototype.animate` stub those need in `test-setup.ts`.
