 ...
 *
 * This is the counterpart to `listbox.svelte.ts`, and the difference between
 * the two is the whole reason both exist. A listbox keeps focus on its input
 * or trigger and points at the active row with `aria-activedescendant`; a menu
 ...
 *   document order diverge routinely — a conditional block, a keyed each that
 ...
 *   by the order `register` happened to be called in.
...
export interface MenuFocusState {
	 ...
	 *
	 * Reactive to the focused *element* changing, not to the item list changing
	 * shape: the registration list is a plain array, so inserting or removing a
	 * sibling while focus stays put does not invalidate a `computed` that read
	 * this. The value is correct on a fresh read and `document.activeElement` is
	 * never wrong — but do not drive rendering off this number reactively.
	 * Use `:focus` or `onFocusChange` for that.
	 */
...
// Matches the window used by `listbox.svelte.ts`, and by native menus on most
...
export function createMenuFocus(options: MenuFocusOptions = {}): MenuFocusState {
	...
	// A shallow ref, never a deep one: the value is a DOM element and must not
	// be handed to a reactive proxy — a proxied element is a different object
	// from the one `document.activeElement` reports, and every identity check
	// in this file would start lying.
	...
	function indexOfFocused(items: HTMLElement[]): number {
		return focusedElement.value ? items.indexOf(focusedElement.value) : -1;
	}
	...
	function register(element: HTMLElement): () => void {
		...
		return () => {
			...
			if (focusedElement.value === element) focusedElement.value = null;
		};
	}
...
/**
 * One menu-focus store per component, torn down with the scope that created it.
 *
 * The identity of the returned object never changes, so it can go straight into
 * a context value without a wrapper: every method closes over the same state
 * for the whole life of the surface. `destroy()` is called on scope dispose
 * rather than left to the caller — the typeahead timer is the only thing it
 * clears, and forgetting it is exactly the leak the Svelte consumers have to
 * remember to avoid by hand.
 */
export function useMenuFocus(options: MenuFocusOptions = {}): MenuFocusState {
	const menu = createMenuFocus(options);
	onScopeDispose(() => menu.destroy());
	return menu;
}

/**
 * Registers an item element with its menu for as long as it is in the document.
 *
 * The element is read from a watcher rather than in `setup`: a template ref is
 * null until the node is patched in, and an item rendered inside a conditional
 * branch is created later still. Registration order does not matter — the
 * store sorts by document position at navigation time — so a post-flush
 * watcher is early enough for the first arrow press and late enough for the
 * node to exist.
 *
 * Timing worth knowing: a string template ref is published from a post-render
 * job, so for a node present at mount this watcher runs on the tick AFTER the
 * mount flush. That is why the surface's open handshake awaits a tick before
 * it moves focus to an edge — it cannot assume the items registered in the
 * same flush that created them.
 */
export function useMenuItem(menu: MenuFocusState, el: WatchSource<HTMLElement | null>): void {
	...
	watch(
		...
		(element) => {
			release();
			if (element) unregister = menu.register(element);
		},
		{ flush: "post" }
	);
