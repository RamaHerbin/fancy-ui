...
describe("formatRelativeTime", () => {
	...
	/*
	 * Added here, with no counterpart in the reference suite: this module carries
	 * a guard the in-branch reference implementation does not have yet, and new
	 * behaviour with no test is how it gets dropped on the next copy down. A
	 * clock that has to read the same on the server and through hydration has no
	 * reading to give until it starts, and reports a non-finite value.
	 */
	it("returns an empty string when the clock has no reading yet", () => {
		expect(formatRelativeTime(NOW - 3 * HOUR, { locale: "en", now: Number.NaN })).toBe("");
		expect(formatRelativeTime(NOW - 3 * HOUR, { locale: "en", now: Infinity })).toBe("");
	});
});
