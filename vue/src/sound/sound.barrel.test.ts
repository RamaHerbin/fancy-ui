 ...
 * page, by a consumer's root layout — without constructing an AudioContext,
 * without reading or writing `localStorage`, and without attaching a single
 * window listener. Preferences hydrate on the first controller METHOD or on
 * `hydrateSound()`, and exactly once. A getter — the render path — never
 * hydrates here (D-V15), which is the one transposition in this file.
 *
...
describe("sound barrel — lazy hydration", () => {
	it("hydrateSound() performs exactly one getItem and no construction", async () => {
		const { sound } = await import("./index.js");
		const { hydrateSound } = await import("./sound.js");
		expect(storageCalls()).toBe(0);

		// A preference READ is a render path and must stay silent.
