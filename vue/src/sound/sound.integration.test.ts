 ...
 * - `hydrateSound()` — the explicit hydration entry point. The Svelte source
 *   hydrates from its getters, so its tests read `sound.enabled` to force the
 *   storage read; here a getter never writes, so the same intent is spelled
 *   out with the call (D-V15). Every assertion is otherwise unchanged.
...
import { getSoundStatus, hydrateSound, resetSoundForTests, sound } from "./sound.js";
