 ...
 * behaviour is covered by `engine.test.ts` and the cross-module
 * `sound.integration.test.ts`. This file is about the controller's own
 * contract: hydration, persistence, preference mutation, subscribers
 * and cross-tab sync.
 *
 * Transposed from the Svelte source. One thing moved: its getters hydrate on
 * first access, so its tests read `sound.enabled` to force the storage read.
 * Here a getter never writes reactive state, so hydration is the explicit
 * `hydrateSound()` (D-V15) and those reads become calls. Every assertion is
 * otherwise unchanged.
 */
...
describe("parseStoredPreferences", () => {
	...
	it("clamps volume into [0, 1] and ignores a non-finite volume", () => {
		...
		expect(
			parseStoredPreferences(
				JSON.stringify({ v: 1, enabled: true, volume: "nope", theme: "fancy" })
			).volume
...
describe("sound controller", () => {
	...
	it("a storage event for the sound key applies the new preferences with no write-back", () => {
		hydrateSound(); // hydrate + attach the storage listener
	...
	// The Svelte source's "first access from inside a $derived must not throw"
	// has no counterpart: a getter no longer writes anything, so there is no
	// reaction to corrupt. What replaced it — a render never reads storage — is
	// pinned by `sound.ssr.test.ts` and by SoundToggle.test.ts.
	...
	it("keeps exactly one pending cue while the context is idle/suspended and replays it once unlocked", async () => {
		...
		engineSpy.unlock.mockImplementation(
			() => new Promise<boolean>((resolve) => (resolveUnlock = resolve))
		);
	...
	it("a storage event marks storage as ok for a tab that only ever received updates", () => {
		hydrateSound(); // the cross-tab listener is attached
		expect(sound.enabled).toBe(false);
		window.dispatchEvent(
			new StorageEvent("storage", {
				key: SOUND_STORAGE_KEY,
...
describe("sound controller — hydration at the module boundary", () => {
	afterEach(() => {
		vi.resetModules();
	});

	it("does not read localStorage on import, nor from any getter; hydrateSound() reads it exactly once", async () => {
		...
		// Every getter — the render path — stays silent.
		void mod.sound.enabled;
		void mod.sound.volume;
		void mod.sound.theme;
		void mod.sound.preferences;
		void mod.sound.status;
		expect(getItemSpy).not.toHaveBeenCalled();

		mod.hydrateSound();
		expect(getItemSpy).toHaveBeenCalledTimes(1);

		// Idempotent: a second mount, or a second control on the page, reads nothing.
		mod.hydrateSound();
		void mod.sound.enabled;
		expect(getItemSpy).toHaveBeenCalledTimes(1);
	...
	it("a controller METHOD still hydrates, so `sound.play()` from outside a component is never stale", async () => {
		const getItemSpy = vi.spyOn(Storage.prototype, "getItem");
		getItemSpy.mockClear();
