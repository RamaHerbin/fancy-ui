...
describe("LineReveal", () => {
	...
	// Not in the Svelte suite: it pins the one behavioural divergence of this
	// port. Svelte's `style:` directives only re-apply the declaration whose
	// value changed, so a font-only change (same px size, other weight/family)
	// re-applies the `font` shorthand — which resets `line-height` — without
	// re-applying the separate `line-height`, and the lines drift inside their
	// fixed-height masks. Vue re-applies every key of the style object on each
	// patch, in declaration order, so the line height survives.
	it("keeps the line height when only the font changes", async () => {
		const { container, rerender } = render(LineReveal, {
			props: { text: "Steady", font: '600 32px "Helvetica Neue", Helvetica, Arial, sans-serif' },
		});
		const root = container.firstElementChild as HTMLElement;
		expect(root.style.lineHeight).toBe("38px");

		await rerender({ text: "Steady", font: "400 32px Georgia, serif" });
		expect(root.style.lineHeight).toBe("38px");
	});
});
