...
describe("ToolCall", () => {
	...
	// Vue-only regression: the source's mount-time effect fires for a call that
	// already failed even while `open` is driven from outside, which the seeded
	// `autoOpen` alone cannot express — see the `onMounted` in the component.
	it("opens a failed call that mounts with v-model:open bound to false", async () => {
		const onToggle = vi.fn();
		const expanded = ref(false);
		const Parent = defineComponent({
			setup: () => () =>
				h(ToolCall, {
					call: call({ status: "error", error: "boom" }),
					open: expanded.value,
					"onUpdate:open": (next: boolean | undefined) => {
						expanded.value = next === true;
					},
					onToggle,
				}),
		});
	...
	it("lets the input and output slots replace the default rendering", () => {
		const { container } = render(ToolCall, {
			props: {
			...
			slots: {
				input: `<template #input="{ value }"><p class="custom-in">in: {{ value.query }}</p></template>`,
				output: `<template #output="{ value }"><p class="custom-out">out: {{ value.hits }}</p></template>`,
			},
		});
	...
	it("keeps the error line above an output slot", () => {
		const { container } = render(ToolCall, {
			...
			slots: {
				output: `<template #output><p class="custom-out">partial</p></template>`,
			},
		});
