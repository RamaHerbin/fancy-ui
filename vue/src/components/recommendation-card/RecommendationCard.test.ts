...
import type { RecommendationState } from "./RecommendationCard.vue";
...
function accept(container: Element): HTMLButtonElement {
	return container.querySelector(".ft-rec-accept") as HTMLButtonElement;
}

function dismiss(container: Element): HTMLButtonElement {
	return container.querySelector(".ft-rec-dismiss") as HTMLButtonElement;
}

function resolved(container: Element): HTMLElement | null {
	return container.querySelector(".ft-rec-resolved");
}

function confidence(container: Element): HTMLElement | null {
	return container.querySelector(".ft-rec-confidence");
}

function ring(container: Element): SVGCircleElement | null {
	return container.querySelector(".ft-rec-ring-value");
}

function offset(container: Element): number {
...
/**
 * Two real frames, then a Vue flush: what the ring waits for before it writes
...
const CLAMPED: Array<[number, number, string]> = [
...
describe("RecommendationCard", () => {
	...
	it.each(BANDS)("reports %s as a percentage in the %s band", (value, percent, className) => {
		const { container } = render(RecommendationCard, {
	...
	it.each(CLAMPED)("clamps a confidence of %s into range", (value, percent, className) => {
		const { container } = render(RecommendationCard, {
			props: { title: TITLE, confidence: value },
		});
	...
	it("writes an acceptance back out through the binding", async () => {
		let bound: RecommendationState = "open";
		const { container, rerender } = render(RecommendationCard, {
			props: {
				title: TITLE,
				state: bound,
				"onUpdate:state": (v: RecommendationState) => {
					bound = v;
				},
			},
		});
		expect(bound).toBe("open");

		await fireEvent.click(accept(container));
		await rerender({ title: TITLE, state: bound });
		expect(bound).toBe("accepted");
	});
	...
	it("renders the children slot as the detail region", () => {
		const { container } = render(RecommendationCard, {
			props: { title: TITLE },
			slots: {
				default: '<code class="detail">CREATE INDEX ON orders (customer_id)</code>',
			},
		});
	...
	describe("sound", () => {
		...
		it("plays close — the deliberate asymmetry with the approval gate — when dismissed", async () => {
			const { container } = render(RecommendationCard, { props: { title: TITLE, sound: true } });
