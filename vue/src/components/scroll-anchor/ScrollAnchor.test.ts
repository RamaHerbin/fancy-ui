...
/**
 * `await nextTick()` after every render that measures: Svelte runs an action and
 * its effects synchronously at mount, while the post-flush watchers that attach
 * the core and take the opening geometry reading are scheduled from the ref
 * assignment and land a microtask later. Waiting for them puts the region in the
 * same state the Svelte test starts from — attached, and pinned by the
 * all-zeroes geometry jsdom reports before `measure()`.
 */
...
describe("ScrollAnchor", () => {
	...
	it("renders its children inside the scrolling region", () => {
		const { container } = render(ScrollAnchor, { slots: children });
	...
	it("forwards every pinned-state flip through onStickChange", async () => {
		...
		const { container } = render(ScrollAnchor, { props: { onStickChange }, slots: children });
		await nextTick();
		measure(region(container));
	...
	it("honours a custom bottomThreshold", async () => {
		...
		const { container } = render(ScrollAnchor, {
			props: { bottomThreshold: 5, onStickChange },
			slots: children,
		});
		await nextTick();
		measure(region(container));
	...
	it("labels the button however it is told to", async () => {
		const { container } = render(ScrollAnchor, {
			props: { returnLabel: "Back to the answer" },
			slots: children,
		});
		await nextTick();
		measure(region(container));
	...
	it("never offers the button when showReturn is off, however far up the reader goes", async () => {
		const onStickChange = vi.fn();
		const { container } = render(ScrollAnchor, {
			props: { showReturn: false, onStickChange },
			slots: children,
		});
		await nextTick();
		measure(region(container));
	...
	it("recomputes the pin state when bottomThreshold changes, with nothing scrolled", async () => {
		...
		await rerender({ bottomThreshold: 100 });
		expect(pill(container)).toBeNull();
	});
	...
	describe("sound", () => {
		...
		it("plays the press cue exactly once when the return pill is activated, with sound enabled", async () => {
			...
			const { container } = render(ScrollAnchor, { props: { sound: true }, slots: children });
			await nextTick();
			measure(region(container));
		...
		it("plays nothing from handleStick or the geometry watcher — only the pill's own click plays", async () => {
			const play = vi.spyOn(sound, "play").mockImplementation(() => {});
			const { container } = render(ScrollAnchor, { props: { sound: true }, slots: children });
			await nextTick();
			measure(region(container));
