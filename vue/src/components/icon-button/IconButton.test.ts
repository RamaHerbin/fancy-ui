...
describe("IconButton", () => {
	...
	it("renders the icon centred, with no visible text", () => {
		const { container } = render(IconButton, { props: { label: "Edit" }, slots: icon() });
	...
	it("exposes the required label as the accessible name", () => {
		const { getByRole } = render(IconButton, { props: { label: "Edit" }, slots: icon() });
		expect(getByRole("button", { name: "Edit" })).toBeTruthy();
	});

	it("carries the label through as aria-label", () => {
		const { container } = render(IconButton, { props: { label: "Settings" }, slots: icon() });
	...
	it("replaces the radius with a full circle when shape is circle", () => {
		const { container } = render(IconButton, {
			props: { shape: "circle", size: "md", label: "Action" },
			slots: icon(),
		});
	...
	it("blocks the click callback while loading", () => {
		const onclick = vi.fn();
		const { container } = render(IconButton, {
			props: { loading: true, onclick, label: "Save" },
			slots: icon(),
		});
	...
	it("disables the control and blocks the click callback", () => {
		const onclick = vi.fn();
		const { container } = render(IconButton, {
			props: { disabled: true, onclick, label: "Delete" },
			slots: icon(),
		});
	...
	it("merges a caller class alongside the geometry classes", () => {
		const { container } = render(IconButton, {
			props: { label: "Action", class: "my-icon-button" },
			slots: icon(),
		});
	...
	describe("sound", () => {
		...
		it("plays nothing while disabled, even with sound enabled", () => {
			const { container } = render(IconButton, {
				props: { sound: true, disabled: true, label: "Delete" },
				slots: icon(),
			});
		...
		it("plays nothing while loading, even with sound enabled", () => {
			const { container } = render(IconButton, {
				props: { sound: true, loading: true, label: "Save" },
				slots: icon(),
			});
