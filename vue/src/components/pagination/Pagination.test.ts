 ...
 * Three shapes changed and nothing else did:
 *
 * - The bindable `page` is `v-model:page`, so the round-trip case passes an
 *   `onUpdate:page` listener instead of binding a local (the `.test.svelte`
 *   harness collapses — the same shape Select.test.ts uses for its own
 *   bindable value).
 * - The bindable `ref` is exposed on the instance, so the ref case mounts
 *   with `@vue/test-utils` and reads `wrapper.vm.ref` rather than binding a
 *   local.
 * - `previousLabel`/`nextLabel` are snippet props in the source; a snippet
 *   passed as a prop value becomes slot content here, so the override case
 *   passes `slots` instead of `props`.
 *
 * Two type-only adjustments the port rules allow: the DOM helpers take
 * `Element` (what `@testing-library/vue` types `container` as, where the
 * source's testing library types it `HTMLElement`), and the single indexed
 * read is asserted non-null for `noUncheckedIndexedAccess`, which this
 * package's tsconfig enables and the source's does not.
 */
...
function pageButton(container: Element, page: number): HTMLButtonElement {
...
function ellipses(container: Element): HTMLElement[] {
...
describe("Pagination", () => {
	...
	it("renders the ellipsis as a non-focusable, aria-hidden span rather than a button", () => {
		const { container } = render(Pagination, { props: { count: 12, page: 1 } });
		const ellipsis = ellipses(container)[0]!;
	...
	it("renders two distinct, non-colliding ellipses when the current page sits in the middle", () => {
		...
		// Vue warns (and mis-reconciles) on a duplicate `v-for` key at this
		// point if the two ellipsis entries aren't keyed apart — reaching this
		// assertion at all is part of what the test is proving, not just the
		// count.
	...
	it("renders custom previousLabel/nextLabel slot content in place of the defaults", () => {
		const { container } = render(Pagination, {
			props: { count: 12, page: 5 },
			slots: {
	...
	it("round-trips page through v-model:page", async () => {
		let page = 1;
		const { container } = render(Pagination, {
			props: { count: 12, page, "onUpdate:page": (v: number) => (page = v) },
		});
	...
	it("exposes the nav element as ref", () => {
		const wrapper = mount(Pagination, { props: { count: 12, page: 1 }, attachTo: document.body });

		expect(wrapper.vm.ref).toBe(document.querySelector("nav"));
		wrapper.unmount();
	});
