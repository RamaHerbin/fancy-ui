...
import type { ApprovalState } from "./ApprovalCard.vue";
...
function approve(container: Element): HTMLButtonElement {
	return container.querySelector(".ft-approval-approve") as HTMLButtonElement;
}

function deny(container: Element): HTMLButtonElement {
...
describe("ApprovalCard", () => {
	...
	it("fires onApprove and resolves the card", async () => {
		...
		const onDeny = vi.fn();
		const { container } = render(ApprovalCard, {
			props: { title: TITLE, onApprove, onDeny },
		});
	...
	it("writes the decision back out through the binding", async () => {
		let bound: ApprovalState = "pending";
		const { container, rerender } = render(ApprovalCard, {
			props: {
				title: TITLE,
				state: bound,
				"onUpdate:state": (v: ApprovalState) => {
					bound = v;
				},
			},
		});
	...
	it("renders the detail region it is handed", () => {
		const { container } = render(ApprovalCard, {
			props: { title: TITLE },
			slots: { default: '<pre class="preview">pnpm db:migrate --prod</pre>' },
		});
	...
	describe("sound", () => {
		...
		it("resolves once under a fully-controlled binding too", () => {
			// `defineModel` only emits — it does not write its own copy — once the
			// consumer supplies both `state` and `@update:state`, so the guard's
			// read-back is still "pending" for the rest of the tick. Two dispatches
			// with no flush between them must still resolve the gate exactly once.
			const onApprove = vi.fn();
			let bound: ApprovalState = "pending";
			const { container } = render(ApprovalCard, {
				props: {
		...
		it("stays live when a controlled consumer declines the decision", async () => {
			// The card is still on screen and still pending because the value never
			// came back, so once that tick has passed the gate takes a fresh
			// decision rather than sitting there inert under live buttons.
			const onApprove = vi.fn();
			const { container } = render(ApprovalCard, {
				props: {
					title: TITLE,
