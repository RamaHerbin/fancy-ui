 ...
 * - Both `*.test.svelte` harnesses become Vue: the FormField case publishes the
 *   context through `global.provide`, and the bindable-`value` case mounts an
 *   inline `defineComponent` + `h()` harness that owns the state, feeds it back
 *   as `value` and echoes it into the DOM — the round trip `bind:value` proves.
 * - The bindable `ref` is exposed on the instance, so the ref case mounts and
 *   reads `wrapper.vm.ref` rather than binding a local.
 * - This package's jsdom has no `PointerEvent`; the outside-click cases build a
 *   `MouseEvent` instead, which carries every field the dismiss layer reads.
 *
...
describe("DatePicker", () => {
	...
	/**
	 * Stands in for the source's `.test.svelte` rig. A listener written into the
	 * props object of `render` would prove only that the component EMITS; the
	 * source harness proves the new value travels out to the consumer and comes
	 * back in. So the parent owns the state, feeds it back as the `value` prop
	 * and echoes it into the DOM, which is `bind:value` in Vue's spelling.
	 */
		...
		setup() {
			...
			return () => [
				...
				h(
					...
					value.value ? formatISODate(value.value) : "none"
				),
			];
		},
	});
	...
	it("round-trips value through v-model:value", async () => {
		...
		// case) and commit it.
		await fireEvent.click(focusedCell()!);
	...
	it("exposes the trigger element as ref", () => {
		...
		expect(wrapper.vm.ref).toBe(document.querySelector(".ft-date-picker-trigger"));
		wrapper.unmount();
	});
	...
	it("inside a FormField, the context wins for controlId, aria-describedby, aria-invalid, required and disabled", async () => {
		...
		// Deliberately passed own props that disagree with the context, so the
		// assertions prove the context wins rather than merely matching by
		// coincidence — the shape of the source's field harness.
		const { container } = render(DatePicker, {
			props: { id: "own-id", invalid: false, required: false, disabled: false, label: "Deadline" },
	...
	describe("exit", () => {
		it("keeps the panel mounted, inert and marked closing for the length of the exit", async () => {
			...
			// exit — which is what stops a day cell taking a click on its way
			// out.
		...
		// `closePanel()` is a plain function outside the mount gate, so it
		// returns focus in the same tick as the dismiss — this component needs
		// no focus-trap handle to satisfy the eager-return rule. The assertion
		// deliberately runs INSIDE the exit window, while the panel is still on
		// screen and already marked `inert`: that is exactly the moment a late
		// return would have stranded a keyboard user on `<body>`.
