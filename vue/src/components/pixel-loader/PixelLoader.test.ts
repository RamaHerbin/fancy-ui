...
function pixels(container: Element): HTMLElement[] {
	return Array.from(container.querySelectorAll<HTMLElement>(".ft-pixel"));
}

// Vue writes inline styles with a trailing semicolon; drop it so the delays
...
describe("PixelLoader", () => {
	...
	it("renders rows * cols pixels for a custom grid size", () => {
		const { container } = render(PixelLoader, { props: { cols: 8, rows: 3 } });
		expect(pixels(container)).toHaveLength(24);
	});

	it("exposes a live status region announcing the label", () => {
		render(PixelLoader, { props: { label: "Thinking" } });
	...
	it("merges custom class names onto the root", () => {
		const { container } = render(PixelLoader, { props: { class: "my-loader" } });
	...
	it("reflects cellSize and gap in the root inline style", () => {
		const { container } = render(PixelLoader, { props: { cols: 4, cellSize: 10, gap: 3 } });
	...
	it("derives a deterministic diagonal delay per pixel", () => {
		...
		const { container } = render(PixelLoader, { props: { cols: 2, rows: 2, speed: 4 } });
