 ...
 *   becomes a template string in `slots` — a scoped one for `icon`.
 * - `tick()` becomes `nextTick()`. Where the source read a transition's
 *   keyframes one tick after it started, the sampler here chains a
 *   zero-duration dummy into the real leg across a microtask, so those cases
 *   drain the chain with `settleLegs()` first.
 * - The bindable `open`/`query` are `v-model:open`/`v-model:query`, so the two
 *   "round-trips" cases pass an `onUpdate:*` listener instead of a
 *   getter/setter pair.
 * - The bindable `ref` is exposed on the instance, so that case mounts and
 *   reads `wrapper.vm.ref` rather than binding a local.
 * - This package's jsdom has no `PointerEvent`; the dismiss layer only reads
 *   `event.target`, so a same-typed `MouseEvent` stands in (marked at its call
 *   site, the same substitution the internals suite makes).
 *
 * No `inert` shim, deliberately (the dialog suite makes the same call). jsdom
 * implements no `inert` IDL property, so a prototype getter/setter reflecting
 * the property to the attribute would mean the exit case passes against the
 * shim rather than against what the component writes. The presence clock writes
 * the ATTRIBUTE through `toggleAttribute`, so `hasAttribute` observes
 * production behaviour directly — which is why the source's `closing.inert`
 * assertion reads `hasAttribute("inert")` here.
 */
...
// The last REAL `element.animate` call made on a given node. The sampler issues
// two per leg: a zero-length dummy that exists only to defer the sampled
// keyframes past the DOM update, then the animation that actually runs. Both
// land on the same element, and the real one is always the later of the two, so
// walking back from the end and matching on the spy's recorded `this` is what
// separates the panel's animation from the scrim's.
...
describe("CommandMenu", () => {
	...
	it("locks the page scroll while open and releases it on close", async () => {
		...
		// The release is deliberately NOT synchronous any more: it is delayed by
		// the exit transition, which is what keeps the page locked until the
		// scrim has actually finished fading.
	...
	it("guards the scrollIntoView call — jsdom does not implement it, so arrowing through the list must not throw", async () => {
		...
		// jsdom (confirmed against this package's `test-setup.ts`, which mocks
	...
	it("highlights the matched range as a real <mark> element, never via v-html", async () => {
	...
	it("renders the empty slot instead of emptyMessage when given", async () => {
		render(CommandMenu, {
			props: {
				items: ITEMS,
				open: true,
	...
	it("renders the icon slot for each row, given that row's own item", async () => {
		render(CommandMenu, {
			props: { items: ITEMS, open: true },
			slots: {
				icon: '<template #icon="{ item }"><span :data-testid="`ico-${item.id}`"></span></template>',
			},
		});
		await nextTick();
	...
	it("round-trips through v-model:query", async () => {
		let query = "";
		render(CommandMenu, {
			props: {
				items: ITEMS,
	...
	it("works with onQueryChange alone, no v-model:query", async () => {
	...
	describe("modal-rung motion", () => {
		...
		it("arrives over 300 ms and leaves over 200 ms, scaling about its own centre", async () => {
			...
			// Opened from CLOSED, not mounted already open: the presence clock
			// does not play an entrance on the very first render of a surface
			// that mounts open, so a component rendered `open` has no entrance
			// to measure.
			...
			expect(inFrames.some((frame) => String(frame.transform).includes("translate"))).toBe(
				false
			);
			...
			expect(outFrames.some((frame) => String(frame.transform).includes("translate"))).toBe(
				false
			);
		});
		...
		it("keeps the panel and its scrim mounted, inert and marked closing for the length of the exit", async () => {
			...
			// values: the presence clock keeps the leaving subtree mounted and
			// reactive, so nothing has to write this attribute by hand.
			expect(closing!.getAttribute("data-state")).toBe("closing");
			// Written by the presence clock, as an attribute, for the whole
			// exit. A closing modal must never be interactive — and this is
			// also what takes the live region below out of the accessibility
			// tree on the way out.
