...
describe("Navbar", () => {
	...
	it("renders the brand, links and actions slots, in that DOM order", () => {
		const { container } = render(Navbar, {
			slots: {
				brand: '<span data-testid="brand">FancyUI</span>',
				default: '<span data-testid="links">Docs</span>',
	...
	it("renders no empty wrapper elements for slots that were not passed", () => {
		const { container } = render(Navbar, {
			slots: { default: '<span data-testid="links">Docs</span>' },
		});
...
describe("NavbarLink", () => {
	...
	function link(container: Element): HTMLAnchorElement {
	...
	// to, and unlayered scoped CSS was overwriting it, so until now the
	...
	it("binds the anchor element", () => {
		const wrapper = mount(NavbarLink, { props: { href: "/docs" } });
		expect(wrapper.vm.ref).toBe(wrapper.element);
	});
