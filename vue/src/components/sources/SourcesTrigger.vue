...
defineOptions({ name: "SourcesTrigger", inheritAttrs: false });

const { label, class: className } = defineProps<SourcesTriggerProps>();
...
const count = computed(() => sources?.count ?? 0);
const isOpen = computed(() => sources?.open.current ?? false);
const stack = computed(() => (sources?.sources ?? []).slice(0, STACK_LIMIT));
const text = computed(() => label ?? `${count.value} ${count.value === 1 ? "source" : "sources"}`);
</script>
...
<template>
	...
	>
		...
		<span
			v-if="stack.length > 0"
			class="ft-sources-stack flex flex-none items-center"
			aria-hidden="true"
		>
			<span v-for="source in stack" :key="source.id" class="ft-sources-chip">{{
