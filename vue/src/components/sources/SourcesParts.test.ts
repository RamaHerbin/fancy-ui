...
/** The same composition rig as the root suite: real parts inside a real root. */
...
function card(container: Element): HTMLElement {
...
describe("SourceCard", () => {
	...
	it("lets an icon slot replace the monogram", () => {
		const { container } = render(SourceCard, {
			props: { source: { id: "s1", title: "T", url: "https://docs.example.dev/a" } },
...
describe("SourcesList", () => {
	...
	it("hands each source and its index to an item slot instead of the card", () => {
		const { container } = render(Harness, {
...
describe("SourceCard — model-supplied urls", () => {
	...
	const cardFor = (url: string, props: Record<string, unknown> = {}) =>
		render(SourceCard, {
	...
	it("makes a bare host absolute, so it cannot resolve against this app's origin", () => {
		const { container } = cardFor("docs.example.dev/guide");
	...
	it("leaves a genuine relative path alone", () => {
		const { container } = cardFor("/local/guide");
	...
	it("renders the plain shape when it is told it cannot host a link", () => {
		const { container } = cardFor("https://docs.example.dev/guide", { interactive: false });
