...
describe("MosaicGlow", () => {
	it("renders host, canvas and children, merges classes and forwards attributes", () => {
		const { container } = render(MosaicGlow, {
			props: { class: "custom h-40" },
			attrs: { "data-testid": "mg" },
	...
	it("exposes the host as a div", async () => {
		const wrapper = mount(MosaicGlow);
		await nextTick();
		expect(wrapper.vm.ref).toBeInstanceOf(HTMLDivElement);
	...
	it("schedules a frame on pointermove over the host and over a child", async () => {
		withCanvas();
		const { container } = render(MosaicGlow, {
			props: { idle: "none", flicker: false },
			slots: { default: "<button data-testid='btn'>go</button>" },
		});
		frame(16);
		expect(rafCallbacks.length).toBe(0);
		const host = getHost(container);
	...
	it("attaches no pointer listeners when interactive is false", () => {
		withCanvas();
		// Mounted through a bare `createApp` rather than the testing-library
		// helper: that helper's wrapper registers EVERY native event type on the
		// root element to record emissions, which would drown the spy.
		const target = document.createElement("div");
		document.body.appendChild(target);
		const spy = vi.spyOn(HTMLElement.prototype, "addEventListener");
		const app = createApp(MosaicGlow, { interactive: false });
		app.mount(target);
