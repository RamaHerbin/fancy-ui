...
defineOptions({ name: "ScrollProgress", inheritAttrs: false });
...
const {
	...
	label,
	class: className,
} = defineProps<ScrollProgressProps>();

/**
 * The root element. Svelte publishes it through `ref = $bindable(null)`;
 * `ref` is a reserved vnode key in Vue, so it is exposed on the instance
 * instead and is absent from `ScrollProgressProps`.
 */
 ...
 * environment — probed exactly ONCE, inside `onMounted`, and never again.
 ...
 * every render before `onMounted` runs — `data-mode` is therefore absent
 ...
 */
const supportsScrollTimeline = ref<boolean | undefined>(undefined);

/**
 * `mode` itself is a `computed`, NOT latched at the same moment as the
 ...
 * because a `target` (or `label`) can arrive AFTER `onMounted` has already
 * run: `<ScrollProgress :target="box" /> …later… <div ref="box">` sees
 * `target === null` on the tick `onMounted` fires (a template ref on an
 * element written below this component resolves in a later flush), and so
 * does any `v-if`-gated scroller, async content, or route change that
 * hands this component a target after the fact. Deriving `mode` means a
 * bar that starts in CSS mode still flips to JS mode the instant such a
 * `target`/`label` shows up, instead of staying latched onto the
 * document's scroll forever. See the README's Implementation notes.
 */
const mode = computed<"css" | "js" | undefined>(() =>
	supportsScrollTimeline.value === undefined
		? undefined
		: !target && label === undefined && supportsScrollTimeline.value
			? "css"
			: "js"
);
 ...
 * state. Routed through a `ref` rather than written straight to the DOM:
 ...
 * here, itself only ever called from the JS-mode watcher below — which
 * never runs during SSR — so the global is safe to name even though this
 * file has no top-of-module `typeof window` guard.
 */
...
function update() {
	...
	// Bypasses Vue reactivity on purpose, per the family's rAF-loop
	// doctrine: this can run up to once per animation frame, and routing
	// it through a `ref` → template re-render would be the exact
	...
	if (label !== undefined) valueNow.value = Math.round(progress * 100);
}

// Only JS mode ever attaches anything. CSS mode's entire reason to exist
// is tracking scroll with ZERO JS — including before this watcher has had
...
watch(
	...
	({ mode: currentMode, node, target: currentTarget }, _prev, onCleanup) => {
		if (currentMode !== "js") return;
		if (!node) return;
		const scroller: Window | HTMLElement = currentTarget ?? window;
 ...
 * already painting correctly before `onMounted` runs at all. It does NOT
 ...
 * paint and `onMounted`, `data-mode` is absent, this rule matches, and the
