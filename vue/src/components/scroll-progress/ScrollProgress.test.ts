...
describe("ScrollProgress", () => {
	...
	it("mode is derived, not latched: a `target` handed to the component AFTER mount flips it from css to js and attaches a scroll listener", async () => {
		...
		// A target arrives afterward (e.g. a template-ref element resolved on a
		// later flush, or a `v-if`-gated scroller). If `mode` were latched at
		// `onMounted` this would never be reflected.
	...
	it("JS mode: a scroll event, throttled to a frame, writes --ft-scrollprogress-value and rounds aria-valuenow", async () => {
		...
		// visible immediately); `aria-valuenow` goes through a `ref` →
		// template, which patches the DOM on Vue's own scheduler flush — a real
		// `nextTick()` is required to observe it, not just the rAF advance.
		await nextTick();
