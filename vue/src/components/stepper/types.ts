 ...
 *
 * `Stepper` owns the active index; every `Step` reads it through the shared
 * context instead of a prop threaded down by hand, the same "root owns the
 * shared state, items read it" shape `ToggleGroup` / `ToggleGroupItem` use
 * for roving focus.
 *
 ...
 * steps are a static composition (typically one `v-for` over a fixed list)
 * that doesn't reorder after mount, so the order each `Step` registers in
 * from its own mount hook already *is* DOM order.
 *
 * That assumption has a consequence, not just a precondition: reordering
 * already-mounted `Step`s (a keyed `v-for` that moves an existing item
 * rather than adding or removing one) does not re-run that item's
 * registration hook, so `indexOf` keeps returning its *original* mount
...
export const STEPPER_KEY: InternalContext<StepperContext> =
	createInternalContext<StepperContext>("StepperContext");
