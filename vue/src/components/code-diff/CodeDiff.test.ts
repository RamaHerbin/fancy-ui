...
function headers(container: Element): HTMLButtonElement[] {
	return Array.from(container.querySelectorAll<HTMLButtonElement>("button[aria-expanded]"));
}

function body(container: Element, index = 0): HTMLElement {
	const id = headers(container)[index]!.getAttribute("aria-controls") as string;
	return container.querySelector<HTMLElement>(`#${id}`) as HTMLElement;
}

function rowsOfKind(container: Element, kind: string): HTMLElement[] {
	return Array.from(container.querySelectorAll<HTMLElement>(`[data-kind="${kind}"]`));
}
...
describe("CodeDiff", () => {
	...
	it("marks the gutters, glyphs and hunk headers as chrome so a copy is code alone", () => {
		...
		const source = readFileSync("src/components/code-diff/CodeDiff.vue", "utf8");
	...
	it("starts open, folds the body on click, and reports it through aria-expanded", async () => {
		...
		// jsdom does not implement the `inert` IDL property, so the attribute the
		// v-bind object writes is what there is to check (see the tool-call port).
	...
	describe("an unrelated patch that reuses a path", () => {
		...
		it("unfolds instead of inheriting the previous patch's fold state", async () => {
			...
			// The reset runs in the watcher that follows the prop update, so the
	...
	describe("sound", () => {
		...
		it("stays silent when the collapsed-prop watcher wipes the fold state, not just the click handler", async () => {
			const { rerender } = render(CodeDiff, { props: { diff: MULTI_HUNK, sound: true } });

			// Flips the master switch from outside — the watcher that resets
		...
		it("stays silent when a streaming patch re-runs the signature watcher", async () => {
			const { rerender } = render(CodeDiff, { props: { diff: BARE_HUNK, sound: true } });

			// Same file, more content: the signature watcher resets `folded` and
