...
describe("AgentPlan", () => {
	...
	// Scoped-style guard, with no counterpart in the source suite: every shape
	// inside the glyph must carry the SFC's scope attribute, or the
	// `.ft-agentplan-ring` / `-dot` / `-dash` / `-icon` rules in the scoped block
	// stop matching and the shapes render at zero size.
	it.each([
		["pending", ".ft-agentplan-ring"],
		["running", ".ft-agentplan-dot"],
		["cancelled", ".ft-agentplan-dash"],
		["done", ".ft-agentplan-icon"],
		["error", ".ft-agentplan-icon"],
	] as const)("scopes the %s shape to the component's styles", (status, selector) => {
		const { container } = render(AgentPlan, { props: { steps: [step({ status })] } });

		const mark = glyph(rows(container)[0]!);
		const scopeAttr = [...mark.attributes]
			.map((attr) => attr.name)
			.find((name) => name.startsWith("data-v-"));
		expect(scopeAttr).toBeDefined();

		const shape = mark.querySelector(selector);
		expect(shape).not.toBeNull();
		expect(shape!.hasAttribute(scopeAttr!)).toBe(true);
	});
	...
	it("moves aria-current onto the substep once the parent is done", () => {
		const steps = nested();
		steps[1]!.status = "done";
		const { container } = render(AgentPlan, { props: { steps } });
	...
	it("lets the item slot replace the row body, keeping the glyph", () => {
		const { container } = render(AgentPlan, {
			props: { steps: nested() },
			slots: {
				item: '<template #item="{ step, index }"><span class="custom-row">{{ index }}: {{ step.label }}</span></template>',
			},
		});
	...
	it("speaks the status of a custom row, since the sr-only label lives only in the default body", () => {
		const { container } = render(AgentPlan, {
			props: { steps: [step({ status: "error" })] },
			slots: {
				item: '<template #item><span class="custom-row">mine</span></template>',
			},
		});
	...
	it("renders a plan that repeats an id instead of crashing on it", () => {
		...
		// A key collision would throw before a single row reached the DOM, and
		// the ids here come from a model.
		const { container } = render(AgentPlan, { props: { steps } });
	...
	describe("sound", () => {
		...
		it("plays select exactly once when a row is activated, with sound enabled", async () => {
			const onSelect = vi.fn();
			const { container } = render(AgentPlan, {
				props: { steps: nested(), sound: true, onSelect },
			});
		...
		it("plays select for a substep row too, through the same shared handler", async () => {
			const onSelect = vi.fn();
			const { container } = render(AgentPlan, {
				props: { steps: nested(), sound: true, onSelect },
			});
			...
			await fireEvent.click(container.querySelectorAll("button")[2]!);
		...
		it("plays nothing when onSelect is absent — rows are plain divs, not buttons", () => {
			const { container } = render(AgentPlan, { props: { steps: nested(), sound: true } });
		...
		it("plays again on a repeat activation of the same row — there is no changed-only guard", async () => {
			const onSelect = vi.fn();
			const { container } = render(AgentPlan, {
				props: { steps: nested(), sound: true, onSelect },
			});
