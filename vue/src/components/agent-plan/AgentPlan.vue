...
defineOptions({ name: "AgentPlan", inheritAttrs: false });
...
interface PlanRow {
	step: PlanStepData;
	/**
	 * What the `v-for` is keyed by. The id leads, so a row keeps its DOM node
...
const doneCount = computed(() => rows.value.filter((row) => row.step.status === "done").length);
const percent = computed(() =>
	total.value === 0 ? 0 : Math.round((doneCount.value / total.value) * 1000) / 10
);
...
const currentIndex = computed(() => rows.value.findIndex((row) => row.step.status === "running"));
...
<template>
	<div ref="rootEl" :class="cn('ft-agentplan w-full text-sm', className)">
		<div class="ft-agentplan-header">
			...
			<div v-if="showProgress" class="ft-agentplan-track" aria-hidden="true">
		...
		<div class="ft-agentplan-list" role="list" :aria-labelledby="labelId">
			<div
				...
				:class="{
					'ft-agentplan-sub': row.depth === 1,
					'ft-agentplan-sub-last': row.lastSub,
				}"
				role="listitem"
			...
			>
				...
				>
					...
					<span
						...
						:class="{
							'ft-status-pending': row.step.status === 'pending',
							'ft-status-running': row.step.status === 'running',
							'ft-status-done': row.step.status === 'done',
							'ft-status-error': row.step.status === 'error',
							'ft-status-cancelled': row.step.status === 'cancelled',
						}"
						aria-hidden="true"
					>
						<svg
						...
						<span v-else-if="row.step.status === 'cancelled'" class="ft-agentplan-dash"></span>
						<span v-else-if="row.step.status === 'running'" class="ft-agentplan-dot"></span>
						<span v-else class="ft-agentplan-ring"></span>
					</span>
					<template v-if="$slots.item">
						<span class="sr-only">{{ STATUS_LABELS[row.step.status] }}</span>
						<slot name="item" :step="row.step" :index="row.index" />
					</template>
					<span v-else class="ft-agentplan-text min-w-0 flex-1">
						<!--
						...
						<span
							...
							:class="{
								...
								'font-medium': row.step.status === 'running',
							}"
							>{{ row.step.label }}</span
						>
						<span class="sr-only">{{ STATUS_LABELS[row.step.status] }}</span>
						<span
							...
							>{{ row.step.detail }}</span
						>
					</span>
				</button>
				<div v-else class="ft-agentplan-body flex items-start gap-2 rounded-md">
					<span
						class="ft-agentplan-glyph"
						:class="{
							'ft-status-pending': row.step.status === 'pending',
