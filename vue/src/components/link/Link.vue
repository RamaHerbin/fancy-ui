...
const el = useTemplateRef<HTMLAnchorElement>("el");
defineExpose({ ref: el });
...
const computedTarget = computed(() => (external ? (target ?? "_blank") : target));
...
const computedRel = computed(() => {
	const t = computedTarget.value;
	const opensNewContext = external || (!!t && !SAME_TAB_TARGETS.has(t.toLowerCase()));
