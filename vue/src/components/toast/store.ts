 ...
 *
 * This file owns the data and the timers. `Toast.vue` and `Toaster.vue`
...
// A SHALLOW ref, not a deep one: the stack is only ever REPLACED (every
// mutation below builds a new array), and an item is frozen the moment it is
// created — so deep reactivity would wrap each `ToastItem` in a proxy that
// nothing here or in either component ever needs. `<Toast>` receives the same
// object `toast()` built, which is what the source's own list does.
...
// juggling reactive state for it.
 ...
 * long-lived server would leak toasts between unrelated users — the same
 * trap `internals/id.ts`'s `uid()` guards against for the same reason.
 */
