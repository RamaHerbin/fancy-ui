...
describe("ReasoningPanel", () => {
	...
	it("opens on its own while streaming", async () => {
		vi.useFakeTimers();
		const { container } = render(ReasoningPanel, {
			props: { text: "trace", streaming: true },
		});

		// The source opens the panel from a mount-time `$effect`, which Svelte
		// flushes synchronously; here the same pass runs in `onMounted` and the
		// DOM catches up on the next tick.
		await nextTick();
		expect(header(container).getAttribute("aria-expanded")).toBe("true");
	});
	...
	it("holds the collapsed trace inert, and releases it when expanded", async () => {
		vi.useFakeTimers();
		const { container } = render(ReasoningPanel, { props: { text: "trace" } });
		// jsdom does not define an `inert` IDL property, so Vue's patcher (which
		// decides prop-vs-attribute by `key in el`) writes it as an attribute
		// rather than the DOM property Svelte's compiler assigns directly —
		// `hasAttribute` is what there is to check on this side.
	...
	it("announces onToggle when a consumer writes to the bound open prop directly", async () => {
		// A write to a value bound with `v-model:open` never reaches the internal
