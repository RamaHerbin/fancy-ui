...
export function parseRgb(input: string): Rgb | null {
	...
	if (hex) {
		// The one capturing group is mandatory in the pattern, so it is always
		// present once `exec` matches — `noUncheckedIndexedAccess` doesn't know
		// that, hence the assertions (vue-tsc addition, not in the Svelte source).
		const h = hex[1]!;
...
function animatedBlob(b: Blob, color: Rgb | string, scaled: boolean): string {
	// `b.slot` always indexes a real SLOTS entry — the assertion is only for
	// `noUncheckedIndexedAccess` (vue-tsc addition, not in the Svelte source).
...
export function buildLayerBackgrounds(o: BackgroundOptions): LayerBackgrounds {
	...
	// Every `b.slot` here indexes a real `colors` entry (nine palette slots,
	// same domain as SLOTS); the assertions are only for
	// `noUncheckedIndexedAccess` (vue-tsc addition, not in the Svelte source).
