 ...
 *
 * - The source's checked/indeterminate/ref harness is an inline
 *   `defineComponent` here: it drives `v-model:checked` and
 *   `v-model:indeterminate` through `onUpdate:*` listeners and reads the
 *   element back off the child's exposed `ref` instead of `bind:ref`.
...
function checkbox(container: Element): HTMLInputElement {
 ...
 * to `render` is not written back by the child, so proving `checked` and
 * `indeterminate` travel back out to the consumer — and that the exposed
 * `ref` points at the real element — needs a parent that owns all three.
 */
const ValueHarness = defineComponent({
	...
	setup(props) {
		...
		const child = useTemplateRef<InstanceType<typeof Checkbox>>("child");
	...
	template: `
		<Checkbox ref="child" v-model:checked="checked" v-model:indeterminate="indeterminate" label="Agree" />
...
describe("Checkbox", () => {
	...
	it("round-trips indeterminate through v-model:indeterminate, cleared by a click", async () => {
		const { container, getByTestId } = render(ValueHarness, {
	...
	it("falls through to the slot's own text as the accessible name when label is not given", () => {
		const { container } = render(Checkbox, {
			slots: { default: "<span>I agree</span>" },
		});
		const el = checkbox(container);
	...
	it("inside a FormField, the context wins for controlId, aria-describedby, aria-invalid, required and disabled", () => {
		...
		// The component is passed id="own-id" invalid={false} required={false}
		// disabled={false} — every one of those is overridden by the context.
		const { container } = render(Checkbox, {
			props: { id: "own-id", invalid: false, required: false, disabled: false, label: "Agree" },
