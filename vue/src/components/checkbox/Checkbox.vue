...
/**
 * Whether the box is checked. This is the real state underneath even while
 * `indeterminate` is true — a click always resolves to this value, never to
 * a third state.
 */
const checked = defineModel<boolean>("checked", { default: false });
/**
 * Mixed/dash visual state. A DOM property with no HTML attribute equivalent,
 * so it is assigned straight to the element and reapplied whenever this model
 * changes, not only on mount. Any interaction that changes `checked` clears it
 * back to `false`.
 */
...
// change (not just on mount) is what keeps a later model update in sync;
// keying on the element ref rather than caching the element once also covers
// the element being (re)bound. `onMounted` carries the very first assignment
// so the box is already mixed on the first paint; the watcher — post-flush,
// never immediate, so it never runs during a server render — carries every
// one after it.
function syncIndeterminate() {
	if (el.value) el.value.indeterminate = indeterminate.value;
}
onMounted(syncIndeterminate);
watch([el, indeterminate], syncIndeterminate, { flush: "post" });
...
// `indeterminate` is still cleared explicitly below so the model mirrors
...
function handleChange(event: Event) {
	...
	if (effectiveDisabled.value) {
		...
		// app's own model. A real browser already refuses to run the default
		// toggle action on a disabled checkbox, but a synthetic event
		// dispatched straight at the element — as a test does — can still
		// mutate the DOM property directly, so the handler puts it back
		// rather than trusting the guard above alone.
