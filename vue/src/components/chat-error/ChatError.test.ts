...
function retryButton(container: Element): HTMLButtonElement | null {
...
describe("ChatError", () => {
	...
	it("replaces the message and detail with the default slot", () => {
		const { container } = render(ChatError, {
			props: {
			...
			slots: {
				default: "<span>Rate limit reached, retrying in 30s</span>",
			},
		});
	...
	it("swaps the default triangle for the icon slot, still hidden from assistive tech", () => {
		const { container } = render(ChatError, {
			slots: {
