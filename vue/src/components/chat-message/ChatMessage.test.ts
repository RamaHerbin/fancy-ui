...
/**
 * Transposed case for case from the source's `ChatMessage.test.ts`. Port-only
 * adjustments:
 *
 * - The `avatar`/`children`/`actions`/`footer` snippets are slots, so they are
 *   passed as `slots` rather than through `createRawSnippet`; `children` is the
 *   default slot.
 ...
 *   starts it (D-V20), and the fill-in lands on the next tick — so the
 *   timestamp cases `await nextTick()` before reading the rendered `<time>`.
 */

function article(container: Element): HTMLElement {
...
describe("ChatMessage", () => {
	...
	it("shows a relative timestamp with the exact time as its tooltip", async () => {
	...
	it("accepts an epoch number as the timestamp, and renders none without one", async () => {
		const { container } = render(ChatMessage, {
	...
	it("lets the children snippet replace the default body", () => {
		const { container } = render(ChatMessage, {
			...
			slots: { default: '<p data-testid="custom">Rendered by the consumer</p>' },
		});
