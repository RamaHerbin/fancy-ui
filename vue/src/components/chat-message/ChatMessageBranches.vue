...
defineOptions({ name: "ChatMessageBranches", inheritAttrs: false });

const { index, count, onNavigate, class: className } = defineProps<ChatMessageBranchesProps>();
...
const atStart = computed(() => index <= 1);
const atEnd = computed(() => index >= count);
...
<template>
	<div
		:class="cn('text-muted-foreground flex items-center gap-0.5 text-xs', message?.role === 'user' && 'justify-end', className)"
		role="group"
