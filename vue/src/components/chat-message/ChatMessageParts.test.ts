...
import ChatMessageBranches from "./ChatMessageBranches.vue";
import { sound } from "../../sound/sound.js";

/**
 * Transposed case for case from the source's `ChatMessageParts.test.ts`. Port-only
 * adjustments:
 *
 * - The source's `ChatMessageHarness.test.svelte` rig is an inline
 *   `defineComponent` + `h()` here: the parts still get exercised through real
 *   markup, which is the only way to prove they read the context the root
 *   actually publishes rather than one the test invented.
 * - `rerender` merges where the source's replaces, so every case passes the
 *   full prop set.
 * - The cue goes through `useSoundCue`, which forwards its optional
 *   `SoundPlayOptions` argument, so the two cue-argument assertions read
 *   `("press", undefined)` / `("select", undefined)` — the package-wide shape.
 */
...
const Harness = defineComponent({
	name: "ChatMessageHarness",
	props: {
		role: { type: String as PropType<"user" | "assistant" | "system">, default: "assistant" },
		content: { type: String, default: "The answer" },
		alwaysVisible: { type: Boolean, default: false },
		index: { type: Number, default: 2 },
		count: { type: Number, default: 3 },
		onNavigate: { type: Function as PropType<(index: number) => void>, default: () => {} },
	...
	setup(props) {
		return () =>
			h(
				ChatMessage,
				{ role: props.role, content: props.content, sound: props.sound },
				{
					actions: () =>
						h(ChatMessageActions, { alwaysVisible: props.alwaysVisible }, {
							default: () =>
								h(
									ChatMessageAction,
									{ label: "Copy", confirmLabel: "Copied", onclick: props.onCopy },
									{ default: () => h("span", { "aria-hidden": "true" }, "C") }
								),
						}),
					footer: () =>
						h(ChatMessageBranches, {
							index: props.index,
							count: props.count,
...
function rail(container: Element): HTMLElement {
