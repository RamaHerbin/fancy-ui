...
/**
 * The key a `RadioGroupItem` reads the group's context under. The Svelte
 * source publishes it under a plain `Symbol` context key; an `InjectionKey`
 * is that same symbol, typed — so the exported name and its symbol shape are
 * both kept:
 *
 * ```ts
 ...
 * ```
 *
 * Read it with an explicit `undefined` default — an item rendered outside a
 * `RadioGroup` gets `undefined` rather than throwing (and rather than a
 * development warning), and renders as a plain, unchecked, standalone radio.
 */
export const RADIO_GROUP_KEY: InjectionKey<RadioGroupContext> = Symbol("radio-group-context");
