...
defineOptions({ name: "ImageGeneration", inheritAttrs: false });
...
const {
	...
	class: className,
	sound = false,
} = defineProps<ImageGenerationProps>();
 ...
 * Turning it on in the mount hook is early enough to still catch the load
...
const blurred = computed(
	() => mounted.value && loadedSrc.value !== src && erroredSrc.value !== src
);

function handleLoad() {
	// A cached image can be complete before the mount hook runs and fire its load
	// event after; both paths land here, and only the first one counts.
	if (loadedSrc.value === src) return;
	loadedSrc.value = src ?? null;
	erroredSrc.value = null;
	onLoad?.();
}
...
<template>
	...
	>
		...
		<p
			v-if="prompt"
			class="ft-imagegen-prompt text-muted-foreground text-xs leading-relaxed"
		>{{ prompt }}</p>
	</div>
</template>
