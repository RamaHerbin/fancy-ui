...
function image(container: Element): HTMLImageElement | null {
	return container.querySelector("img");
}

function frame(container: Element): HTMLElement {
...
describe("ImageGeneration", () => {
	...
	// Tier-3 law: the server render is deterministic and carries no blur class.
	it("renders identically twice on the server, sharp and unarmed", async () => {
		const props = { status: "done" as const, src: SRC, alt: "A red barn", prompt: "a red barn" };
		const a = await renderToString(createSSRApp(ImageGeneration, props));
		const b = await renderToString(createSSRApp(ImageGeneration, props));

		expect(a).toBe(b);
		expect(a).toContain("ft-imagegen-img");
		expect(a).not.toContain("ft-imagegen-img-blurred");
		expect(a).toContain("aspect-ratio:1 / 1");
	});
});
