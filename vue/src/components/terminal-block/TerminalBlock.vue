...
defineOptions({ name: "TerminalBlock", inheritAttrs: false });
...
const {
	...
	maxHeight = "20rem",
	class: className,
} = defineProps<TerminalBlockProps>();
...
useAutoscroll(
	() => log.value,
	() => ({ enabled: running, pinOnConnect: true })
);
...
const finished = computed(() => exitCode !== null && exitCode !== undefined);
const ok = computed(() => exitCode === 0);
const duration = computed(() => formatDuration(durationMs));
...
/**
 * Both of a segment's attributes are conditional, and an attribute the source
 * omits has to stay omitted: a falsy class binding would still write `class=""`
 * and an undefined colour would still write `style=""` on the server.
 */
function segmentAttrs(segment: AnsiSegment): Record<string, unknown> {
	const attrs: Record<string, unknown> = {};
	if (segment.bold) attrs.class = "ft-bold";
	if (segment.fg !== null) attrs.style = { color: colour(segment.fg) };
	return attrs;
}
...
<template>
	<div ref="el" :class="cn('ft-terminal overflow-hidden rounded-lg font-mono text-sm', className)">
		<div v-if="$slots.header" class="ft-rule flex items-center gap-2 border-b px-4 py-2">
		...
		<div
			...
			:style="{ maxHeight }"
		>
			<div v-if="command" class="ft-line flex gap-2">
				<span class="ft-prompt shrink-0 select-none" aria-hidden="true">{{ prompt }}</span>
				<span class="ft-wrap">{{ command }}</span>
			</div>
			<div v-for="(segments, row) in rows" :key="row" class="ft-line ft-wrap"
				><span
					v-for="(segment, i) in segments"
					:key="i"
					v-bind="segmentAttrs(segment)"
					>{{ segment.text }}</span
				><span v-if="running && row === rows.length - 1" class="ft-cursor" aria-hidden="true"
					>▊</span
				></div
			>
		</div>
		...
		>
			...
			<span v-if="duration" class="ft-muted ml-auto tabular-nums">{{ duration }}</span>
		</div>
	</div>
</template>
