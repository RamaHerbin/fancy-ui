...
function log(container: Element): HTMLElement {
...
describe("TerminalBlock", () => {
	...
	it("renders the header slot above the output", () => {
		const { container } = render(TerminalBlock, {
			props: { output: "a" },
			slots: { header: "<span>build.log</span>" },
		});
	...
	it("pins an already-running transcript that mounts overflowing", async () => {
		...
		try {
			...
			// The pin runs in a post-flush watcher (`useAutoscroll`'s mount effect),
			// which lands after the tick this render already awaited.
			await nextTick();
