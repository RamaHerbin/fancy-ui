...
import LineHoverLink from "./LineHoverLink.vue";
...
describe("LineHoverLink", () => {
	...
	// Not in the Svelte suite: Vue camelises declared prop names, so the Svelte
	// key `"aria-label"` could never be read back off `props`. The prop is
	// spelled `ariaLabel` here and both spellings must reach the anchor as
	// `aria-label`, exactly as the Svelte component renders it.
	it("forwards the accessible label to the anchor", () => {
		const { container } = render(LineHoverLink, {
			props: { href: "#", ariaLabel: "Read the manifesto" },
		});
	...
	it("accepts the hyphenated `aria-label` spelling a template would write", () => {
		const { container } = render(LineHoverLink, {
			props: { href: "#" },
			attrs: { "aria-label": "Read the manifesto" },
		});
		expect(container.querySelector("a")?.getAttribute("aria-label")).toBe(
			"Read the manifesto"
		);
	...
	it("emits no aria-label attribute when no label is given", () => {
		const { container } = render(LineHoverLink, { props: { href: "#" } });
