 ...
 *
 * - The custom `item` snippet becomes a scoped slot template string, its
 *   tuple params (`crumb`, `index`) turned into named object props.
...
function crumbLis(container: Element): HTMLLIElement[] {
...
function separatorsByPosition(container: Element): HTMLLIElement[] {
...
describe("Breadcrumb", () => {
	...
	it("hands a custom item slot the item and its index, and lets it fully replace the default rendering", () => {
		const { container } = render(Breadcrumb, {
			props: { items: TRAIL },
			slots: {
				item: '<template #item="{ item, index }"><b data-testid="custom">{{ index }}:{{ item.label }}</b></template>',
			},
		});
		...
		// custom slot is supplied — the consumer owns those semantics now.
	...
	it("exposes the nav element as ref", () => {
		const wrapper = mount(Breadcrumb, { props: { items: TRAIL }, attachTo: document.body });
	...
	describe("sound", () => {
		...
		it("plays nothing for a crumb rendered through the item slot — coverage is partial by construction, the override owns its own cue", async () => {
			const { container } = render(Breadcrumb, {
				props: { items: TRAIL, sound: true },
				slots: {
					item: '<template #item="{ item, index }"><a data-testid="custom" href="#">{{ index }}:{{ item.label }}</a></template>',
				},
			});
