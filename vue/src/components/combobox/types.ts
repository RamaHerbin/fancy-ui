...
/**
 * The counterpart of the source's `unique symbol` context key: a typed
 * injection key carrying the same name, because the source's `index.ts`
 * exports it and it keeps its public identity here. Read it with
 * `inject(COMBOBOX_KEY)`.
 */
export const COMBOBOX_KEY: InjectionKey<ComboboxContext> = Symbol("combobox-context");
