...
defineOptions({ name: "Pressable", inheritAttrs: false });
...
// Plain `ref`, not a media-query-gated one: the press *state* always tracks
// pointer/keyboard activity regardless of `prefers-reduced-motion`. Only the
// CSS transition that animates `[data-pressed]` is gated (see the style
// block below) — reduced motion swaps a moving scale for a static opacity
// change, it never turns press tracking off.
...
function isInsideRoot(target: EventTarget | null): boolean {
	return !!el.value && target instanceof Node && el.value.contains(target);
}

function press() {
	if (!disabled) pressed.value = true;
}
...
function handlePointerDown(event: PointerEvent) {
	if (disabled) return;
	// Mirrors this repo's own Drawer precedent: a non-primary mouse button
	// (right-click, middle-click) is not "a press". Touch and pen contacts
	// always report `button === 0`, so this only ever filters the mouse case.
...
/* Universal fallback, declared ABOVE the `no-preference` block on
   ...
   the `watch` in the script block actually handles (it clears `pressed`
