 ...
 *
 * - The source's value/ref harness is an inline `defineComponent` here: it
 *   drives `v-model:value` through an `onUpdate:value` listener and reads the
 ...
 * - `tick()` is `nextTick()`, which `fireEvent` already awaits.
 */
...
const ValueHarness = defineComponent({
	...
	setup(props) {
		const value = ref("");
		const child = ref<InstanceType<typeof Textarea> | null>(null);
...
describe("Textarea", () => {
	...
	it("round-trips the textarea element through the exposed ref", () => {
	...
	it("marks the limit-reached state with a non-colour attribute, not just a colour change", async () => {
		...
		await rerender({ value: "12345", maxlength: 5, showCount: true });
	...
	it("sets an explicit pixel height when autoResize is on, and never throws across repeated input", async () => {
		...
		// Repeated input exercises the watcher -> grow() -> style write path
		// several times in a row; a read/write loop on reactive state would
		// blow the recursive-update guard well before this many iterations.
		for (let i = 0; i < 20; i++) {
