...
function scrollRegion(container: Element): HTMLElement {
...
async function scrollAway(container: Element): Promise<HTMLElement> {
...
describe("ChatPanel", () => {
	...
	it("names itself as the conversation and lays out its four regions", () => {
		const { container } = render(ChatPanel, {
			slots: {
				...
				default: '<div data-testid="stream">two turns</div>',
	...
	it("mirrors streaming and empty onto the root as data attributes", async () => {
		...
		await rerender({ streaming: true, empty: true });
	...
	it("re-reads the geometry when the empty state gives way to a transcript", async () => {
		const { container, rerender } = render(ChatPanel, {
			props: { empty: true, streaming: false },
			slots: { default: "<p>two turns</p>", emptyState: "<p>How can I help?</p>" },
		});
		const region = patchGeometry(scrollRegion(container), 0);
		await fireEvent.scroll(region);
		region.scrollTop = BOTTOM;
		...
		await rerender({ empty: false, streaming: false });
...
describe("ChatEmptyState", () => {
	...
	it("lets a slot replace the mark, and renders children under the description", () => {
		const { container } = render(ChatEmptyState, {
			props: { description: "Pick a starting point." },
			slots: {
				icon: '<span data-testid="icon">✻</span>',
				default: '<span data-testid="suggestions">pills</span>',
			},
		});
