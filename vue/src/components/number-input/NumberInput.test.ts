 ...
 *   `setContext(FIELD_KEY, …)` around a `<NumberInput>`.
 * - The bindable `ref` is exposed on the instance, so the harness reads
 *   `numberRef.value.ref` instead of binding a local getter/setter pair.
 * - The cue goes through `useSoundCue`, which forwards its optional
 *   `options` argument, so the spy sees `("tick", undefined)`.
 */
...
function decrementButton(container: Element): HTMLButtonElement {
	return container.querySelector('button[aria-label="Decrease value"]') as HTMLButtonElement;
}

function incrementButton(container: Element): HTMLButtonElement {
...
const ValueHarness = defineComponent({
	name: "NumberInputValueHarness",
	setup() {
		const value = ref<number | null>(null);
		const numberRef = ref<{ ref: HTMLInputElement | null } | null>(null);

		watch(
			() => numberRef.value?.ref ?? null,
			(node) => {
		...
		return () => [
			h(NumberInput, {
				value: value.value,
				"onUpdate:value": (next: number | null) => (value.value = next),
				label: "Quantity",
				ref: numberRef,
			}),
			h(
...
describe("NumberInput", () => {
	...
	// buttons. Scoped CSS is unlayered and Tailwind's utilities sit in
	...
	it("reduced motion: stepping still works from both the buttons and the arrow keys", async () => {
		...
		// the moment this component grows a `prefersReducedMotion()` read. Matching on
	...
	it("round-trips the input element through the exposed ref", async () => {
	...
	it("inside a FormField, the context wins for controlId, aria-describedby, aria-invalid, required and disabled", () => {
		...
		// Deliberately passed props that disagree with the context, so this
	...
	describe("stepper feedback for the keyboard", () => {
		function stepping(container: Element): string[] {
