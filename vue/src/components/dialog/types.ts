 ...
 * of a root publishing a context to children a caller composes by hand.
 * `Dialog` and `AlertDialog` diverge enough in public API — a free-form
 * footer versus a fixed confirm/cancel pair, `dismissible` versus a
 * role-appropriate fixed default, `role="dialog"` versus `"alertdialog"` —
 * that neither should be built as a thin variant of the other. What they
 * actually share is this plumbing, so that is what is factored out, as
 * plain props passed straight from each root component to `DialogSurface`.
 * There is no arbitrary caller-authored markup in between for a context to
 * reach across — both call sites are this folder's own two root components
 * — so passing props directly says the same thing a context would, with one
 * less indirection.
 *
 * Not exported from `index.ts`: `DialogSurface` is an implementation
 * detail, not part of the package's public surface.
 *
 * Two fields of the source interface are absent here because this package
 * spells them differently: the panel's content is the default SLOT rather
 * than a `children` prop, and the panel element is published through
 * `defineExpose({ ref })` rather than a bindable `ref` prop (`ref` is a
 * reserved vnode key and cannot be either a prop or a model).
 */
export interface DialogSurfaceProps {
	 ...
	 * list). Forwarded to the focus trap's `fallbackFocus` — see that file's
	 * doc comment for the full fallback chain. Both `Dialog` and
