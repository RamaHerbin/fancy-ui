...
// unlike the Svelte package's, so every `new PointerEvent(...)` in the source
// suite would throw. Local polyfill only — `pointerType` and `isPrimary` are
// the two fields Magnetic actually reads.
 ...
 * archetype. Magnetic constructs its observer by calling the `inView` core as
 * a plain function (not through the `useInView` composable, see the
 * component's own comment on why), but the constructor it reaches is the
 * same global `IntersectionObserver` this mock replaces.
 */
...
/** Same shape as `media-query.test.ts`'s `stubMatchMedia`, keyed on the query
 * string rather than a blanket value for every query (checklist item 14: a
 * blanket `matches: true` would also flip an unrelated query like
 * `(hover: hover)` the moment Magnetic ever queries one). Returns a live
 * emitter so a test can simulate `prefers-reduced-motion` changing at
 * runtime, not just fix it once at mount. */
...
const CHILD = '<button type="button">Hover me</button>';
...
/**
 * `magnetState`'s `data-state` binding is a reactive attribute updated
 * through Vue's own scheduler — unlike the `--ft-magnetic-x/y` vars (written
 * by a direct, synchronous `style.setProperty` call), a `ref` write made from
 * a raw `window`/`document` listener does not flush the DOM until the next
 * microtask. Every dispatch that can flip `data-state` runs the frame queue
 * and then awaits `nextTick()` before assertions read the DOM.
 */
...
describe("Magnetic", () => {
	...
	function mount(
		...
		const { container, rerender, unmount } = render(Magnetic, {
			props,
			slots: { default: CHILD },
		});
	...
	it("exposes the outer wrapper as `ref`", () => {
		const wrapper = vtuMount(Magnetic, { slots: { default: CHILD } });
