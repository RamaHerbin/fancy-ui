...
//
// Declared in this module block rather than in `<script setup>`: a
// destructure default is compiled into the component's runtime `props`
// option, which lives OUTSIDE `setup()` and so cannot see a binding
// declared inside it.
...
defineOptions({ name: "Magnetic", inheritAttrs: false });
...
// on a node that itself moved, the field would desync from the pull and
// visually "chase" the pointer.
const outerEl = useTemplateRef<HTMLDivElement>("outer");
const innerEl = useTemplateRef<HTMLDivElement>("inner");

defineExpose({ ref: outerEl });

const magnetState = ref<"idle" | "active">("idle");
...
// The query is started from the composable's own `onMounted`, so nothing
// reads `matchMedia` during setup or on the server.
const reduced = useReducedMotion();

// `undefined` (not an object with an undefined value) when the prop is left
// at its default, so the attribute is never written at all.
const radiusStyle = computed(() =>
	radius === DEFAULT_RADIUS ? undefined : { "--ft-magnetic-radius": `${radius}px` }
);
 ...
 */
function release() {
	magnetState.value = "idle";
	innerEl.value?.style.setProperty("--ft-magnetic-x", "0px");
	innerEl.value?.style.setProperty("--ft-magnetic-y", "0px");
	needsRelease = false;
}
...
const handlePointerMove = rafThrottle((event: PointerEvent) => {
	...
	magnetState.value = "active";
	needsRelease = true;
	inner.style.setProperty("--ft-magnetic-x", `${pullX}px`);
	inner.style.setProperty("--ft-magnetic-y", `${pullY}px`);
});

/** The live teardown of whatever `buildField()` last put in place — the
 * Vue counterpart of the source effect's returned cleanup. */
...
function buildField() {
	...
	const outer = outerEl.value;
	if (!outer || disabled || reduced.value) {
		release();
		return;
	}
	...
	// The framework-free core is called directly (never through the
	// `useInView` composable): the observer must not exist AT ALL while
	// disabled or reduced-motion (this function's own guard above), and the
	// composable has no way to skip construction without also giving up the
	// attach/detach pairing below.
	//
	...
	//
	// The core's declared return type is `void | { update?, destroy? }` (the
	// generic action shape, to support implementations that legitimately
	// return nothing) — but this specific implementation always returns an
	// object, either `{}` (SSR / no-IntersectionObserver fallback) or
	// `{ update, destroy }`. The cast reflects what the real function does,
	// not a workaround for what it might do.
	const ioHandle = inView(outer, {
		once: false,
		threshold: 0,
...
// The source's single `$effect` splits into a mount hook plus one
// post-flush watcher: a `watch` without `immediate` never fires for the
// initial values, and an `immediate` one would run in SSR setup where
// `document` does not exist. `reduced` is already answered by the time
// this hook runs — the composable registered its own `onMounted` first.
onMounted(buildField);

watch([() => disabled, () => reduced.value, () => radius], buildField, { flush: "post" });
...
<template>
	...
	>
		<div ref="inner" class="ft-magnetic-inner">
			<slot />
		</div>
	</div>
</template>
...
.ft-magnetic::before {
	...
	/* No `background` declared here (not even `transparent`, which is the
	   ...
	   The scoping attribute lands on THIS half of a compound selector
	   (`.ft-magnetic[data-v-x]::before`, specificity (0,2,1)), which would
	   out-specify a consumer's `.ft-magnetic::before` override (0,1,1) — so
	   a `background` declared here can never actually be beaten by one
	   declared there. Leaving the property undeclared means the consumer's
	   rule is the ONLY one setting it, which is what makes README's
	   `.ft-magnetic::before` halo recipe (and the contract's promise of it)
	   actually work. */
