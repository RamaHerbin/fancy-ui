...
describe("ToolTimeline", () => {
	...
	it("renders the item slot in place of the built-in row body", () => {
		const { container } = render(ToolTimeline, {
			props: { items },
			slots: {
				item: '<template #item="{ item }"><span class="custom-row">custom</span></template>',
			},
		});
	...
	it("renders the timestamp as relative text with the exact time in the attributes", async () => {
		vi.useFakeTimers();
		vi.setSystemTime(T0);
		...
		// The shared clock's first real reading lands in `onMounted`, one flush
		// after the initial (pre-start sentinel) render.
		await nextTick();
	...
	it("accepts a Date timestamp as readily as epoch ms", async () => {
		vi.useFakeTimers();
		vi.setSystemTime(T0);
