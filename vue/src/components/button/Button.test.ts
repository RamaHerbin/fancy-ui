 ...
 * lead fade reads it while building each leg's spec, so an override
 ...
 * never observe the ONE flush the cross-fade lives in — by the time it
 * returns, the stubbed Web Animations chain has already resolved and the
 * outgoing node has left either way.
 *
 * Starts NOT loading on purpose: a transition never plays on the initial
 * render of the block that owns it, so the first click is the first genuine
 * swap, and the second click the genuine swap back.
 */
/**
 * `@vue/test-utils` stubs `<Transition>` out of every render by default, which
 * would leave the swap below cutting instead of cross-fading and the three
 * tests that watch it passing for the wrong reason. These renders opt the real
 * component back in.
 */
const REAL_TRANSITIONS = { global: { stubs: { transition: false } } };

const LeadHarness = defineComponent({
	name: "ButtonLeadHarness",
	setup() {
		...
		return () => [
			...
			h(
				...
				{
					default: () => "Save",
					iconStart: () => h("span", { class: "my-icon-start" }, "+"),
				}
			),
		];
	},
...
describe("Button", () => {
	...
	it("renders a real button with children as its label", () => {
		const { container } = render(Button, { slots: { default: "<span>Save</span>" } });
	...
	it("renders iconStart before the label when not loading", () => {
		const { container } = render(Button, {
			slots: {
				iconStart: '<span class="my-icon-start">+</span>',
				default: "<span>New</span>",
			},
		});
	...
	it("renders iconEnd after the label", () => {
		const { container } = render(Button, {
			slots: {
				iconEnd: '<span class="my-icon-end">→</span>',
				default: "<span>Continue</span>",
			},
		});
	...
	it("strips href and adds aria-disabled/tabindex=-1 on a disabled anchor, and swallows the click", () => {
		const onclick = vi.fn();
		const { container } = render(Button, {
			props: { href: "https://example.com", disabled: true, onclick },
			slots: { default: "<span>Visit</span>" },
		});
		const el = root(container);
	...
	it("makes a loading anchor inert: strips href, marks it aria-disabled, and drops it from the tab order", () => {
		...
		const onclick = vi.fn();
		const { container } = render(Button, {
			props: { href: "https://example.com", loading: true, onclick },
			slots: { default: "<span>Pay</span>" },
		});
		const el = root(container);
	...
	it("merges a caller class alongside the base classes, letting it win over a conflicting utility", () => {
		const { container } = render(Button, {
			props: { variant: "primary", class: "bg-red-500 my-button" },
			slots: { default: "<span>Go</span>" },
		});
	...
	// The scoped `<style>` declares a `transition` shorthand on this same
	// element, so the press scale can join the colour channel under
	// `prefers-reduced-motion: no-preference`. A scoped block is unlayered and
	// Tailwind's utilities sit in `@layer utilities`, so leaving
	...
	it("drops the transition-colors utility in favour of the hand-written channel", () => {
		const { container } = render(Button, { slots: { default: "<span>Go</span>" } });
	...
	describe("lead slot", () => {
		...
		function toggleOf(container: Element) {
		...
		it("wraps iconStart in the shared lead cell, exposed to assistive tech", () => {
			const { container } = render(Button, { slots: leadSlots });
		...
		it("keeps aria-busy on the control and aria-hidden on the cell, never the other way round", () => {
			const { container } = render(Button, { props: { loading: true }, slots: leadSlots });
			const el = root(container);
			const lead = container.querySelector(".ft-btn-lead")!;
		...
		it("reduced motion: the swap is synchronous, with no window where both are mounted", async () => {
			...
			// Duration 0 makes `runTransition` skip `element.animate()` entirely,
			// so the outgoing icon is gone in the same flush that mounts the
			// spinner — exactly the behaviour this button had before the fade
			// existed.
			expect(container.querySelector(".ft-btn-spinner")).toBeTruthy();
