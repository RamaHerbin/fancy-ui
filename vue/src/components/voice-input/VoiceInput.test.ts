...
/**
 * The Svelte suite's `VoiceInputHarness.test.svelte`, inlined. `v-model:active`
 * cannot be expressed from the props object a test hands to `render` — the
 * component's own writes never land back on it — so binding here and echoing
 * the value into the DOM is the only way to prove the recording state travels
 * back out to the consumer rather than merely changing what the component
 * draws.
 */
const Harness = defineComponent({
	name: "VoiceInputHarness",
	props: {
	...
	setup(props) {
		...
		return () => [
			h(VoiceInput, {
				...
				demo: props.demo,
				onStart: props.onStart as (() => void) | undefined,
				onStop: props.onStop as (() => void) | undefined,
				onCancel: props.onCancel as (() => void) | undefined,
				sound: props.sound,
			}),
			h("span", { "data-testid": "bound-active" }, active.value ? "recording" : "idle"),
		];
	},
});
...
function mic(container: Element): HTMLButtonElement | null {
	return container.querySelector('button[aria-label="Start voice input"]');
}

function cancelButton(container: Element): HTMLButtonElement {
	return container.querySelector('button[aria-label="Cancel voice input"]') as HTMLButtonElement;
}

function confirmButton(container: Element): HTMLButtonElement {
	return container.querySelector('button[aria-label="Stop voice input"]') as HTMLButtonElement;
}

function canvas(container: Element): HTMLCanvasElement | null {
	return container.querySelector("canvas");
}

function statusLine(container: Element): HTMLElement {
	return container.querySelector(".ft-voice-status") as HTMLElement;
}

function transcriptLine(container: Element): HTMLElement | null {
...
describe("VoiceInput", () => {
	...
	describe("sound", () => {
		...
		/*
		 * The two cases above are the suite's own transposition, and they run the
		 * component with a plain `active` prop. The two below run the SAME gesture
		 * through the harness, i.e. with a two-way binding in place. Both
		 * configurations exist here because they do not share a code path: with a
		 * binding the model ref hands the write to the consumer instead of
		 * applying it locally, so the guard inside the handler is the only thing
		 * left holding — the source's bindable, which writes through
		 * synchronously, holds in both.
		 */
		it("does not double-fire a repeated cancel dispatch under a two-way binding either", () => {
			const onCancel = vi.fn();
			const { container } = render(Harness, { props: { active: true, sound: true, onCancel } });
			const button = cancelButton(container);
		...
		it("does not double-fire a repeated confirm dispatch under a two-way binding either", () => {
			const onStop = vi.fn();
			const { container } = render(Harness, { props: { active: true, sound: true, onStop } });
			const button = confirmButton(container);
		...
		it("does not double-fire a repeated mic dispatch under a two-way binding either", () => {
			const onStart = vi.fn();
			const { container } = render(Harness, { props: { sound: true, onStart } });
			const button = mic(container) as HTMLButtonElement;
