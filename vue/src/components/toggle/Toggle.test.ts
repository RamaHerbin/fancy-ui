...
// Test-only rig, mirroring the Svelte harness: `v-model` cannot be expressed
// from a plain props object handed to `render`, so a small wrapper binds
// `pressed` and echoes both the value and whatever ref comes out of the
// binding into the DOM.
const Harness = defineComponent({
	components: { Toggle },
	setup() {
		const pressed = ref(false);
		const toggleRef = useTemplateRef<InstanceType<typeof Toggle>>("toggleRef");
		onMounted(() => {
			toggleRef.value?.ref?.setAttribute("data-bound-ref", "yes");
		});
		return { pressed, toggleRef };
	},
	template: `
		<Toggle ref="toggleRef" v-model:pressed="pressed" label="Bold">B</Toggle>
...
describe("Toggle", () => {
	...
	it("round-trips pressed through v-model:pressed", async () => {
	...
	// element. Vue's scoped CSS is unlayered and Tailwind's utilities sit in
	...
	it("round-trips the button element through the exposed ref", () => {
		// The harness marks whatever comes out of the binding; finding the mark
		// on the rendered button is what proves the two are the same element.
