...
import InlineCitation from "./InlineCitation.vue";
...
/*
 * The Svelte suite keeps this rig in a `.test.svelte` harness: the one thing a
 * `.ts` test cannot otherwise set up is the marker inside real prose, with
 * punctuation pressed straight up against it. Whether the component leaks a
 * whitespace text node of its own is only observable from the outside, in the
 * sentence it interrupts. Inlined here as a render function, because `h()`
 * writes exactly the child list a template would have to spell without
 * whitespace.
 */
const Harness = defineComponent({
	name: "InlineCitationHarness",
	props: {
		source: { type: Object as PropType<SourceData>, required: true },
		index: { type: Number, default: 1 },
		href: { type: String, default: undefined },
	},
	setup(props) {
		return () =>
			h("p", { "data-testid": "prose" }, [
				"Worth a read",
				h(InlineCitation, { source: props.source, index: props.index, href: props.href }),
				".",
			]);
	},
});

function marker(container: Element): HTMLElement {
	return container.querySelector(".ft-citation-marker") as HTMLElement;
}

function preview(container: Element): HTMLElement | null {
	return container.querySelector(".ft-citation-preview");
}

/** Hover the marker and let the open delay elapse. */
async function hoverOpen(container: Element) {
...
describe("InlineCitation", () => {
	...
	it("leaves the card's own chrome to the card, and puts it back for a custom body", async () => {
		...
		const custom = render(InlineCitation, {
			props: { source: source(), index: 1 },
			slots: { preview: "<template #preview><span>own</span></template>" },
		});
	...
	it("replaces the card body with the preview slot", async () => {
		const { container } = render(InlineCitation, {
			props: { source: source(), index: 1 },
			slots: {
				preview:
					'<template #preview="{ source }"><span class="custom">{{ source.title }} · own card</span></template>',
			},
		});
		await hoverOpen(container);
