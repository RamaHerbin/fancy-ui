 ...
 * renders it after any custom default-slot content and passes Button no
 * `iconEnd`. It is the visible label unless something else already owns that
 * (icon-only, or custom content), in which case it is `sr-only`. It no longer
 * carries `aria-live` in the default composition: StatusMorph owns the
 * announcement.
 */
function visibleLabel(container: Element): HTMLElement {
...
function morph(container: Element): HTMLElement | null {
 ...
 * `fireEvent.click` await only flushes as far as Vue's own tick. Advancing
...
describe("CopyButton", () => {
	...
	it("writes the value to the clipboard on click", async () => {
		...
		const { container } = render(CopyButton, { props: { value: "npm install fancy-ui-vue" } });

		await fireEvent.click(button(container));
		await flush();

		expect(writeText).toHaveBeenCalledWith("npm install fancy-ui-vue");
	});
	...
	it("announces a failed copy assertively, and a successful one politely, through the portalled region", async () => {
		...
		// The region renders in place on the server pass and only relocates once
		// StatusMorph's `onMounted` has flipped its portal on, so the move lands
		// one tick after mount rather than during it. Svelte's `use:portal`
		// action relocates it at mount too — a tick either way, same end state.
		await nextTick();
	...
	// Custom default-slot content replaces `iconStart`, so there is no
	// StatusMorph and no portalled region in this composition — the label span
	// keeps `aria-live` here, and only here, so custom content is not left
	// announcing nothing.
	...
	// to idle, and custom default-slot content removes StatusMorph entirely — so
	// the parent has to own the timer for that composition or the failure state
	// sticks forever.
	...
	it("cancels the pending reset timer on unmount", async () => {
		...
		unmount();
		// Proves the teardown actually called `copyState.destroy()`, not just
		// that the DOM disappeared — a leaked timer would still be sitting in
		// the fake-timer queue here.
