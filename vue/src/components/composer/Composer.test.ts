...
import { defineComponent, h, inject, nextTick, ref, type PropType } from "vue";
 ...
 * - The bindables are `v-model:value` / `v-model:attachments`, so the rig
 *   passes `onUpdate:*` listeners rather than `bind:`.
 * - The `.test.svelte` rig is declared inline here: it exists in the source
 *   only because a Svelte component needs its own file.
 * - This framework's `fireEvent` does not return `dispatchEvent`'s boolean, so
 *   the Shift+Enter case dispatches the event itself and reads it back.
 */

/** The context probe the source's rig renders as a child of the composer. */
const Probe = defineComponent({
	name: "ComposerProbe",
	props: {
		onContext: {
			type: Function as PropType<(context: ComposerContext | undefined) => void>,
			required: false,
		},
	},
	setup(props) {
		...
		return () => [
			h("output", { "data-testid": "context-value" }, context?.value.current ?? ""),
			h(
				"output",
				{ "data-testid": "context-attachments" },
				String(context?.attachments.current.length ?? 0)
			),
			h("output", { "data-testid": "context-disabled" }, context?.disabled ? "yes" : "no"),
			h("output", { "data-testid": "context-streaming" }, context?.streaming ? "yes" : "no"),
		];
	},
});
	...
	setup(props) {
		...
		return () => [
			h(
				...
				{
					default: () => [
						h(ComposerInput, { placeholder: props.placeholder, maxRows: props.maxRows }),
						h("div", { class: "flex items-center gap-2" }, [
							h("div", { class: "flex-1" }),
							h(ComposerSubmit),
						]),
						h(Probe, { onContext: props.onContext }),
					],
				}
			),
			h("output", { "data-testid": "bound-value" }, value.value),
...
/**
 * This framework's `fireEvent` does not return `dispatchEvent`'s boolean, so
 * the one case that asserts on it dispatches the event itself.
 */
...
describe("Composer", () => {
	...
	it("renders the accessory as an overlay on top of the composer", () => {
		const { container } = render(Composer, { slots: { accessory: "<p>Voice</p>" } });
...
describe("ComposerInput", () => {
	...
	it("submits on Enter and adds a newline on Shift+Enter", async () => {
		...
		const shifted = await keyDown(textarea(container), { key: "Enter", shiftKey: true });
		expect(onSubmit).not.toHaveBeenCalled();
