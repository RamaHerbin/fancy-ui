 ...
 * may issue. Every part reads it through `inject` instead of having six values
 ...
 * reactive scope — a `computed`, a template — re-runs when the root's draft
 * changes. The object itself is built once and never replaced.
 *
 ...
 * value type so `provide`/`inject` are checked. Read it as optional — a part
 * rendered outside a `Composer` gets `undefined` rather than throwing:
 *
 * ```ts
