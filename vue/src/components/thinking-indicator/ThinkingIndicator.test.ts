...
describe("ThinkingIndicator", () => {
	...
	it("renders the done slot instead of the status once stopped", () => {
		const { container } = render(ThinkingIndicator, {
			props: { status: "Reading files", running: false },
	...
	it("keeps showing the status while running even when a done slot is provided", () => {
		const { container } = render(ThinkingIndicator, {
