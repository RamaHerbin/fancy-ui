...
defineOptions({ name: "ThinkingIndicator", inheritAttrs: false });
...
const {
	...
	showElapsed = true,
	class: className,
} = defineProps<ThinkingIndicatorProps>();

/** The source's `done` snippet: rendered instead of the status label once `running` is false. */
const slots = defineSlots<{ done?(): unknown }>();
...
// with the markup being hydrated. The mount hook below reads the real elapsed
// time immediately after mounting — client-only, as `onMounted` never runs
// during SSR — so a row that mounts already finished stays at 0s, which is
// what the README documents for that case.
const elapsed = createElapsed();

// The whole of the source's timer effect: `start` takes over from whatever ran
// before, and `stop` is the cleanup, so flipping `running` off, swapping in an
// external `elapsedMs`, or changing `since` all tear the interval down before
// the next run starts a new one. `onMounted` + a non-immediate watcher rather
// than `immediate: true`, because an immediate watcher would run in SSR setup
// and schedule an interval on the server.
function syncTimer() {
	elapsed.stop();
	if (running && since !== undefined && elapsedMs === undefined) elapsed.start(since);
}

onMounted(syncTimer);
watch([() => running, () => since, () => elapsedMs], syncTimer, { flush: "post" });
onScopeDispose(() => elapsed.stop());

const hasElapsed = computed(() => elapsedMs !== undefined || since !== undefined);
const effectiveMs = computed(() => elapsedMs ?? elapsed.ms);
...
<template>
	...
	>
		<span
			v-if="variant === 'pill'"
			:class="['ft-thinking-dot', { 'ft-thinking-dot-live': running }]"
			aria-hidden="true"
		></span>
