...
const classes = computed(() =>
	cn(
		...
		variant.value === "underline"
			? cn("border-border", orientation.value === "vertical" ? "border-r" : "border-b")
...
const indicatorEl = useTemplateRef<HTMLDivElement>("indicator");
...
// not reactive state — only the placement pass reads or writes them, and
// nothing needs to react to them changing. They are what lets one pass
...
function activeTrigger(): HTMLElement | null {
	return (
		el.value?.querySelector<HTMLElement>('[data-ft-tabs-trigger][aria-selected="true"]') ?? null
	);
}
...
function place(animate: boolean): void {
	...
	if (variant.value === "segmented") {
		...
		// should set.
		node.style.setProperty("--ft-tabs-indicator-sx", String(active.offsetWidth));
		node.style.setProperty("--ft-tabs-indicator-sy", String(active.offsetHeight));
	}

	// The selection did not move — this is a focus walk (the placement pass
	// below reads `focusedValue`), or a resize that changed nothing. Rewriting
	// a byte-identical transform would force a layout for nothing, and — on
	...
	// destination.
	if (node.style.transform === transform) return;
	...
	if (animate && !node.style.transform) animate = false;
...
// Everything that outlives a single render lives here, and it is keyed to
// the component's own mount — deliberately NOT to the selection.
//
// DEVIATION from the contract's "one effect owns all of it": a
// `ResizeObserver` delivers a callback as soon as `observe()` runs, so
// folding the observer into the placement pass below would tear it down
// and re-establish it on every tab change, and that first callback would
// land right after the tween's write and snap it. Keeping it mount-scoped
// keeps the observer alive across selections; the teardown still does both
...
onMounted(() => {
	...
	function observeTriggers(node: HTMLElement): void {
		for (const trigger of node.querySelectorAll<HTMLElement>("[data-ft-tabs-trigger]")) {
...
function runPlacement(): void {
	...
	// registry, so a trigger mounting or unmounting re-runs this pass
	...
	const nextOrientation = orientation.value;
	const nextVariant = variant.value;
...
// `onMounted` is the first placement — the one phase that matches both
// halves of Svelte's `$effect` (never runs on the server, first client run
// lands after the DOM exists). An `immediate` watcher would run in SSR
// setup and measure a DOM that does not exist. Every later placement is the
// `post`-flush watcher below, landing in the same flush a Svelte effect
// would.
onMounted(runPlacement);
