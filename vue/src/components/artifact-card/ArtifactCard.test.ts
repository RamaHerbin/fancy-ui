...
function nav(container: Element, label: string): HTMLButtonElement {
	return container.querySelector(`button[aria-label="${label}"]`) as HTMLButtonElement;
}

function openCue(container: Element): HTMLButtonElement {
	return container.querySelector(".ft-artifact-open") as HTMLButtonElement;
}

const ACTION_SLOT = { actions: '<button type="button" class="custom-action">Copy</button>' };
...
describe("ArtifactCard", () => {
	...
	it("names the status out loud, since the sweep and the tint are silent", async () => {
		...
		await rerender({ title: "Draft", status: "done" as const });
	...
	it("disables the arrow that would run off the end", async () => {
		...
		await rerender({ title: "Memo", version: 5, versionCount: 5, onVersionChange });
	...
	it("trails a cursor only while the document is being written", async () => {
		...
		await rerender({ title: "Memo", status: "done" as const, preview: "Half a" });
	...
	it("runs the top sweep only while streaming", async () => {
		...
		await rerender({ title: "Memo", status: "done" as const });
