...
defineOptions({ name: "ArtifactCard", inheritAttrs: false });
...
const isStreaming = computed(() => status === "streaming");
const isError = computed(() => status === "error");

const hasPreview = computed(() => preview !== undefined && preview !== "");
const hasVersion = computed(() => version !== undefined);
const hasCount = computed(
	() => hasVersion.value && versionCount !== undefined && versionCount > 0
);
...
const navigable = computed(() => hasCount.value && onVersionChange !== undefined);

const versionLabel = computed(() =>
	hasCount.value ? `v${version}/${versionCount}` : hasVersion.value ? `v${version}` : ""
);
const atFirst = computed(() => (version ?? 1) <= 1);
const atLast = computed(() => (version ?? 1) >= (versionCount ?? 1));
	...
	>
		...
		<div class="flex items-start gap-2.5 px-3 py-2.5">
			...
			<div class="flex flex-none items-center gap-1">
				...
				<span
					v-else-if="hasVersion"
					class="ft-artifact-version text-muted-foreground text-xs tabular-nums"
					>{{ versionLabel }}</span
				>

				<!--
					`{#if actions}` is a truthiness test on a reactive snippet prop; the
					slot is the port's stand-in and the gate reads it in the template, so
					every render sees the slots the parent passes now. `$slots` is not a
					reactive source: a `computed` over it has no dependency to invalidate
					and would freeze the rail at its first value.
				-->
				<div v-if="$slots.actions" class="ft-artifact-actions flex items-center gap-1">
		...
		<div
			v-if="hasPreview"
			class="ft-artifact-preview border-border border-t px-3 py-3 text-sm"
		>
		...
		<p v-if="isError" class="ft-artifact-error border-border border-t px-3 py-2 text-xs">
			{{ ERROR_TEXT }}
		</p>
	</div>
</template>
