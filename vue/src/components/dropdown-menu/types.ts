/**
 * The three contracts this family publishes, and the injection keys that
 * carry them.
 *
 * The source publishes each one under a `unique symbol` context key; here
 * each key is the package's own typed-key-plus-bindings helper (convention
 * C-3), so the exported names are kept and a consumer reads one with
 * `MENU_KEY.useRequired()` — a named error outside its provider rather than
 * `undefined`. The underlying `InjectionKey` is `MENU_KEY.key`.
 */
...
export interface MenuContext {
	 ...
	 * a panel straight to its own direct items. Provide/inject isn't
	 * DOM-based, though: it follows the *component* tree, which teleporting a
...
export const MENU_KEY: InternalContext<MenuContext> = createInternalContext<MenuContext>(
	"MenuContext"
);
...
export const SUB_KEY: InternalContext<SubContext> =
	createInternalContext<SubContext>("MenuSubContext");
...
export const DROPDOWN_MENU_KEY: InternalContext<DropdownMenuRootContext> =
	createInternalContext<DropdownMenuRootContext>("DropdownMenuRootContext");
