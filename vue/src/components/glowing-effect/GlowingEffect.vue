...
defineOptions({ name: "GlowingEffect", inheritAttrs: false });
...
const {
	...
	movementDuration = 2,
	borderWidth = 1,
} = defineProps<GlowingEffectProps>();
...
onMounted(() => {
	...
	engine = createGlowingEffect({ container }, { inactiveZone, proximity, movementDuration });
});
...
watch(
	() => [inactiveZone, proximity, movementDuration] as const,
	([nextInactiveZone, nextProximity, nextMovementDuration]) => {
		engine?.setOptions({
			inactiveZone: nextInactiveZone,
