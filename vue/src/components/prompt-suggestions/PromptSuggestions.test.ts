...
function pills(container: Element): HTMLButtonElement[] {
...
describe("PromptSuggestions", () => {
	...
	it("renders the item slot in place of the plain label", () => {
		const { container } = render(PromptSuggestions, {
			props: { suggestions: SUGGESTIONS },
			slots: {
				item: '<template #item="{ suggestion, index }"><span class="custom-pill">{{ index }}: {{ suggestion }}</span></template>',
			},
		});
	...
	describe("sound", () => {
		...
		it("never plays when the staggered entrance replays — only a pick plays the cue", async () => {
			const { rerender } = render(PromptSuggestions, {
