...
import type { RunStatus } from "../../internals/ai-types.js";

defineOptions({ name: "SubagentList", inheritAttrs: false });
...
const {
	...
	class: className,
	sound = false,
} = defineProps<SubagentListProps>();

defineSlots<{
	/** Replaces the built-in row body, keeping the status dot */
	item?(props: { agent: SubagentData; index: number }): unknown;
}>();
 ...
 * instead and is absent from `SubagentListProps`.
 */
...
const runningCount = computed(() => agents.filter((agent) => agent.status === "running").length);
...
const autoLabel = computed(() => {
	...
	if (runningCount.value > 0) return `${runningCount.value} ${noun(runningCount.value)} running`;
	if (allFinished.value) return `${total} ${noun(total)} finished`;
	return `${total} ${noun(total)}`;
});

const listLabel = computed(() => label ?? autoLabel.value);
...
<template>
	<div ref="rootEl" :class="[cn('ft-subagents w-full text-sm', className), { 'ft-subagents-compact': compact }]">
		<!--
		...
		<ul class="ft-subagents-list" role="list" :aria-label="listLabel">
			<!--
				...
				disambiguated or the block would crash in the Svelte source — see `rows`
				for how, and why it counts occurrences rather than positions.
			-->
			<li v-for="{ agent, index, key } in rows" :key="key" class="ft-subagents-row">
				<button
					v-if="onSelect"
					type="button"
				...
				>
					...
					<span
						...
						:class="{
							'ft-status-pending': agent.status === 'pending',
							'ft-status-running': agent.status === 'running',
							'ft-status-done': agent.status === 'done',
							'ft-status-error': agent.status === 'error',
							'ft-status-cancelled': agent.status === 'cancelled',
						}"
						aria-hidden="true"
					></span>
					...
					<template v-if="$slots.item">
						...
						<slot name="item" :agent="agent" :index="index" />
					</template>
					<template v-else>
						<span class="ft-subagents-main min-w-0 flex-1">
							<span class="flex min-w-0 items-baseline gap-1.5">
								...
								<span
									v-if="agent.model"
									class="ft-subagents-model text-foreground/70 shrink-0 font-mono"
									>{{ agent.model }}</span
								>
							</span>
							<span
								v-if="!compact && agent.task"
						...
						<span class="ft-subagents-meta flex shrink-0 items-center">
							<template v-if="showsProgress(agent)">
								<!--
								...
								-->
								<span class="sr-only"
									>{{ STATUS_LABELS[agent.status] }}, {{ percent(agent.progress) }}%</span
								>
								<span
									...
									aria-valuemin="0"
									aria-valuemax="100"
									:aria-valuenow="percent(agent.progress)"
								>
									<span
										class="ft-subagents-progress-fill"
										:style="{ width: `${percent(agent.progress)}%` }"
									></span>
								</span>
							</template>
							<span v-else class="ft-subagents-state text-muted-foreground">{{
								STATUS_WORDS[agent.status]
							}}</span>
						</span>
					</template>
				</button>
				<span v-else class="ft-subagents-body flex w-full items-start gap-2.5 rounded-md">
					<span
						class="ft-subagents-dot"
						:class="{
							'ft-status-pending': agent.status === 'pending',
