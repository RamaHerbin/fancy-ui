...
function states(container: Element): string[] {
...
describe("SubagentList", () => {
	...
	it("lets the item slot replace the row body, keeping the status dot", () => {
		const { container } = render(SubagentList, {
			props: { agents },
			slots: {
				item: '<template #item="{ agent, index }"><span class="custom-row">{{ index }}: {{ agent.name }}</span></template>',
			},
		});
	...
	it("speaks the status of a replaced row, since the dot is hidden from assistive tech", () => {
		const { container } = render(SubagentList, {
			props: { agents: [agent({ status: "error" })] },
			slots: {
