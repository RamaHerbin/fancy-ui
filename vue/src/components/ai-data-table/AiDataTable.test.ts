...
import AiDataTable from "./AiDataTable.vue";
import type { AiDataTableColumn, AiDataTableRow } from "./AiDataTable.vue";

/**
 * Transposed case for case from the source component's suite. One shape
 * changed: the `cell` snippet is a scoped slot here, so the snippet case
 * passes slot markup reading `value` and `at` instead of a raw snippet.
 */
...
function headers(container: Element): HTMLTableCellElement[] {
	return [...container.querySelectorAll("thead th")] as HTMLTableCellElement[];
}

function bodyRows(container: Element): HTMLTableRowElement[] {
...
describe("AiDataTable", () => {
	...
	it("hands the cell slot the raw value and where it sits", () => {
		const { container } = render(AiDataTable, {
			props: { columns: COLUMNS, rows: ROWS },
			slots: {
				cell: `<template #cell="{ value, at }"><span class="custom-cell">{{ at.row }}:{{ at.key }}={{ String(value) }}</span></template>`,
			},
		});
