...
defineOptions({ name: "AiDataTable", inheritAttrs: false });
...
const {
	...
	highlightColumn,
	class: className,
} = defineProps<AiDataTableProps>();

defineSlots<{
	/**
	 * Replaces the default cell rendering. Receives the raw value and its
	 * position — the source's two snippet parameters, named.
	 */
	cell?(props: { value: unknown; at: { row: number; key: string } }): unknown;
}>();

// The scrolling root, published on the instance: `ref` is a reserved vnode key,
// so the source's bindable `ref` cannot be a prop here.
...
<template>
	...
	>
		<table
			class="ft-datatable-table w-full border-collapse text-sm"
			:class="{ 'ft-datatable-dense': dense }"
		>
			<caption
				v-if="caption"
				:class="
			...
			<tbody>
				<tr
					...
					:key="rowIndex"
					v-bind="rowIndex % 2 === 1 ? { class: 'ft-datatable-zebra' } : {}"
				>
					<td
						v-for="(column, columnIndex) in columns"
						:key="columnIndex"
					...
					>
						<slot
							...
							:at="{ row: rowIndex, key: column.key }"
						/>
						<template v-else-if="typeof row[column.key] === 'boolean'">
							<template v-if="row[column.key]">
								<svg
									...
									aria-hidden="true"
								><path d="m5 13 4 4L19 7" /></svg> <span class="sr-only">{{ TRUE_LABEL }}</span>
							</template>
							<template v-else>
								<span class="ft-datatable-false text-muted-foreground" aria-hidden="true">–</span> <span class="sr-only">{{ FALSE_LABEL }}</span>
							</template>
						</template>
						<template
							v-else-if="
						...
						>
							...
							-->
							<span class="ft-datatable-empty text-muted-foreground" aria-hidden="true">—</span> <span class="sr-only">{{ EMPTY_LABEL }}</span>
						</template>
