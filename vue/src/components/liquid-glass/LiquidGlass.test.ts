...
describe("LiquidGlass", () => {
	...
	// is minted after mount from the shared counter rather than from
	// `Math.random()`. Same client-only timing as the Svelte source's `onMount`,
	// same per-instance uniqueness, nothing random in a render path — so a server
	// render and its hydration cannot disagree.
	it("gives each mounted instance its own filter id", async () => {
		const { container } = render({
			render: () => [h(LiquidGlass), h(LiquidGlass)],
		});
		// The id is minted in `onMounted`, so the filter lands on the next flush.
		await nextTick();
	...
	it("renders no filter on the server, exactly as the initial client render does", async () => {
		const html = await renderToString(createSSRApp(LiquidGlass));
