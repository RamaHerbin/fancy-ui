...
// by default, which would silently delete the row exit these tests exist to
// pin — a stubbed group removes its child in the same patch, exactly the
// regression the source's `|global` outro was written to stop. Opted out for
// this file only; Vitest isolates the module registry per test file.
...
function dropzone(container: Element): HTMLElement {
	return container.querySelector(".ft-file-upload-dropzone") as HTMLElement;
}

function rows(container: Element): NodeListOf<HTMLLIElement> {
	return container.querySelectorAll("li");
}

function removeButtons(container: Element): NodeListOf<HTMLButtonElement> {
	return container.querySelectorAll("[data-file-remove]");
}

function progressFill(container: Element): HTMLElement {
 ...
 *
 * `runTransition` issues two `animate()` calls per leg: a zero-length dummy
 * whose `onfinish` builds the real keyframes, then the real one. The dummy
 * still resolves on a microtask; only the real one is held. Returns a
 * `release()` that finishes everything held so far.
 */
function holdExits() {
...
/**
 * Transposed from the Svelte value harness. `v-model:files` cannot be
 * expressed from a `.ts` test file through `render(Cmp, { props })` alone —
 * the props object is copied into the component's own model, so a write
 * inside the component never lands back on it. Owning the list here and
 * echoing the count into the DOM is the only way to prove `files` travels
 * back out to the consumer rather than merely changing what FileUpload draws
 * internally, and the same goes for the exposed `ref`.
 */
const ValueHarness = defineComponent({
	name: "ValueHarness",
	setup() {
		const files = ref<UploadFile[]>([]);
		const upload = ref<{ ref: HTMLInputElement | null } | null>(null);

		onMounted(() => {
			upload.value?.ref?.setAttribute("data-bound-ref", "yes");
		});
		...
		return () => [
			h(FileUpload, {
				ref: upload,
				files: files.value,
				"onUpdate:files": (next: UploadFile[]) => {
					files.value = next;
				},
				multiple: true,
			}),
			h("span", { "data-testid": "bound-count" }, String(files.value.length)),
		];
	},
});
...
describe("FileUpload", () => {
	...
	it("re-announces an identical rejection instead of leaving the live region silent the second time", async () => {
		...
		await fireEvent.drop(zone, {
			dataTransfer: { files: [makeFile("notes.txt", 100, "text/plain")] },
		});
		await nextTick();
		expect(live.textContent).toMatch(/notes\.txt/);

		// A regression assigns the identical string back to the same ref,
	...
	it("omits the progress custom property entirely while progress is null", () => {
		...
		// The binding resolves to `undefined`, which the style patcher treats as
		// "omit" (the source's `null` reaches the same branch, and `CSSProperties`
		// admits only the former) — writing the property with an empty value
		// instead would leave the indeterminate block at `scaleX()` of nothing.
	...
	// A row leaves through a transition now, so "list changes, node goes" stops
	// being one step. The row must still be in the DOM while it fades, and it
	// must be `inert` for the whole of it — a closing row is not something a
	// pointer or a screen reader should be able to reach.
	...
	// The LAST row is the one case where the row's own list is not what tears it
	// down: `files.length` hits zero and the enclosing `v-if` would destroy the
	// whole `<ul>`, and a transition on a child of an unmounting element never
	// runs — so every other removal would fade and the final row would vanish on
	// the spot. Holding the list mounted for the length of the leg is what puts
	// it back on the same clock.
	...
	// The §1.2 fast path: `duration: 0` makes `runTransition` call `onFinish`
	...
	it("round-trips files and ref through v-model:files / the exposed ref", async () => {
	...
	it("inside a FormField, the context wins for controlId, aria-describedby, aria-invalid, required and disabled", () => {
		...
		// Deliberately passed own props that disagree with the context, so this
		// proves the context wins rather than merely matching by coincidence.
		const { container } = render(FileUpload, {
