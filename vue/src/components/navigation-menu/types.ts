 ...
 * Here each one is an `InternalContext` — a typed `InjectionKey` plus its
 * `provide` / `useRequired` bindings (the `ToggleGroup` precedent) — so the
 * exported names are kept and a piece rendered outside its provider throws a
 * named error instead of reading `undefined` off nothing.
 */
...
export interface NavigationMenuContext {
	...
	/**
	 * The item value currently owed a focus-into-panel move, or `null`.
	 *
	 * Added by this package, and the one member with no counterpart in the
	 * source (see the folder's divergence note). The source's panel effect
	 * subscribes to the pending value simply by CALLING
	 * `consumeFocusRequest()` inside a tracked effect; a Vue watcher's
	 * callback body is untracked by construction, so the panel has to name
	 * the value it depends on as an explicit watch source. Read-only, and
	 * nothing but that watch source should read it — `consumeFocusRequest` is
	 * still the only way to take the request.
	 */
	readonly pendingFocusValue: string | null;
...
export const NAVIGATION_MENU_KEY: InternalContext<NavigationMenuContext> =
	createInternalContext<NavigationMenuContext>("NavigationMenuContext");
...
export const NAVIGATION_MENU_ITEM_KEY: InternalContext<NavigationMenuItemContext> =
	createInternalContext<NavigationMenuItemContext>("NavigationMenuItemContext");
