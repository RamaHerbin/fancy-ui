...
import EditorialEngine from "./EditorialEngine.vue";
...
// of letting the `.then()` handler call into the measurement pass and throw.
...
// Lets the mount hook boot the (mocked) engine, so `ready` flips to true.
...
describe("EditorialEngine", () => {
	...
	it("mounts without throwing when the environment has no font loading API", () => {
		// No `document.fonts` at all — the shape of every non-browser DOM
		// implementation. Reading `.ready` off it must not escape the mount hook.
		Object.defineProperty(document, "fonts", {
			configurable: true,
			writable: true,
			value: undefined,
