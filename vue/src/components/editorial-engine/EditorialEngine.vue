 ...
 * from cached font metrics — the render loop performs zero DOM reads.
 */
...
defineOptions({ name: "EditorialEngine", inheritAttrs: false });
...
const {
	...
	class: className,
} = defineProps<EditorialEngineProps>();
...
// Bumped on every teardown. A `document.fonts.ready` handler queued by a
// superseded options snapshot reads it and bails, which is what the Svelte
// effect's `cancelled` flag does per run.
let generation = 0;
...
function start() {
	...
	// Text is measured against the canvas font engine — fonts must be loaded
	// first or widths come from the fallback font. `document.fonts` is not
	// universally present (non-browser DOM implementations omit the font
	// loading API entirely), and reading `.ready` off `undefined` would throw
	// straight out of the mount hook and take the whole app down with it.
	// Falling back to an already-settled promise keeps the boot on its normal
	// path; widths then come from whatever font the environment resolves.
...
// Created on mount, and re-created whenever the text props change so the
// prepared measurements always match the rendered content. Neither hook nor
// watcher runs during SSR.
onMounted(start);
onBeforeUnmount(stop);

watch(
	[() => headline, () => body, () => pullquotes, () => fontFamily],
	() => {
		stop();
		start();
	},
...
<template>
	<div :class="[cn('ee-stage', className), { ready }]" :style="{ fontFamily }">
		<!--
		...
		<div ref="layer" class="ee-layer" aria-hidden="true"></div>
	</div>
</template>
...
.ee-stage :deep(.ee-headline-line) {
	position: absolute;
	white-space: pre;
	font-weight: 700;
	color: #fff;
...
.ee-stage :deep(.ee-pullquote-box) {
	position: absolute;
	pointer-events: none;
	z-index: 3;
...
.ee-stage :deep(.ee-pullquote-line) {
	position: absolute;
	white-space: pre;
	font-style: italic;
	color: #b8a070;
