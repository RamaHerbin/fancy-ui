...
function more(container: Element): HTMLButtonElement | null {
...
describe("WebSearch", () => {
	...
	it("counts the results in the header, singular and plural", async () => {
		...
		await rerender({ query: QUERY, results: RESULTS.slice(0, 1) });
	...
	it("renders the item slot in place of the built-in row body", () => {
		const { container } = render(WebSearch, {
			props: { query: QUERY, results: RESULTS },
			slots: {
				item: '<template #item="{ result, index }"><span class="custom-row">{{ index }}: {{ result.title }}</span></template>',
			},
		});
	...
	describe("sound", () => {
		...
		it("plays select exactly once when a row is activated, with sound enabled", async () => {
			...
			// `useSoundCue` always forwards its (optional) options parameter, so an
			// omitted options argument still arrives as an explicit `undefined` —
			// divergence from the Svelte source's single-argument `soundFx.play(cue)`.
