 ...
 *   3. ornaments — functional components the skin injects into fixed
 *                  component slots for DOM that differs per skin (Brutal's
 *                  arrow, Terminal's caret).
 *
...
export interface SkinOrnaments {
	...
	buttonTrailing?: FunctionalComponent<RecipeArgs>;
	/** Eyebrow row at the top of Card (Brutal dot + mono label + / NN index). */
	cardEyebrow?: FunctionalComponent<{ index?: number; label?: string }>;
}
