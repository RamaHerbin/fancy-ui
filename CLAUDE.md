# Project: inspira-sveltekit

## Purpose
This repository ports selected components from the InspiraUI (Vue/Nuxt) project into idiomatic Svelte components using SvelteKit, Tailwind CSS, and shadcn-svelte.
...
- `src/lib/inspira/`  
  Svelte ports of InspiraUI components (one folder per component)

- `src/lib/examples/`  
  Svelte ports of InspiraUI example compositions (usage demos)

- `src/lib/blocks/`  
  Svelte ports of InspiraUI blocks (hero sections, testimonials, etc.)
...
### For UI Components (`vendor/inspira/ui/` → `src/lib/inspira/`)
...
3. Reimplement in Svelte:
   - `src/lib/inspira/x/X.svelte`
   - `src/lib/inspira/x/index.ts` (re-exports)
4. Create a demo page:
   - `src/routes/demo/x/+page.svelte`
5. Add short notes:
   - `src/lib/inspira/x/README.md`
...
### File Structure
```
src/lib/inspira/component-name/
...
```svelte
<script lang="ts">
  import { ComponentName } from '$lib/inspira/component-name';
</script>

<svelte:head>
  <title>ComponentName - Inspira Svelte</title>
</svelte:head>
