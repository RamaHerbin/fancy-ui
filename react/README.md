...
- **confetti**: BLOCKING PACKAGING GAP: `canvas-confetti` (+ `@types/canvas-confetti`) is NOT declared in react/package.json. Both files import it exactly as the Svelte source does; it only resolves today because the monorepo root node_modules hoists the Svelte package's copy. `fancy-ui-react` will fail to install/resolve for consumers until `canvas-confetti` is added to `dependencies` (and `@types/canvas-confetti` to devDependencies). I was forbidden from editing package.json — orchestration must add it.
...
- **displacement-text**: BLOCKER for publish: `react/package.json` declares no `three`. The Svelte package has `"three": "^0.183.2"` in `dependencies` and `react/vite.config.ts` already externalizes `/^three(\/|$)/`, so the same entry must be added to `react/package.json` dependencies centrally. It currently resolves only because the monorepo root `node_modules` has three@0.183.2 + @types/three@0.183.1 installed for the Svelte package; a standalone `fancy-ui-react` install would fail at runtime.
...
- **editorial-engine**: `@chenglou/pretext` is NOT declared in `react/package.json` `dependencies` (no package.json edits were sanctioned). `react/vite.config.ts` already lists `/^@chenglou\/pretext(\/|$)/` in `rollupOptions.external`, so the build treats it as a peer of the consumer — but nothing installs it for them. Consumers of `fancy-ui-react` importing `EditorialEngine` will hit an unresolved module until it is added to `dependencies`. Needs a package.json follow-up by whoever owns that file.
...
- **editorial-engine**: `react/README.md` was not updated (sanctioned deviation): EditorialEngine's props table / usage snippet still needs adding there. Same for `react/src/index.ts`, which is regenerated centrally.
...
- **image-trail-cursor**: react/package.json does not declare `gsap` — this is the package's first GSAP consumer. The import resolves today only because the pnpm workspace root installs gsap@^3.14.2 and node resolution walks up; `gsap` must be added to react/package.json (dependency or peerDependency) before publish or a consumer install breaks. vite.config.ts already externalises /^gsap(\/|$)/, so the build side is ready. I was barred from editing package.json.
...
- **line-reveal**: `@chenglou/pretext` is NOT declared in react/package.json dependencies. It only appears in vite.config.ts's `external` list. It resolves today via the pnpm workspace root node_modules, so tsc and vitest are green, but a published `fancy-ui-react` install would fail to resolve it. Someone with package.json ownership must add `@chenglou/pretext` to `dependencies` (Svelte package pins ^0.0.5; installed version here is 0.0.5).
...
- **matrix-rain**: React README not updated (orchestration forbids editing `react/README.md`); the `seed` divergence above is normally recorded there per PORTING.md.
...
- **neon-border**: Context correcting the finding's premise: neon-border is not uniquely missing. react/src/components/ has 83 folders and the barrel lists only 13; ~70 ported components are absent from it, so this is barrel lag across the whole wave.
- **neon-border**: Minor, left as-is: neon-border.css copies the Svelte <style> rules unanchored (.neon-layer-one, .neon-layer-two, .neon-animated) whereas marquee.css and ripple-button.css anchor under the root class per PORTING.md styling rule 2. meteors.css is unanchored the same way, so this reads as a wave-wide convention question rather than a neon-border defect; not touched since it was outside the reported failure.
- **neon-border**: UNFIXED / OUT OF SCOPE: react/src/index.ts is missing `export * from "./components/neon-border/index.js";` (belongs alphabetically between meteors and presence). Not applied — my task forbade barrel edits, the file's own header says GENERATED between migration waves, do not edit per-component by hand, and it is being written concurrently (13 component lines now vs 10 when I first read it this session). Needs the wave-level barrel regeneration, not a per-component patch.
...
- **noise-reveal**: `three` is imported but is NOT declared in react/package.json (no dependency/peerDependency entry) — it only resolves through the repo-root install in this worktree. Pre-existing gap shared with displacement-text; a consumer installing fancy-ui-react would fail to resolve it. Needs a package.json entry before publish (I was barred from editing package.json).
...
- **sidebar**: PACKAGE GAP (outside this folder, not fixed): react/tailwind.css declares --color-background/--color-primary/--color-primary-foreground/--color-ring but NOT --color-accent, --color-accent-foreground, --color-muted-foreground or --color-border. The sidebar spends bg-accent, text-accent-foreground, text-muted-foreground (+ /70) and border-border, so those resolve to nothing for a consumer without their own shadcn token set. PORTING.md §Styling rule 6 says the fix belongs in react/tailwind.css.
...
- **stepper**: Semantic Tailwind colour names `border-border`, `bg-border`, `text-foreground` and `text-muted-foreground` are used verbatim but are not declared in `react/tailwind.css` (which currently maps only background/primary/primary-foreground/ring). Per PORTING.md §Styling rule 6 they should be added there; the file is outside this port's ownership. Pre-existing: `dialog` and `button-group` already ship the same names.
