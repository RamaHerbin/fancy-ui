 ...
 * Import-time silence.
 *
 * The whole sound family may be imported — by the package barrel, by a Next.js
 * layout, by a consumer's client component — without constructing an
 * AudioContext, without reading or writing `localStorage`, and without
 * attaching a single window listener. Preferences hydrate lazily, on the first
 * getter or method call, and exactly once.
 *
 * Every spy here is installed BEFORE `vi.resetModules()` + the dynamic import,
 * which is the only ordering that can observe module-evaluation side effects.
 * `sound.ssr.test.ts` covers the DOM-less server path; this file is its
 * browser-side twin, and the two are the only places module evaluation itself
 * is under test.
 *
 * Transposed from `src/lib/fancy-ui/sound/sound.barrel.test.ts`. The only
 * changes are the module list (`sound.svelte.ts` → `sound.ts`, `SoundToggle`
 * is a `.tsx`, and React adds `use-sound.ts`) — no React runs here at all.
 */
