...
// plain dimensions, so it is fully testable without a DOM. `attachAnchorPosition`
// is the framework-free core that wires it up to a live anchor element; the
// React binding lives in `use-anchor-position.ts`.
//
// Usage:
...
export interface AnchorPositionHandle {
	/** Swaps the options and recomputes — the action's `update` under a new name. */
...
export function attachAnchorPosition(
	...
	if (typeof window === "undefined") {
		return { update() {}, recompute() {}, destroy() {} };
	}
	...
	return {
		update(newOpts: AnchorPositionOptions) {
			...
			// Without this, the idiomatic call site — an inline arrow in the
			// options object, rebuilt on every re-render — silently never
			// fires after the first one: `reportedSide` is scoped to the node,
			// not to the callback, so the gate that exists to suppress
			// duplicate reports to the *same* listener ends up suppressing the
			// first report to a *different* one. Unreachable from the React
			// hook, whose `onPlacement` goes through `useEventCallback` and so
			// keeps one identity for the life of the component; kept because
			// the core is public to any caller that hands over a fresh arrow.
