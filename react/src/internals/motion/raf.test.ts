...
// Frames are captured rather than actually run, so `runFrames()` only advances
// when a test says so. Kept as a Map (id → callback) rather than a plain array
// so the cancelAnimationFrame mock can remove a specific pending callback — a
