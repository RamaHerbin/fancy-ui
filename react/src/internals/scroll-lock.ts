...
/**
 * Acquires the scroll lock while mounted and `enabled` (default true).
 *
 * TIMING RULE FOR PORTS: call it INSIDE the presence-mounted subtree — the
 * panel component that `usePresence` keeps alive through the exit — and do
 * NOT pass `open`. Release timing is the whole point: an effect keyed on
 * `open` releases the instant `open` flips, leaving the page scrollable under
 * a scrim that is still on screen. Mounting scope is what holds the lock for
 * the length of the fade instead. `enabled` exists only for non-animated
 * surfaces (a `duration: 0` drawer, a reduced-motion path).
 *
 * Goes on the PANEL, never the scrim — the panel is the component that
 * already carries the portal, the focus trap and the dismissable layer.
 *
 * Reference-counted through `lockScroll()`, so nesting is unchanged, and a
 * layout effect rather than a passive one: a lock applied after paint is a
 * visible one-frame scroll jump on a long page.
 */
