...
export function nearestIndex(slots: string[], value: string | null | undefined): number {
	...
	for (let i = 0; i < slots.length; i++) {
		// The `!` is the package's `noUncheckedIndexedAccess` tax on a loop
		// already bounded by `slots.length` — a type-level edit only, the
		// runtime behaviour is the source's unchanged.
