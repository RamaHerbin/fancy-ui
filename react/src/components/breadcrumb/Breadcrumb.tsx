...
export const Breadcrumb = forwardRef<HTMLElement, BreadcrumbProps>(function Breadcrumb(
	...
	return (
		<nav ref={ref} aria-label={label} className={cn("ft-breadcrumb", className)}>
				...
				{rows.map((row, i) => (
					<Fragment key={row.key}>
						{i > 0 && (
							...
							<li aria-hidden="true" className="text-muted-foreground/50 select-none">
								{separator}
							</li>
						)}
						{"kind" in row ? (
							...
							<li className="flex items-center">
								{item ? (
									...
									<span className="text-muted-foreground">{row.item.label}</span>
								)}
							</li>
						)}
					</Fragment>
