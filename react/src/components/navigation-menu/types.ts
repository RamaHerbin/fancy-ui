 ...
 *
 * The source publishes both contracts under a `unique symbol` context key;
 * React's own context object plays that role here, so the exported names are
 * kept and the values are `React.Context` objects rather than symbols:
 *
 * ```tsx
 * const root = useContext(NAVIGATION_MENU_KEY);
 * ```
 */
...
export interface NavigationMenuContext {
	...
	/**
	 * Consumes the focus request for `itemValue`, if there is one. Returns
	 * whether there was one.
	 *
	 * Its identity changes whenever the pending request changes, which is how a
	 * panel effect that lists it as a dependency learns a new request arrived —
	 * the React stand-in for the source reading a `$state` inside its `$effect`.
	 */
...
export const NAVIGATION_MENU_KEY = createContext<NavigationMenuContext | undefined>(undefined);
NAVIGATION_MENU_KEY.displayName = "NavigationMenuContext";
...
export const NAVIGATION_MENU_ITEM_KEY = createContext<NavigationMenuItemContext | undefined>(
	undefined
);
NAVIGATION_MENU_ITEM_KEY.displayName = "NavigationMenuItemContext";

/**
 * Reads the enclosing `NavigationMenu`'s root context.
 *
 * Every compound piece below the root is only ever rendered inside it — the
 * same assumption the source's own `getContext` calls make, which throw
 * nothing and simply hand back `undefined` — so a missing provider is a
 * wiring mistake worth a named error rather than a shape to degrade into.
 * Internal: not exported from `index.ts`.
 */
export function useNavigationMenuContext(): NavigationMenuContext {
	const value = useContext(NAVIGATION_MENU_KEY);
	if (value === undefined) {
		throw new Error(
			"NavigationMenuContext is missing: this component must be rendered inside a <NavigationMenu>."
		);
	}
	return value;
}
...
/**
 * Reads the enclosing `NavigationMenuItem`'s context. Internal: not exported
 * from `index.ts`.
 */
export function useNavigationMenuItemContext(): NavigationMenuItemContext {
	const value = useContext(NAVIGATION_MENU_ITEM_KEY);
	if (value === undefined) {
		throw new Error(
			"NavigationMenuItemContext is missing: this component must be rendered inside a <NavigationMenuItem>."
		);
	}
	return value;
}
