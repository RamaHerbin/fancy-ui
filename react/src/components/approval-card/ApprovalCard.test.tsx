 ...
 * prove the decision travels back out to the consumer rather than merely
 * changing what the card draws. Mirrors the Svelte harness's `bind:state`.
 */
function Harness({ title }: { title: string }) {
	const [state, setState] = useState<ApprovalState>("pending");
	return (
		<>
			<ApprovalCard title={title} state={state} onStateChange={setState} />
...
describe("ApprovalCard", () => {
	...
	it("shows the description only when there is one", () => {
		const withDetail = render(
			<ApprovalCard title={TITLE} description="Adds two columns to `invoices`." />
		);
		...
		cleanup();
		const without = render(<ApprovalCard title={TITLE} />);
	...
	it("offers both decisions while pending, with the labels it was given", () => {
		const { container } = render(
			<ApprovalCard title={TITLE} approveLabel="Run it" denyLabel="Not now" />
		);
	...
	it("fires onApprove and resolves the card", async () => {
		...
		const onDeny = vi.fn();
		const { container } = render(
			<ApprovalCard title={TITLE} onApprove={onApprove} onDeny={onDeny} />
		);
	...
	it("writes the decision back out through the binding", async () => {
		const { container } = render(<Harness title={TITLE} />);
		expect(screen.getByTestId("bound-state").textContent).toBe("pending");

		await fireEvent.click(approve(container));
		expect(screen.getByTestId("bound-state").textContent).toBe("approved");
	});
	...
	it("follows a state prop driven from outside", () => {
		...
		rerender(<ApprovalCard title={TITLE} state="approved" />);
	...
	it("collapses to a quiet approved line, with no buttons left to press", () => {
		const { container } = render(<ApprovalCard title={TITLE} state="approved" />);
	...
	it("collapses to a muted denied line, with no buttons left to press", () => {
		const { container } = render(<ApprovalCard title={TITLE} state="denied" />);
	...
	it("disables both buttons and marks the card busy while the consumer works", () => {
		const { container } = render(<ApprovalCard title={TITLE} busy />);
	...
	it("refuses a decision that arrives while busy", async () => {
		const onApprove = vi.fn();
		const { container } = render(<ApprovalCard title={TITLE} busy onApprove={onApprove} />);
	...
	it("marks the approve button and the card destructive", () => {
		const { container } = render(<ApprovalCard title={TITLE} destructive />);
	...
	it("renders the detail region it is handed", () => {
		const { container } = render(
			<ApprovalCard title={TITLE}>
				<pre className="preview">pnpm db:migrate --prod</pre>
			</ApprovalCard>
		);
	...
	it("merges custom classes onto the root", () => {
		const { container } = render(<ApprovalCard title={TITLE} className="my-gate" />);
