...
describe("SubagentList", () => {
	...
	it("renders one row per agent", () => {
		const { container } = render(<SubagentList agents={agents} />);
	...
	it("drops the plural for a single running worker", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "running" }), agent({ id: "b", status: "done" })]} />
		);
	...
	it("still names an empty list", () => {
		const { container } = render(<SubagentList agents={[]} />);
	...
	it("lets an explicit label win over the derived one", () => {
		const { container } = render(<SubagentList agents={agents} label="Research fan-out" />);
	...
	it.each([
		...
		const { container } = render(<SubagentList agents={[agent({ status })]} />);
	...
	it("draws the progress bar at the reported fraction", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "running", progress: 0.42 })]} />
		);
	...
	it.each([
		...
		const { container } = render(
			<SubagentList agents={[agent({ status: "running", progress })]} />
		);
	...
	it("shows the status word instead when a running worker reports no progress", () => {
		const { container } = render(<SubagentList agents={[agent({ status: "running" })]} />);
	...
	it("keeps the bar off a finished worker even when it left a progress value behind", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "done", progress: 1 })]} />
		);
	...
	it("drops the task line in compact mode, keeping the name and the status", () => {
		const { container } = render(<SubagentList agents={agents} compact />);
	...
	it("says nothing twice on a row that already captions itself", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "done" }), agent({ id: "b", status: "pending" })]} />
		);
	...
	it("speaks the status of a row whose caption was replaced by a progress bar", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "running", progress: 0.62 })]} onSelect={vi.fn()} />
		);
	...
	it("does not repeat the progress bar's own announcement on a non-selectable row", () => {
		const { container } = render(
			<SubagentList agents={[agent({ status: "running", progress: 0.62 })]} />
		);
	...
	it("keeps the rows already on screen keyed the same when a worker is spawned", () => {
		const { container, rerender } = render(<SubagentList agents={agents} />);
		const first = rows(container)[0];

		rerender(<SubagentList agents={[...agents, agent({ id: "d", name: "Auditor" })]} />);
	...
	it("merges custom classes onto the root", () => {
		const { container } = render(<SubagentList agents={agents} className="my-fanout" />);
	...
	it("keeps a row's key across an insertion above it, and survives colliding ids", () => {
		const { container, rerender } = render(
			<SubagentList
				agents={[agent({ id: "a", name: "Alpha" }), agent({ id: "b", name: "Bravo" })]}
			/>
		);
