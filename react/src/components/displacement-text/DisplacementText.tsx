...
export function DisplacementText({
	...
	// source's `$effect` does: any of the six props changing disposes the old
	// scene and builds a new one rather than updating it incrementally. The
	// dependency array is the same set the `$effect` reads.
	useEffect(() => {
		...
		const onPointerMove = (e: PointerEvent) => {
			...
			// Non-null assertions: `uniforms` is a string index signature in the
			// type definitions, so `noUncheckedIndexedAccess` widens both reads
			// to `| undefined`. Both keys are declared just above.
