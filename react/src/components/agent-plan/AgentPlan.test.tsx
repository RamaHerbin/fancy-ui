...
describe("AgentPlan", () => {
	...
	it('heads the plan with its label, defaulting to "Plan"', () => {
		const { container } = render(<AgentPlan steps={[step()]} />);
	...
	it("uses the label it is given and names the list with it", () => {
		const { container } = render(<AgentPlan steps={[step()]} label="Migration plan" />);
	...
	it("counts substeps in the n/m total, not just the top-level steps", () => {
		const { container } = render(<AgentPlan steps={nested()} />);

		expect(count(container)).toBe("2/5");
		expect(rows(container)).toHaveLength(5);
	});

	it("recounts when a step finishes", () => {
		const { container, rerender } = render(<AgentPlan steps={nested()} />);
		expect(count(container)).toBe("2/5");

		const finished = nested();
		finished[2]!.status = "done";
		rerender(<AgentPlan steps={finished} />);
	...
	it.each([
		...
		const { container } = render(<AgentPlan steps={[step({ status })]} />);
	...
	it("names each status out loud, since the glyph is hidden from assistive tech", () => {
		const { container } = render(
			<AgentPlan steps={[step({ status: "error" }), step({ id: "s2", status: "cancelled" })]} />
		);
	...
	it("flattens anything nested deeper than one level instead of indenting further", () => {
		...
		const { container } = render(<AgentPlan steps={steps} />);
	...
	it("marks nothing current when the plan has not started", () => {
		const { container } = render(<AgentPlan steps={[step(), step({ id: "s2" })]} />);
	...
	it("renders the detail line under the step it belongs to", () => {
		const { container } = render(
			<AgentPlan steps={[step({ detail: "tests/retry.spec.ts" })]} />
		);
	...
	it("mutes a finished step without striking it through", () => {
		const { container } = render(
			<AgentPlan steps={[step({ status: "done" }), step({ id: "s2", status: "running" })]} />
		);
	...
	it("leaves the bar empty rather than dividing by nothing", () => {
		const { container } = render(<AgentPlan steps={[]} />);
	...
	it("drops the bar when progress is turned off, keeping the count", () => {
		const { container } = render(<AgentPlan steps={nested()} showProgress={false} />);
	...
	it("turns every row into a button when onSelect is supplied", () => {
	...
	it("lets the item render prop replace the row body, keeping the glyph", () => {
		const { container } = render(
			<AgentPlan
				steps={nested()}
				item={(stepArg, index) => (
	...
	it("speaks the status of a custom row, since the sr-only label lives only in the default body", () => {
		const { container } = render(
			<AgentPlan
				...
				item={() => <span className="custom-row">mine</span>}
			/>
		);
	...
	it("renders a plan that repeats an id instead of crashing on it", () => {
		...
		// Keying on the id alone would collide before a single row reached the
		// DOM, and the ids here come from a model.
		const { container } = render(<AgentPlan steps={steps} />);
	...
	it("keeps the rows already on screen keyed the same when the plan grows", () => {
		const { container, rerender } = render(<AgentPlan steps={nested()} />);
		const first = rows(container)[0];

		rerender(<AgentPlan steps={[...nested(), step({ id: "s4", label: "Open a PR" })]} />);
	...
	it("keeps a row's key stable when a step is inserted earlier in the plan", () => {
		const { container, rerender } = render(<AgentPlan steps={nested()} />);
		...
		rerender(
			<AgentPlan steps={[step({ id: "s0", label: "Set up the sandbox" }), ...nested()]} />
		);
	...
	it("merges custom classes onto the root", () => {
		const { container } = render(<AgentPlan steps={[step()]} className="my-plan" />);
