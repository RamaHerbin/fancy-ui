 ...
 * of a root publishing context to children a caller composes by hand.
 ...
 * reach across — both call sites are this folder's own root components — so
 * passing props directly says the same thing a context would, with one less
 * indirection.
 *
 * Not exported from `index.ts`: `DialogSurface` is an implementation
 * detail, not part of the package's public surface.
 *
 * One field of the source interface is deliberately absent: `ref`. A
 * bindable element reference is spelled `forwardRef` here (PORTING.md
 * §"API contract"), so the panel element arrives through the component's
 * ref channel rather than through a prop.
 */
export interface DialogSurfaceProps {
	 ...
	 * list). Forwarded to `useFocusTrap`'s `fallbackFocus` — see that file's
