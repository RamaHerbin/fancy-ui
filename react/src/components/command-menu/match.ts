...
export function getMatchRange(label: string, query: string): MatchRange | null {
	...
	// worse than no highlight — `CommandMenu.tsx`'s
	...
	// Port note: this package's tsconfig DOES enable `noUncheckedIndexedAccess`
	// (the Svelte side's does not, which is what the comment above records), so
	// the read is spelled out as a local and the `undefined` case folds into
	// the same `label.length` branch the bounds check already selects. Same
	// values out, in every case — `nextStart` cannot be `undefined` while
	// `endFoldedIndex < map.length` holds.
	const endFoldedIndex = index + foldedQuery.length;
	const nextStart = map[endFoldedIndex];
	const end = endFoldedIndex < map.length && nextStart !== undefined ? nextStart : label.length;
	return { start, end };
}
