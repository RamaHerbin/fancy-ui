...
/**
 * Test-only rig echoing the controlled `state` into the DOM — the only way to
 * prove the answer travels back out to the consumer rather than merely changing
 * what the card draws. Mirrors the Svelte harness's `bind:state`.
 */
function Harness({ title }: { title: string }) {
	const [state, setState] = useState<RecommendationState>("open");
	return (
		<>
			<RecommendationCard title={title} state={state} onStateChange={setState} />
...
describe("RecommendationCard", () => {
	...
	it("renders the description under the title", () => {
		const { container } = render(
			<RecommendationCard
				title={TITLE}
				description="The last three slow queries scanned the whole table."
			/>
		);
	...
	it("renders the badge as a kicker above the title", () => {
		const { container } = render(<RecommendationCard title={TITLE} badge="Suggestion" />);
	...
	it.each([
		...
		const { container } = render(<RecommendationCard title={TITLE} confidence={value} />);

		const block = confidence(container) as HTMLElement;
		expect(block.getAttribute("aria-label")).toBe(`Confidence ${percent}%`);
		expect(block.dataset.band).toBe(String(className).replace("ft-status-", ""));
		expect(block.textContent).toContain("%");
		expect(ring(container)?.classList.contains(String(className))).toBe(true);
	});
	...
	it("treats a confidence that is not a real number as no confidence at all", () => {
		const { container } = render(<RecommendationCard title={TITLE} confidence={Number.NaN} />);
	...
	it("paints the ring empty first, so the fill has somewhere to run from", async () => {
		const { container } = render(<RecommendationCard title={TITLE} confidence={0.8} />);
	...
	it("sweeps more of the ring for a higher confidence", async () => {
		const high = render(<RecommendationCard title={TITLE} confidence={0.8} />);
		const low = render(<RecommendationCard title={TITLE} confidence={0.3} />);
		await settle();
	...
	it("hides the counter and the ring behind the one spoken figure", () => {
		const { container } = render(<RecommendationCard title={TITLE} confidence={0.87} />);
	...
	it("takes custom labels for both buttons", () => {
		const { container } = render(
			<RecommendationCard title={TITLE} acceptLabel="Apply patch" dismissLabel="Not now" />
		);
	...
	it("calls onAccept and marks the card accepted", async () => {
		...
		const { container } = render(<RecommendationCard title={TITLE} onAccept={onAccept} />);
	...
	it("calls onDismiss and marks the card dismissed", async () => {
		...
		const { container } = render(<RecommendationCard title={TITLE} onDismiss={onDismiss} />);
	...
	it("writes an acceptance back out through the binding", async () => {
		const { container, getByTestId } = render(<Harness title={TITLE} />);
	...
	it("collapses to a quiet applied line, with no buttons left to press", () => {
		const { container } = render(<RecommendationCard title={TITLE} state="accepted" />);
	...
	it("collapses to an even quieter dismissed line", () => {
		const { container } = render(<RecommendationCard title={TITLE} state="dismissed" />);
	...
	it("follows a state prop driven from outside", () => {
		const { container, rerender } = render(<RecommendationCard title={TITLE} state="open" />);
		expect(accept(container)).not.toBeNull();

		rerender(<RecommendationCard title={TITLE} state="accepted" />);
		expect(accept(container)).toBeNull();
		expect(resolved(container)?.textContent?.trim()).toBe("Applied");
	});
	...
	it("renders the children as the detail region", () => {
		const { container } = render(
			<RecommendationCard title={TITLE}>
				<code className="detail">CREATE INDEX ON orders (customer_id)</code>
			</RecommendationCard>
		);
