...
export function FluidCursor({
	...
	// The Svelte source derives these with plain `const`s in the instance
	// script, so they are computed once at init and a later prop change never
	...
	// Singleton registry and monotonic mount counter, so an async (WebGPU)
	// startup that finishes out of order cannot let an older instance destroy
	// a newer one.
	//
	// Per-instance, matching the Svelte source: it declares both in the
	// instance `<script>` rather than `<script module>`, so each component owns
	// its own pair and no instance ever sees another's cleanup. `allowMultiple`
	// is therefore inert on both sides. The bug is ported deliberately
	// (PORTING.md: port the bug, note it) — fixing it here would silently make
	// a second `<FluidCursor />` destroy the first in React and not in Svelte.
	const activeInstanceRef = useRef<(() => void) | null>(null);
	const mountCounterRef = useRef(0);

	// Cached pre-scaled colors to avoid re-parsing hex on every splat.
	const colorCacheRef = useRef<{
		colorIndex: number;
		cachedFluidColorHex: string | undefined;
		cachedFluidColor: ColorRGB | null;
		cachedFluidColorsRef: string[] | undefined;
		cachedFluidColorsScaled: ColorRGB[];
	}>({
		colorIndex: 0,
		cachedFluidColorHex: undefined,
		cachedFluidColor: null,
		cachedFluidColorsRef: undefined,
		cachedFluidColorsScaled: [],
	});

	// The props the running engine re-reads on every frame, mirroring the
	// Svelte source's reactive prop getters: `generateColor` reads the colors
	...
	function getCachedFluidColor(hex: string): ColorRGB {
		...
		if (cache.cachedFluidColor === null || cache.cachedFluidColorHex !== hex) {
			cache.cachedFluidColorHex = hex;
			cache.cachedFluidColor = getScaledColor(hex);
		}
		return cache.cachedFluidColor;
	}

	function getCachedFluidColors(colors: string[]): ColorRGB[] {
		const cache = colorCacheRef.current;
		if (cache.cachedFluidColorsRef !== colors) {
			cache.cachedFluidColorsRef = colors;
			cache.cachedFluidColorsScaled = colors.map((hex) => getScaledColor(hex));
		}
		return cache.cachedFluidColorsScaled;
	}
	...
	function generateColor(): ColorRGB {
		...
		c.r *= derived.clampedColorIntensity;
		c.g *= derived.clampedColorIntensity;
		c.b *= derived.clampedColorIntensity;
		return c;
	}
	...
	function registerInstance(cleanup: () => void): () => void {
		if (!allowMultiple) {
			...
			activeInstanceRef.current = cleanup;
			return () => {
				cleanup();
				if (activeInstanceRef.current === cleanup) {
	...
	// Mount-only, exactly like the Svelte `onMount`: every simulation prop is
	// captured here and a later change is ignored until the component is
	// re-keyed. Empty deps on purpose — re-running would tear down the GL
	// context and restart the fluid on every unrelated re-render.
	useEffect(() => {
		const canvasNode = canvasRef.current;
		if (!canvasNode) return;
		// Re-bound with an explicit non-null type: `startWebGl` below is a
		// hoisted function declaration, and TypeScript does not carry an outer
		// null check into one.
		const canvas: HTMLCanvasElement = canvasNode;

		const mountToken = ++mountCounterRef.current;
		...
		const config = {
			...
			PAUSED: false,
			BACK_COLOR: derived.resolvedBackColor,
			TRANSPARENT: transparent,
			DITHER: dither,
			DITHER_PIXEL_SIZE: derived.clampedDitherPixelSize,
			DITHER_LEVELS: derived.clampedDitherLevels,
		};
		...
		if (hdr && !dither && typeof navigator !== "undefined" && "gpu" in navigator) {
			...
			startWebGpuFluid(canvas, {
				...
				exposure: derived.clampedHdrBoost,
				contained,
				interactive,
				autoSplat,
				autoSplatInterval,
				...
				.then((handle) => {
					...
					if (!allowMultiple && mountToken !== mountCounterRef.current) {
						handle?.cleanup();
						return;
					}
					if (!handle) {
	...
	return (
		...
		>
			<canvas
				ref={canvasRef}
				className={contained ? "block h-full w-full" : "block h-screen w-screen"}
			/>
		</div>
	);
}
 ...
 */
export const FluidCursorAdvanced = FluidCursor;

/**
 * @deprecated Use `FluidCursorProps` instead. `FluidCursorAdvanced` has been merged into `FluidCursor`.
 */
export interface FluidCursorAdvancedProps extends FluidCursorProps {}
