...
describe("Slider", () => {
	...
	it("respects custom min, max and step", () => {
		const { container } = render(<Slider min={10} max={20} step={0.5} value={15} />);
		const el = slider(container);
	...
	it("keeps aria-valuemin, aria-valuemax and aria-valuenow in sync with the bounds and value", () => {
		const { container } = render(<Slider min={0} max={50} value={30} />);
		const el = slider(container);
	...
	it("calls onValueChange with the new value on input, matching a step-sized move", async () => {
		...
		const { container } = render(<Slider value={40} step={5} onValueChange={onValueChange} />);
		const el = slider(container);
	...
	it("works with a plain non-bound value plus a callback", async () => {
		const onValueChange = vi.fn();
		const { container } = render(<Slider value={20} onValueChange={onValueChange} />);
		const el = slider(container);
	...
	it("blocks the callback while disabled", async () => {
		const onValueChange = vi.fn();
		const { container } = render(<Slider disabled onValueChange={onValueChange} />);
		const el = slider(container);
	...
	it("clamps aria-valuenow and the input's value when the prop starts out of range", () => {
		const onValueChange = vi.fn();
		const { container } = render(<Slider value={150} max={100} onValueChange={onValueChange} />);
		const el = slider(container);

		expect(el.getAttribute("aria-valuenow")).toBe("100");
		expect(el.value).toBe("100");
		// Also corrects the internal state itself, the same way NumberInput
		// clamps on blur, so a caller's own bound variable does not stay
		// silently out of range.
		expect(onValueChange).toHaveBeenCalledWith(100);
	});

	it("clamps the value (and calls onValueChange) when max shrinks below the current value at runtime", () => {
		const onValueChange = vi.fn();
		const { container, rerender } = render(
			<Slider value={80} max={100} onValueChange={onValueChange} />
		);
		const el = slider(container);
		expect(el.getAttribute("aria-valuenow")).toBe("80");

		rerender(<Slider value={80} max={50} onValueChange={onValueChange} />);
	...
	it("keeps the showValue bubble in sync with the clamped value, not the raw out-of-range prop", () => {
		const { container } = render(<Slider value={150} max={100} showValue />);
		const bubble = container.querySelector(".ft-slider-bubble");
		expect(bubble?.textContent?.trim()).toBe("100");
	});

	it("round-trips value through the harness's own state", async () => {
		const { container, getByTestId } = render(<SliderHarness />);
		const el = slider(container);
	...
	it("shows the value bubble only when showValue is set, tracking the current value", () => {
		const { container, rerender } = render(<Slider value={60} showValue={false} />);
		expect(container.querySelector(".ft-slider-bubble")).toBeNull();

		rerender(<Slider value={60} showValue={true} />);
		const bubble = container.querySelector(".ft-slider-bubble");
	...
	it("shows min/max end labels only when showBounds is set", () => {
		const { container: withoutBounds } = render(<Slider min={0} max={100} showBounds={false} />);
		expect(withoutBounds.querySelectorAll(".ft-slider-wrap > div")).toHaveLength(1);

		const { container } = render(<Slider min={0} max={100} showBounds />);
	...
	it("works standalone: useField() has no provider, so its own props apply untouched", () => {
		const { container } = render(<Slider id="solo" disabled={false} />);
		const el = slider(container);
	...
	it("inside a FormField, the context wins for controlId, aria-describedby, aria-invalid and disabled", () => {
		...
		const { container } = render(<SliderFieldHarness context={context} />);
		const el = slider(container);
