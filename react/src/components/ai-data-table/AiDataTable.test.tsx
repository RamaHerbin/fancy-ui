...
import { AiDataTable } from "./AiDataTable.js";
import type { AiDataTableColumn, AiDataTableRow } from "./AiDataTable.js";
...
describe("AiDataTable", () => {
	...
	it("renders one header per column, in order, inside a real table", () => {
		const { container } = render(<AiDataTable columns={COLUMNS} rows={ROWS} />);
	...
	it("reads an empty string as the model saying nothing, like a null", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={[{ option: "Managed cluster", sla: "" }]} />
		);
	...
	it("treats a key the row never mentioned as empty rather than crashing", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={[{ option: "Only a name" }]} />
		);
	...
	it("keeps the caption out of sight but in the accessibility tree by default", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={ROWS} caption="Deployment options" />
		);
	...
	it("tints one column top to bottom, and only that one", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={ROWS} highlightColumn="latency" />
		);
	...
	it("tints nothing when the highlight key matches no column", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={ROWS} highlightColumn="nope" />
		);
	...
	it("hands the cell renderer the raw value and where it sits", () => {
		const { container } = render(
			<AiDataTable
				columns={COLUMNS}
				rows={ROWS}
	...
	it("tightens the rows when dense", () => {
		const { container } = render(<AiDataTable columns={COLUMNS} rows={ROWS} dense={true} />);
	...
	it("survives a model that returned columns but no rows", () => {
		const { container } = render(<AiDataTable columns={COLUMNS} rows={[]} />);
	...
	it("merges custom classes onto the scrolling root", () => {
		const { container } = render(
			<AiDataTable columns={COLUMNS} rows={ROWS} className="my-table" />
		);
		const root = container.firstElementChild as HTMLElement;
