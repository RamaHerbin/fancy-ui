...
export function EditorialEngine({
	...
	// writes `pullquotes={["…"]}` inline hands a new array on every render, and
	// an identity-keyed effect would tear the engine down and re-measure the
	// whole article each time. The Svelte source re-runs its `$effect` on a
	// genuine prop reassignment only, so keying on content is what preserves
	// its observable behaviour here.
	const pullquotesKey = pullquotes.join("\u0000");
	...
	useEffect(() => {
		...
		// Text is measured against the canvas font engine — fonts must be
		...
		// `pullquotes` is read through `pullquotesKey`; see the note above.
