...
describe("FireworksHdr", () => {
	...
	it("applies a custom class name", () => {
		const { container } = render(<FireworksHdr className="my-fireworks" />);
		const div = container.firstElementChild as HTMLElement;
	...
	it("does not fire onReady when no GPU renderer is available (jsdom)", async () => {
		const onReady = vi.fn();
		render(<FireworksHdr onReady={onReady} />);
	...
	it("does not fire onLost when no engine ever booted (that is not a loss)", async () => {
		const onLost = vi.fn();
		render(<FireworksHdr onLost={onLost} />);
		await new Promise((resolve) => setTimeout(resolve, 0));
		expect(onLost).not.toHaveBeenCalled();
	});

	it("re-exports the figure helpers consumers are told to import", async () => {
		// The package barrel re-exports this folder's index, so what this module
		// exposes is exactly what `import { … } from "fancy-ui-react"` can reach.
	...
	it("does not attempt WebGPU when hdr={false}", () => {
		const { container } = render(<FireworksHdr hdr={false} />);
		expect(container.querySelector("canvas")).toBeInTheDocument();
	});

	it("unmounts without throwing", () => {
		const { unmount } = render(<FireworksHdr interactive ambient />);
	...
	it("mounts without error across quality tiers and reduced-motion", () => {
		for (const quality of ["auto", "high", "mid", "low"] as const) {
			const { container, unmount } = render(
				<FireworksHdr quality={quality} respectReducedMotion />
			);
			expect(container.querySelector("canvas")).toBeInTheDocument();
			unmount();
		}
...
// factory must return null without throwing, letting FireworksHdr try nothing
// further and stay silent. Real GPU behavior is covered by the app's Playwright
// e2e, not here.
...
describe("pattern shells", () => {
	...
	it("heart is upright, closed, and inside the unit circle", () => {
		...
		expect(
			Math.hypot(pts[0]!.x - heartOutline(1).x, pts[0]!.y - heartOutline(1).y)
		).toBeLessThan(1e-9);
...
describe("ringBurst", () => {
	...
	it("is angularly even to ±10%", () => {
		...
		for (let i = 1; i < dirs.length; i++) {
			const a0 = Math.atan2(dirs[i - 1]!.dir.y, dirs[i - 1]!.dir.x);
			const a1 = Math.atan2(dirs[i]!.dir.y, dirs[i]!.dir.x);
			let diff = a1 - a0;
