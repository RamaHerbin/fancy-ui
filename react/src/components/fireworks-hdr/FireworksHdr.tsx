...
export function FireworksHdr({
	...
	const qualityRef = useLiveRef(quality);
	const interactiveRef = useLiveRef(interactive);
	const handleReady = useEventCallback(onReady);
	const handleLost = useEventCallback(onLost);

	// Mount/unmount only, matching the source's onMount + its returned teardown:
	// the engine is booted once and no prop change re-inits it. The refs above
	// are what keeps the long-lived closures current, so the dependency list is
	// honestly empty rather than artificially frozen.
	useEffect(() => {
		const node = canvasRef.current;
		if (!node) return;
		// Re-bound with a non-nullable declared type: the helpers below are hoisted
		// function declarations, which TypeScript analyses independently of the
		// guard above and would otherwise still see `HTMLCanvasElement | null`.
		...
		function activate(eng: FireworksEngineHandle) {
			...
			const q = resolveQualityTier(qualityRef.current, level, currentDpr(), minDim, lowPower);
		...
		async function boot(): Promise<boolean> {
			...
			if (!eng) {
				...
				const minDim = Math.min(canvas.clientWidth, canvas.clientHeight);
				const webglTier = resolveQualityTier(
					qualityRef.current,
					"webgl-p3",
					currentDpr(),
					minDim,
