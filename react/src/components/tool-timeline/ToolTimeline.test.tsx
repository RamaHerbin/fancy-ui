...
describe("ToolTimeline", () => {
	...
	it("renders one row per item", () => {
		const { container } = render(<ToolTimeline items={items} />);
	...
	it("renders no stats for items without them", () => {
		const { container } = render(
			<ToolTimeline items={[{ id: "a", verb: "Read", target: "a.ts" }]} />
		);
	...
	it("renders each stat independently of the other", () => {
		const { container } = render(
			<ToolTimeline items={[{ id: "a", verb: "Deleted", target: "old.ts", deletions: 41 }]} />
		);
		expect(container.querySelector(".ft-tooltimeline-additions")).toBeFalsy();
	...
	it("renders a zero stat rather than treating it as absent", () => {
		const { container } = render(
			<ToolTimeline
				items={[{ id: "a", verb: "Edited", target: "a.ts", additions: 0, deletions: 3 }]}
			/>
		);
	...
	it("hides the detail line in compact mode", () => {
		const { container } = render(<ToolTimeline items={items} compact />);
	...
	it("renders the item render prop in place of the built-in row body", () => {
		const { container } = render(
			<ToolTimeline items={items} item={() => <span className="custom-row">custom</span>} />
		);
	...
	it("renders the timestamp as relative text with the exact time in the attributes", () => {
		...
		const { container } = render(
			<ToolTimeline items={[{ id: "a", verb: "Read", target: "a.ts", timestamp }]} />
		);
	...
	it("names the list Activity by default and takes an override", () => {
		...
		const named = render(<ToolTimeline items={items} label="Session log" />);
	...
	it("merges custom class names alongside the base classes", () => {
		const { container } = render(<ToolTimeline items={items} className="my-timeline" />);
	...
	it("ages its timestamps while it stays mounted", async () => {
		...
		const { container } = render(
			<ToolTimeline
				items={[{ id: "a", verb: "Read", target: "a.ts", timestamp: T0 - 20_000 }]}
			/>
		);
