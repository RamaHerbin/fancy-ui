...
/** Flushes the microtasks a transition leg finishes on, inside act, so the
 *  settle's state updates land where React expects them. */
...
/**
 * Test-only rig proving PasswordInput consumes the shared field context
 * rather than throwing or ignoring it. Provides FieldContext by hand instead
 * of rendering a real FormField — this wave's components are built against
 * the frozen FieldContext surface, not against each other, so a fake
 * provider here is the one way to test the consumer side in isolation.
 *
...
function FieldHarness({
	...
	return (
		<FieldProvider value={context}>
			<PasswordInput id={id} invalid={invalid} required={required} disabled={disabled} />
		</FieldProvider>
	);
}
...
describe("PasswordInput", () => {
	...
	it("renders a real input, type password, resting", () => {
		const { container } = render(<PasswordInput placeholder="Enter your password" />);
	...
	it("readonly stays focusable and keeps its name, unlike disabled — and still submits", () => {
		const { container } = render(
			<PasswordInput readonly name="password" defaultValue="hunter2" />
		);
		const el = input(container);
	...
	it("calls onValueChange with the new value on input", async () => {
		...
		const { container } = render(<PasswordInput defaultValue="" onValueChange={onValueChange} />);
		const el = input(container);
	...
	it("works with a plain uncontrolled initial value plus a callback", async () => {
		const onValueChange = vi.fn();
		const { container } = render(
			<PasswordInput defaultValue="start" onValueChange={onValueChange} />
		);
		const el = input(container);
	...
	it("blocks the callback while disabled, even from a synthetic input event", async () => {
		const onValueChange = vi.fn();
		const { container } = render(<PasswordInput disabled onValueChange={onValueChange} />);
		const el = input(container);
	...
	it("merges the className prop with the base classes on the field surface", () => {
		const { container } = render(<PasswordInput className="mt-4" />);
	...
	it("works standalone: useField() has no provider, so its own props apply untouched", () => {
		const { container } = render(
			<PasswordInput id="solo" invalid required disabled={false} />
		);
		const el = input(container);
	...
	describe("reveal toggle", () => {
		...
		it("is absent when showToggle is false", () => {
			const { container } = render(<PasswordInput showToggle={false} />);
		...
		it("is disabled, and inert, while the field is disabled", async () => {
			const { container } = render(<PasswordInput disabled defaultValue="hunter2" />);
			const el = input(container);
			const btn = toggleButton(container)!;
	...
	describe("strength meter", () => {
		...
		it("is absent while the field is empty, even with showStrength set", () => {
			const { container } = render(<PasswordInput showStrength defaultValue="" />);
			expect(container.querySelector(".ft-password-strength")).toBeNull();
		});

		it("labels a short, single-class password as very weak", () => {
			const { container } = render(<PasswordInput showStrength defaultValue="a" />);
		...
		it("labels a long password mixing all character classes as strong", () => {
			const { container } = render(<PasswordInput showStrength defaultValue="Abcdefghijkl1!" />);
		...
		it("changes label as the value changes", async () => {
			...
			fireEvent.input(el, { target: { value: "Abcdefghijkl1!" } });
			expect(strengthLabel(container)?.textContent?.trim()).toBe("Strong");
		});
		...
		it("a custom strength function overrides the built-in scorer entirely", () => {
			...
			const { container } = render(
				<PasswordInput showStrength defaultValue="a" strength={strength} />
			);
		...
		it("gives every segment the ft-password-strength-bar hook the colour transition attaches to", () => {
			...
			// strong-tier modifier, so the shared hook is what the stylesheet's
	...
	describe("motion", () => {
			...
			try {
				...
				for (const [i, expected] of labels.entries()) {
					...
					if (i === 0) {
						...
						// on its way out. That is what "cross-fade" means, and it is
						// the assertion an enter-only animation could not satisfy —
						// with an enter-only mechanism the outgoing layer never
						// animates at all, so this set would hold exactly one element.
						// Read off the spy's receivers rather than by counting DOM
						// nodes mid-flight, since the overlap is a microtask wide.
						// `mock.contexts` is the `this` of each call — the element
						// `animate()` ran on. It is typed `unknown` (lib.dom declares
						// no `this` parameter), hence the narrowing. Both legs start
						// inside the click's own layout effects, so the receivers are
						// already recorded here — no waiting mid-flight.
		...
		it("with prefers-reduced-motion: reduce, the icon swaps instantly — one layer throughout, no animation", async () => {
			...
			try {
				...
				// `duration: 0` makes the transition runner take its synchronous
				// path, so the outgoing layer is gone in the same commit the
				// incoming one mounts — the swap still happens, it just does not
				// fade.
				expect(eyeLayers(container)).toHaveLength(1);
				expect(el.type).toBe("text");
				expect(btn.getAttribute("aria-label")).toBe("Hide password");
		...
		it("keeps the strength meter's colour easing outside the reduced-motion branch — the bars still restate on every tier change", async () => {
			...
			try {
				...
				fireEvent.input(input(container), { target: { value: "Abcdefghijkl1!" } });
