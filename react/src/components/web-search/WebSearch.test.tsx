...
describe("WebSearch", () => {
	...
	it("renders the query in the header", () => {
		const { container } = render(<WebSearch query={QUERY} results={[]} />);
		expect(text(container, ".ft-websearch-query")).toBe(QUERY);
	});

	it("renders one row per result, in order", () => {
		const { container } = render(<WebSearch query={QUERY} results={RESULTS} />);
	...
	it("counts the results in the header, singular and plural", () => {
		const { container, rerender } = render(<WebSearch query={QUERY} results={RESULTS} />);
		expect(text(container, ".ft-websearch-count")).toBe("3 results");

		rerender(<WebSearch query={QUERY} results={RESULTS.slice(0, 1)} />);
	...
	it("drops the domain line and falls back to the title when the url is blank", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={[{ id: "r1", title: "Local note", url: "" }]} />
		);
	...
	it("survives a url it cannot parse", () => {
		const { container } = render(
			<WebSearch
				query={QUERY}
				results={[{ id: "r1", title: "Relative", url: "/not/absolute" }]}
			/>
		);
	...
	it("keeps the rows already on screen when a result is appended", async () => {
		...
		const { container, rerender } = render(<WebSearch query={QUERY} results={first} />);

		const before = items(container);
		expect(before).toHaveLength(2);

		rerender(<WebSearch query={QUERY} results={[...first, RESULTS[2]!]} />);
	...
	it("says it is searching while nothing has landed yet", () => {
		const { container } = render(<WebSearch query={QUERY} results={[]} searching />);
	...
	it("reports an empty result set once the search is over", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={[]} searching={false} />
		);
	...
	it("renders an inert row when there is neither a url nor an onSelect", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={[{ id: "r1", title: "Nowhere to go", url: "" }]} />
		);
	...
	it("calls onSelect with the result and its index", async () => {
		...
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} onSelect={onSelect} />
		);
	...
	it("clamps to maxVisible and offers the rest behind an expander", async () => {
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} maxVisible={1} />
		);
	...
	it("points the expander at the list it controls", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} maxVisible={2} />
		);
	...
	it("treats a maxVisible of zero or less as no cap at all", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} maxVisible={0} />
		);
		expect(items(container)).toHaveLength(3);
		...
		const negative = render(
			<WebSearch query={QUERY} results={RESULTS} maxVisible={-3} />
		);
	...
	it("keeps the expander honest as results keep landing", async () => {
		const { container, rerender } = render(
			<WebSearch query={QUERY} results={RESULTS.slice(0, 2)} maxVisible={1} />
		);
		expect(more(container)?.textContent?.trim()).toBe("Show 1 more");

		rerender(<WebSearch query={QUERY} results={RESULTS} maxVisible={1} />);
	...
	it("retires the expansion when the list empties, so the next search starts capped", async () => {
		...
		rerender(<WebSearch query={QUERY} results={[]} maxVisible={1} />);
	...
	it("renders the item render-prop in place of the built-in row body", () => {
		const item = (result: SearchResultData, index: number) => (
		...
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} item={item} />
		);
	...
	it("exposes a labelled group, overridable through the label prop", () => {
		...
		const named = render(
			<WebSearch query={QUERY} results={RESULTS} label="Sources consulted" />
		);
	...
	it("merges the class prop with the base layout classes", () => {
		const { container } = render(
			<WebSearch query={QUERY} results={RESULTS} className="mt-4" />
		);
	...
	describe("model-supplied result urls", () => {
		const one = (url: string) =>
			render(
				<WebSearch query="runes" results={[{ id: "r", title: "A result", url }]} />
			);
