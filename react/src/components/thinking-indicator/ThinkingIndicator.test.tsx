...
describe("ThinkingIndicator", () => {
	...
	it("renders the status label", () => {
		const { container } = render(<ThinkingIndicator status="Reading files" />);
	...
	it("formats an externally supplied elapsedMs", () => {
		const { container } = render(
			<ThinkingIndicator status="Searching the web" elapsedMs={65_000} />
		);
	...
	it("ticks the elapsed time forward from since while running", async () => {
		vi.useFakeTimers();
		vi.setSystemTime(T0);
		const { container } = render(<ThinkingIndicator status="Reading files" since={T0} />);
	...
	it("leaves the internal timer stopped when elapsedMs overrides it", async () => {
		...
		vi.setSystemTime(T0);
		const { container } = render(
			<ThinkingIndicator status="Reading files" since={T0} elapsedMs={7000} />
		);
	...
	it("hides the ticking timer from assistive tech only while running", () => {
		const running = render(<ThinkingIndicator status="Thinking" elapsedMs={4000} />);
		expect(
		...
		const stopped = render(
			<ThinkingIndicator status="Thinking" elapsedMs={4000} running={false} />
		);
		expect(
	...
	it("drops the shimmer class and starts no timer when running is false", async () => {
		...
		vi.setSystemTime(T0);
		const { container } = render(
			<ThinkingIndicator status="Reading files" since={T0} running={false} />
		);
	...
	it("renders the done content instead of the status once stopped", () => {
		const { container } = render(
			<ThinkingIndicator
				status="Reading files"
				running={false}
				done={<span>Thought for 12s</span>}
			/>
		);
	...
	it("keeps showing the status while running even when done content is provided", () => {
		const { container } = render(
			<ThinkingIndicator status="Reading files" done={<span>Thought for 12s</span>} />
		);
	...
	it("adds the pill class and a pulse dot for the pill variant", () => {
		const { container } = render(<ThinkingIndicator status="Reading files" variant="pill" />);
	...
	it("stills the pulse dot when the pill is not running", () => {
		const { container } = render(
			<ThinkingIndicator status="Reading files" variant="pill" running={false} />
		);
	...
	it("hides the elapsed time when showElapsed is false", () => {
		const { container } = render(
			<ThinkingIndicator status="Reading files" elapsedMs={9000} showElapsed={false} />
		);
		expect(container.querySelector(".ft-thinking-elapsed")).toBeFalsy();
	});
	...
	it("merges custom class names alongside the base classes", () => {
		const { container } = render(
			<ThinkingIndicator status="Reading files" className="my-indicator" />
		);
		const root = container.firstElementChild as HTMLElement;
	...
	it("shows 0s when it mounts already finished, whatever since says", () => {
		...
		vi.setSystemTime(T0);
		const { container } = render(
			<ThinkingIndicator status="Reading files" since={T0 - 5000} running={false} />
		);
