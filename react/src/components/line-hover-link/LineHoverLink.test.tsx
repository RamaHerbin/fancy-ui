...
describe("LineHoverLink", () => {
	...
	it("renders an anchor element", () => {
		const { container } = render(<LineHoverLink href="/about" />);
		expect(container.querySelector("a")).toBeTruthy();
	});

	it("applies the variant class", () => {
		const { container } = render(<LineHoverLink variant="slide" href="#" />);
	...
	it("reduced motion: every variant still renders the markup its effect hangs off", () => {
		...
		try {
			for (const variant of ["slide", "pulse", "sweep", "bounce", "ink"] as const) {
				const { container, unmount } = render(<LineHoverLink variant={variant} href="#" />);
