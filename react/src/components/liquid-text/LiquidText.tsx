...
// gaussian one. The full pass order is documented on the shader constants
// and on `step()` below.
// ===========================================================================
...
export function LiquidText({
	...
	// Always starts "static" so the very first render (SSR or hydration) never
	// touches window/document — the mount effect promotes to "canvas" if
	// eligible.
	const [mode, setMode] = useState<"static" | "canvas">("static");
	const [themeColor, setThemeColor] = useState(lightColor);
	const [resolvedFont, setResolvedFont] = useState(font);
	const [displayFontSize, setDisplayFontSize] = useState(fontSize > 0 ? fontSize : 64);

	// Synchronous mirrors of the three values above. A Svelte `$state`
	// assignment is readable on the very next line; a React setter is not, and
	// `resizeAll` below reads the fitted font size immediately after
	// `fitFontSize()` writes it. The refs are what the sim reads, the state is
	// what the fallback markup renders — they are always written together.
	const themeColorRef = useRef(themeColor);
	const resolvedFontRef = useRef(resolvedFont);
	const displayFontSizeRef = useRef(displayFontSize);

	// Hoisted so the prop-reactivity effect below can push a re-rasterize into
	// the live WebGL texture after mount. Assigned by setupCanvasSim once the
	// sim is running; stays null on the static-fallback path, where updating
	// the state above is enough since the fallback is just reactive markup.
	const rasterizeRef = useRef<(() => void) | null>(null);

	// Every prop the running sim reads live. The Svelte source reads the
	// destructured `$props()` bindings straight from its long-lived closures
	// (the pointer handler, the rAF loop, the observers) and therefore always
	// sees the current value; a React `[]`-deps effect would otherwise freeze
	// the mount-time values into those same closures.
	...
	const rootHeight = `${Math.round(displayFontSize * 1.2)}px`;

	const activeThemeColor = useCallback((): string => {
		return typeof document !== "undefined" && document.documentElement.classList.contains("dark")
			? live.current.darkColor
			: live.current.lightColor;
	}, [live]);

	const applyThemeColor = useCallback((next: string) => {
		themeColorRef.current = next;
		setThemeColor(next);
	}, []);

	// Recomputes resolvedFont + displayFontSize together (using a local
	// `family` const rather than re-reading the resolved font it just wrote) so
	// this can safely run inside the reactive effect below without the effect
	// both writing and re-reading the same piece of state in one pass.
	const fitFontSize = useCallback(() => {
		const rootEl = rootRef.current;
		if (!rootEl) return;
		const { font, fontSize, fontWeight, text } = live.current;
		const family = font || getComputedStyle(rootEl).fontFamily || "sans-serif";
		resolvedFontRef.current = family;
		setResolvedFont(family);
		if (fontSize > 0) {
		...
		const next = Math.min(Math.max(fitted, 8), 2000);
		displayFontSizeRef.current = next;
		setDisplayFontSize(next);
	}, [live]);

	// Per the "text/font changes re-rasterize" requirement: the WebGL path
	// otherwise only reads text/font/fontSize/fontWeight/lightColor/darkColor
	// once, at setup, so updating them in e.g. a docs control would silently do
	// nothing. Listing the props as this effect's dependencies (rather than
	...
	// Mount-only. `staticBelow` is deliberately read from the mount-time
	...
	useEffect(() => {
		...
		/**
		 * Attempts to acquire a WebGL context and, if successful, boots the
		 * fluid sim + render loop and switches `mode` to "canvas". Returns a
		 * cleanup function on success, or `null` if WebGL is unavailable
		 * (caller should keep the static DOM fallback).
		 *
		 * `startFallbackResize` is the fallback-resize starter above, threaded
		 * through so the context-loss handler below can re-arm it when it drops
		 * the sim to the static fallback mid-session — it closes over this
		 * effect's `root`/`fallbackResizeObserver`, so calling it here keeps
		 * that observer reachable from the effect's own cleanup return, and
		 * it's a no-op if the observer is already attached.
		 */
		function setupCanvasSim(
			...
			// Declared here (outside the try below) so both the success path
			// and the failure-recovery catch block can safely reference them —
			// `let`/`function` declared inside a `try { … }` are not visible
			// from its `catch { … }`, since try/catch are separate block
			// scopes.
			...
			// synchronous setup pass, so a later failure (a program that fails
			// to compile/link, exhausted buffers, …) can free everything
			// created before it instead of stranding those objects on the
			// canvas's still-live GL context. Cleared once setup fully
			// succeeds.
			...
			try {
				...
				function rasterizeText() {
					...
					ctx.font = `${live.current.fontWeight} ${displayFontSizeRef.current * dpr}px ${resolvedFontRef.current}`;
					ctx.fillStyle = themeColorRef.current;
					ctx.textAlign = "left";
					ctx.textBaseline = "middle";
					ctx.fillText(live.current.text, 0, h / 2);
				...
				function resizeAll() {
					...
					cssHeight = Math.round(displayFontSizeRef.current * 1.2) || 1;
				...
				resizeAll();
				setMode("canvas");
				rasterizeRef.current = rasterizeText;
				...
				function onPointerMove(e: PointerEvent) {
					...
					if (pointerHasPrev) {
						pointerForce.x = (x - pointerPrevUV.x) * live.current.forceGain;
						pointerForce.y = (y - pointerPrevUV.y) * live.current.forceGain;
						pointerMoved = true;
					}
					pointerPrevUV.x = x;
					pointerPrevUV.y = y;
				...
				// docs control) takes effect immediately instead of only
				// reflecting its value at initial setup.
				...
				// below, so toggling the prop after mount takes effect
				// immediately instead of only reflecting its value at initial
				// setup.
				...
				function applySplat() {
					...
					splatProgram.set1f("radius", live.current.radius);
				...
				function diffuseVelocity(dt: number) {
					...
					const alpha = live.current.viscosity * dt;
				...
				function step(dt: number) {
					...
					advectionProgram.set1f("dissipation", live.current.dissipation);
					blit(velocity.write);
					velocity.swap();
				...
				function draw() {
					...
					displayProgram.set1f("strength", live.current.strength);
					displayProgram.set1f("chromaticRatio", live.current.chromaticRatio);
					blit(null);
				}
				...
				function frame() {
					...
					if (live.current.pauseWhenHidden && hidden) return;
					step(FIXED_DT);
					draw();
				}

				// Tears down the running sim's listeners/GL resources. Shared
				// by the context-loss handler below (which must stop issuing
				// calls against a dead context) and the normal unmount cleanup.
				function teardownSim() {
					...
					disposeGeometry();
					rasterizeRef.current = null;
				}

				// A real GPU context loss (driver reset, GPU switch, too many
				// live contexts, …) must not leave the rAF loop silently
				// issuing calls against a dead context, and — without
				// preventDefault() — the context would never become eligible
				// for restoration. Recovery here is deliberately conservative:
				// every GL object created above is invalidated by the loss, so
				// rather than rebuilding the whole sim in place this drops to
				// the stable static DOM fallback for the remainder of this
				// mount.
				...
				// exhausted geometry buffers, a texture allocation failure
				// inside the initial resizeAll(), …), so a failed init doesn't
				// strand them on the canvas's still-live context. The
				// disposeOnFailure entries are each individually guarded since
				// how far setup got before throwing determines which of them
				// actually ran; the sim buffers/texture are unconditionally
				// declared above (outside this try), so they're always safe to
				// tear down here.
	...
	return (
		<div
			ref={rootRef}
			className={cn("liquid-text relative block w-full", className)}
